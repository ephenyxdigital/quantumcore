    ...
    $.each($rowData, function(index, value) { 
		...
		$.ajaxq("TranslateProcess", {
            ...
            if(current == totalExpressions) {
                $('#grid_AdminBackTab .pq-grid-title').html(value.name + ' '+translation_complete);
            }
...
function saveGridTranslations() {
	...
	$.ajax({
		...
		complete: function complete(data) {
			$('#uperTranslatetabs').remove();
			$('#contentTranslatetabs').remove();
			$('#uperAdminTranslations a').trigger('click');
		}
	});	
}

function saveGridTranslationsAndStay() {
