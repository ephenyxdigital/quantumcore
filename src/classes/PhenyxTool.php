...
class PhenyxTool {
    ...
    $path              = null,
    $rewrite_settings  = null,
    $cache_control     = null,
    $specific          = '',
    $disable_multiviews = null,
    $medias            = false,
    $disable_modsec    = null
): bool {
    ...
    // EPH_HTACCESS_SECURITY_HEADERS peut ne pas exister en base — on sécurise le get()
    $security_headers = (int) ($this->context->phenyxConfig->get('EPH_HTACCESS_SECURITY_HEADERS') ?? 0);

    // ── Préservation du contenu hors-balises ─────────────────────────────────

    $specific_before = $specific_after = '';

    if (file_exists($path)) {
        $content = static::isSubmit('htaccess')
            ? static::getValue('htaccess')
        ...
        if (preg_match('#^(.*)\# ~~start~~.*\# ~~end~~[^\n]*(.*)$#s', $content, $m)) {
            ...
            '#\# http://www\.ephenyx\.com - http://www\.ephenyx\.com/forums\s*(.*)<IfModule mod_rewrite\.c>#si',
            $content, $m
        )) {
            $specific_before = $m[1];
        } else {
    ...
    // ── Collecte des domaines ────────────────────────────────────────────────

    $domains = [];

    foreach (CompanyUrl::getCompanyUrls() as $company_url) {
        foreach ([$company_url->domain, $company_url->domain_ssl] as $dom) {
    ...
    // Déduplication (quand domain == domain_ssl, on a inséré deux fois la même entrée)
    foreach ($domains as $dom => &$uris) {
        $seen = [];
        $unique = [];
        foreach ($uris as $uri) {
            $key = $uri['physical'] . '|' . $uri['virtual'] . '|' . $uri['id_company'];
            if (!isset($seen[$key])) {
                $seen[$key] = true;
                $unique[] = $uri;
            }
    ...
    $buf = '';
    $w   = function (string $line = '') use (&$buf): void {
        $buf .= $line . "\n";
    };
    ...
    if ($specific_before) {
        $buf .= trim($specific_before) . "\n\n";
    }

    $w('# ~~start~~ Do not remove this comment, Ephenyx Digital will keep automatically the code outside this comment when .htaccess will be generated again');
    $w('# .htaccess automatically generated by Ephenyx Digital open-source solution');
    $w('# https://ephenyx.com - https://forum.ephenyx.com');
    $w();

    // ── Sécurité : protection des fichiers sensibles ──────────────────────────
    // Note : ces règles sont compatibles Apache 2.2 et 2.4.
    // Si votre hébergeur surcharge AllowOverride et bloque <FilesMatch>,
    // retirez ce bloc et gérez la protection au niveau du VirtualHost.
    $w('# ── Protection des fichiers sensibles ──────────────────────────────────────');
    $w('<FilesMatch "(\.(env|log|sql|bak|gz|tar|swp|orig)|composer\.(json|lock))$">');
    ...
    // config.inc.php et defines_inc.php dans un bloc séparé pour faciliter le debug
    $w('<FilesMatch "^(config\.inc|defines_inc|defines_uri_inc)\.php$">');
    $w('    <IfModule mod_authz_core.c>');
    $w('        Require all denied');
    $w('    </IfModule>');
    $w('    <IfModule !mod_authz_core.c>');
    ...
    $w('# ── Méthodes HTTP non autorisées ───────────────────────────────────────────');
    $w('<IfModule mod_rewrite.c>');
    $w('    RewriteCond %{REQUEST_METHOD} ^(TRACE|TRACK)');
    $w('    RewriteRule .* - [F,L]');
    $w('</IfModule>');
    $w();
    ...
    $w('# ── WebP : MIME type & négociation de contenu ──────────────────────────────');
    $w('<IfModule mod_mime.c>');
    $w('    AddType image/webp .webp');
    $w('    AddType image/avif .avif');
    $w('</IfModule>');
    $w();
    $w('<IfModule mod_setenvif.c>');
    $w('    SetEnvIf Request_URI "\.(jpe?g|png)$" REQUEST_image');
    $w('</IfModule>');
    $w();
    $w('<IfModule mod_headers.c>');
    $w('    Header append Vary Accept env=REQUEST_image');
    $w('</IfModule>');
    $w();

    // ── MIME types polices ───────────────────────────────────────────────────

    $w('# ── MIME types polices ─────────────────────────────────────────────────────');
    $w('AddType application/vnd.ms-fontobject .eot');
    $w('AddType font/ttf                       .ttf');
    $w('AddType font/otf                       .otf');
    $w('AddType font/woff                      .woff');
    $w('AddType font/woff2                     .woff2');
    $w();

    // ── En-têtes de sécurité (optionnel) ─────────────────────────────────────

    if ($security_headers) {
        $w('# ── En-têtes de sécurité ───────────────────────────────────────────────────');
        $w('<IfModule mod_headers.c>');
        $w('    Header always set X-Content-Type-Options "nosniff"');
        $w('    Header always set X-Frame-Options "SAMEORIGIN"');
        $w('    Header always set X-XSS-Protection "1; mode=block"');
        $w('    Header always set Referrer-Policy "strict-origin-when-cross-origin"');
        $w('    Header always set Permissions-Policy "geolocation=(), microphone=(), camera=()"');
        $w('</IfModule>');
        $w();
    }

    // ── CORS polices — restreint au domaine principal ────────────────────────

    // Récupère le premier domaine pour le CORS des polices
    reset($domains);
    $first_domain_key  = key($domains);
    $cors_font_origin  = 'https://' . $first_domain_key;

    $w('# ── CORS polices (restreint au domaine principal) ──────────────────────────');
    $w('<IfModule mod_headers.c>');
    $w('    <FilesMatch "\.(ttf|ttc|otf|eot|woff|woff2|svg)$">');
    $w('        Header set Access-Control-Allow-Origin "' . $cors_font_origin . '"');
    $w('    </FilesMatch>');
    $w('</IfModule>');
    $w();

    // ── mod_security (uniquement si explicitement activé) ────────────────────

    if ($disable_modsec) {
        $w('# ── mod_security : désactivé par configuration EPH_HTACCESS_DISABLE_MODSEC ──');
        $w('# ATTENTION : cette directive réduit la protection du serveur.');
        $w('# Ne l\'activer que si mod_security génère des faux positifs avérés.');
        $w('<IfModule mod_security.c>');
        $w('    SecFilterEngine Off');
        // SecFilterScanPOST intentionnellement NON désactivé (protège les POST)
        $w('</IfModule>');
        $w();
    }

    // ── Options Apache (niveau RACINE — jamais dans un bloc IfModule) ────────
    // Options doit être à la racine du .htaccess.
    // Sur un hébergement mutualisé, nécessite AllowOverride Options dans le VirtualHost.
    if ($disable_multiviews) {
        $w('# Désactivation de MultiViews (évite les conflits de routage)');
        $w('Options -Multiviews');
        $w();
    }
    ...
    $w('<IfModule mod_env.c>');
    $w('    SetEnv HTTP_MOD_REWRITE On');
    $w('</IfModule>');
    $w();
    ...
    $w('# Substitution WebP si disponible et acceptée par le navigateur');
    $w('RewriteCond %{HTTP_ACCEPT} image/webp');
    $w('RewriteCond %{DOCUMENT_ROOT}/$1.webp -f');
    $w('RewriteRule (.+)\.(jpe?g|png)$ $1.webp [T=image/webp,L]');
    $w();

    // ── Webservice API — écrit UNE seule fois avant la boucle domaines ────────

    $w('# ── Webservice Ephenyx Digital ─────────────────────────────────────────────');
    $w('RewriteRule ^api/?$ %{ENV:REWRITEBASE}webephenyx/dispatcher.php [QSA,L]');
    $w('RewriteRule ^api/(.*)$ %{ENV:REWRITEBASE}webephenyx/dispatcher.php?url=$1 [QSA,L]');
    $w();

    // ── CGI Authorization header ─────────────────────────────────────────────

    if ($this->context->phenyxConfig->get('EPH_WEBSERVICE_CGI_HOST')) {
        $w('# Transmission du header Authorization en mode CGI');
        $w('RewriteCond %{HTTP:Authorization} ^(.*)');
        $w('RewriteRule . - [E=HTTP_AUTHORIZATION:%1]');
        $w();
    }
    ...
    $domain_rewrite_cond = ''; // initialisé pour le hook

    foreach ($domains as $domain => $list_uri) {
        $w('#Domain: ' . $domain);
        $domain_rewrite_cond = 'RewriteCond %{HTTP_HOST} ^' . preg_quote($domain, null) . '$ [NC]';

        foreach ($list_uri as $uri) {

            $w('RewriteRule . - [E=REWRITEBASE:' . $uri['physical'] . ']');
            $w();

            // ── Routes spécifiques ephenyx.io ────────────────────────────────

            if ($domain === 'ephenyx.io') {
                $w('# Routes spécifiques ephenyx.io');
                $w('RewriteRule ^veille/?$ %{ENV:REWRITEBASE}webephenyx/veille.php [QSA,L]');
                $w('RewriteRule ^veille/(.*)$ %{ENV:REWRITEBASE}webephenyx/veille.php?url=$1 [QSA,L]');
                $w();
                $w('RewriteRule ^css/?$ %{ENV:REWRITEBASE}webephenyx/css.php [QSA,L]');
                $w('RewriteRule ^css/(.*)$ %{ENV:REWRITEBASE}webephenyx/css.php?url=$1 [QSA,L]');
                $w();
                $w('# Sous-domaine cdn.ephenyx.io → ressource/');
                $w('RewriteCond %{HTTP_HOST} ^cdn\.ephenyx\.io$ [NC]');
                $w('RewriteRule ^(.*)$ https://ephenyx.io/ressource/$1 [L,R=301,NC,QSA]');
                $w();
                $w('# Sous-domaine fonts.ephenyx.io');
                $w('RewriteCond %{HTTP_HOST} ^fonts\.ephenyx\.io$ [NC]');
                $w('RewriteRule ^(.*)$ %{ENV:REWRITEBASE}webephenyx/fonts.php?url=$1 [QSA,L]');
                $w();
            ...
            // ── URI virtuelle multishop ───────────────────────────────────────

            if ($uri['virtual']) {
                $virtual_trimmed = trim($uri['virtual'], '/');

                if (!$rewrite_settings) {
                    $w($domain_rewrite_cond);
                    $w('RewriteRule ^' . $virtual_trimmed . '/?$ ' . $uri['physical'] . $uri['virtual'] . 'index.php [L,R=301]');
                } else {
                    $w($domain_rewrite_cond);
                    $w('RewriteRule ^' . $virtual_trimmed . '$ ' . $uri['physical'] . $uri['virtual'] . ' [L,R=301]');
                }

                $w($domain_rewrite_cond);
                $w('RewriteRule ^' . ltrim($uri['virtual'], '/') . '(.*) ' . $uri['physical'] . '$1 [L]');
                $w();
            }
        }

        // ── Hook addRewriteSeetings ───────────────────────────────────────────

        $addrewrite_settings = Hook::getInstance()->exec(
            'addRewriteSeetings',
            [
                'media_domains'       => '',
                'domain_rewrite_cond' => $domain_rewrite_cond,
                'uri'                 => $uri ?? [],
        ...
        if (is_array($addrewrite_settings)) {
            foreach ($addrewrite_settings as $settings) {
        ...
        if ($rewrite_settings) {
            $w('# AlphaImageLoader pour IE / fancybox');
            $w('RewriteRule ^images_ie/?([^/]+)\.(jpe?g|png|gif)$ js/jquery/plugins/fancybox/images/$1.$2 [L]');
            $w();
            $w('# Dispatcher principal : laisse passer les fichiers et répertoires existants');
            $w('RewriteCond %{REQUEST_FILENAME} -s [OR]');
            $w('RewriteCond %{REQUEST_FILENAME} -l [OR]');
            $w('RewriteCond %{REQUEST_FILENAME} -d');
            $w('RewriteRule ^ - [NC,L]');
            $w('RewriteRule ^ %{ENV:REWRITEBASE}index.php [NC,L]');
            $w();
        }
    }
    ...
    if ($cache_control) {
        $w('# ── Cache navigateur ───────────────────────────────────────────────────────');
        $w('<IfModule mod_expires.c>');
        $w('    ExpiresActive On');
        $w('    # Images');
        $w('    ExpiresByType image/gif          "access plus 1 month"');
        $w('    ExpiresByType image/jpeg         "access plus 1 month"');
        $w('    ExpiresByType image/png          "access plus 1 month"');
        $w('    ExpiresByType image/webp         "access plus 1 month"');
        $w('    ExpiresByType image/avif         "access plus 1 month"');
        $w('    ExpiresByType image/svg+xml      "access plus 1 year"');
        $w('    ExpiresByType image/x-icon       "access plus 1 year"');
        $w('    ExpiresByType image/vnd.microsoft.icon "access plus 1 year"');
        $w('    # CSS / JS');
        $w('    ExpiresByType text/css                  "access plus 1 week"');
        $w('    ExpiresByType text/javascript            "access plus 1 week"');
        $w('    ExpiresByType application/javascript     "access plus 1 week"');
        $w('    ExpiresByType application/x-javascript   "access plus 1 week"');
        $w('    # Polices');
        $w('    ExpiresByType font/woff                          "access plus 1 year"');
        $w('    ExpiresByType font/woff2                         "access plus 1 year"');
        $w('    ExpiresByType font/ttf                           "access plus 1 year"');
        $w('    ExpiresByType font/otf                           "access plus 1 year"');

        $w('    ExpiresByType application/font-woff              "access plus 1 year"');
        $w('    ExpiresByType application/x-font-woff            "access plus 1 year"');
        $w('    ExpiresByType application/vnd.ms-fontobject      "access plus 1 year"');
        $w('    ExpiresByType font/opentype                      "access plus 1 year"');
        $w('    ExpiresByType application/x-font-ttf             "access plus 1 year"');
        $w('    ExpiresByType application/x-font-otf             "access plus 1 year"');
        $w('</IfModule>');
        $w();

        // FileETag : on conserve "none" — syntaxe valide sur toutes versions Apache.
        // "FileETag MTime Size" sans tiret n'est pas reconnu avant Apache 2.4.x.
        $w('<IfModule mod_headers.c>');
        $w('    Header unset ETag');
        $w('</IfModule>');
        $w('FileETag none');
        ...
        $w('# ── Compression DEFLATE ────────────────────────────────────────────────────');
        $w('<IfModule mod_deflate.c>');
        $w('    <IfModule mod_filter.c>');
        $w('        AddOutputFilterByType DEFLATE text/html text/css text/javascript text/xml text/plain application/javascript application/x-javascript application/json application/xml image/svg+xml font/ttf application/x-font-ttf font/otf application/x-font-otf font/opentype');
        $w('    </IfModule>');
        $w('</IfModule>');
        $w();
    }
    ...
    reset($domains);
    $first_domain = current($domains);
    $w('#If rewrite mod isn\'t enabled');
    $w('ErrorDocument 404 ' . $first_domain[0]['physical'] . 'index.php?controller=404');
    $w();

    // ── Pied de page ─────────────────────────────────────────────────────────

    $w('# ~~end~~ Do not remove this comment, Ephenyx Digital will keep automatically the code outside this comment when .htaccess will be generated again');

    if ($specific_after) {
        $buf .= "\n" . trim($specific_after);
    }

    // ── Écriture atomique en une seule passe ─────────────────────────────────

    if (file_put_contents($path, $buf, LOCK_EX) === false) {
        return false;
    }
