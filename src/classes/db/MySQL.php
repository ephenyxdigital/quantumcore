...
use DbCore;
use PhenyxDatabaseExceptionException;
...
class MySQL extends Db {
    ...
    public function connect() {
        ...
        if (!$this->link = mysql_connect($this->server, $this->user, $this->password)) {
            throw new PhenyxDatabaseExceptionException(Tools::displayError('Link to database cannot be established.'));
        }

        if (!$this->set_db($this->database)) {
            throw new PhenyxDatabaseExceptionException(Tools::displayError('The database selection cannot be made.'));
        }

        // UTF-8 support

        if (!mysql_query('SET NAMES \'utf8\'', $this->link)) {
            throw new PhenyxDatabaseExceptionException(Tools::displayError('PhenyxShop Fatal error: no utf-8 support. Please check your server configuration.'));
        }
