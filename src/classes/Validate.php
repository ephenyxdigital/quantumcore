...
class Validate {
     ...
     *        that is the only RFC supported by ephenyx
     */
     ...
     * @version 1.8.1.0 Initial version
     */
    public static function isPluginUrl($url, &$errors) {

        if (!$url || $url == 'http://' || $url == 'https://') {
            $errors[] = Tools::displayError('Please specify plugin URL');
        } else if (substr($url, -4) != '.tar' && substr($url, -4) != '.zip' && substr($url, -4) != '.tgz' && substr($url, -7) != '.tar.gz') {
    ...
    /**
     * Check for plugin name validity
     *
     ...
     * @version 1.8.1.0 Initial version
     */
    public static function isPluginName($pluginName) {

        return (is_string($pluginName) && preg_match('/^[a-zA-Z0-9_-]+$/', $pluginName));
    }
    ...
    public static function isLinkRewrite($link) {
        if (!is_null($link)) {
            if (Context::getContext()->phenyxConfig->get('EPH_ALLOW_ACCENTED_CHARS_URL')) {
     ...
     * @return bool Validity is ok or not
     *
     * @since 1.9.1.0 Changed date in future check
     */
    ...
    /**
     * Check if $data is a PhenyxShop cookie object
     *
     ...
     * @version 1.8.1.0 Initial version
     */
    public static function isPhenyxShopVersion($version) {
    ...
    public static function isOrderInvoiceNumber($id) {
        ...
        return (preg_match('/^(?:' . Context::getContext()->phenyxConfig->get('EPH_INVOICE_PREFIX', Context::getContext()->language->id) . ')\s*([0-9]+)$/i', $id));
    }
