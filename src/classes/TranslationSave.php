...
class Translation extends PhenyxObjectModel {
    ...
    public static function getInstance($id = null, $isos = null) {
        ...
        if (static::$instance === null) {
            static::$instance = new Translation($id, $isos);
        }
	...
	public function add($autoDate = false, $nullValues = false) {
		...
		if($this->context->company->domain_ssl != 'ephenyx.io' && $this->context->company->target_domain != 'ephenyx.io')  {
			$url = _EPH_PHENYX_API_;
			$string = $this->context->phenyxConfig->get('_EPHENYX_LICENSE_KEY_').'/'.$this->context->phenyxConfig->get('EPH_SHOP_DOMAIN');
			$crypto_key = $this->context->_tools->encrypt_decrypt('encrypt', $string, _PHP_ENCRYPTION_KEY_, _COOKIE_KEY_);
			...
			// Timeouts explicites : un endpoint distant lent/injoignable ne doit
			// pas bloquer le worker PHP-FPM jusqu'à max_execution_time.
    ...
    public function getGlobalTranslations($isos = null) {
        ...
        foreach ($languages as $lang) {
            ...
            $translations[$iso] = Db::getInstance()->executeS(
                (new DbQuery())
                    ->select('*')
                    ->from('translation')
    ...
    public function getExistingTranslation($iso_code, $origin, $file = null) {
		...
		// Fix : condition toujours vraie avec ||. Le site maître ephenyx.io/.com
		// doit utiliser la base LOCALE ; seules les boutiques clientes appellent l'API.
		if($this->context->company->domain_ssl != 'ephenyx.io' && $this->context->company->target_domain != 'ephenyx.io')  {
			$url = _EPH_PHENYX_API_;
			$string = $this->context->phenyxConfig->get('_EPHENYX_LICENSE_KEY_').'/'.$this->context->phenyxConfig->get('EPH_SHOP_DOMAIN');
			$crypto_key = $this->context->_tools->encrypt_decrypt('encrypt', $string, _PHP_ENCRYPTION_KEY_, _COOKIE_KEY_);
        ...
        $translation = Db::getInstance()->getValue(
            (new DbQuery())
                ->select('`translation`')
                ->from('translation')
    ...
    public function getExistingObjectTranslation($iso_code, $origin, $file_name = null) {
		...
		if($this->context->company->domain_ssl != 'ephenyx.io' && $this->context->company->target_domain != 'ephenyx.io')  {
			...
			$data_array = [
				'crypto_key' => $crypto_key,
				'action' => 'getExistingObjectTranslation',
            	'iso_code' => $iso_code,
				'origin' => $origin,
				'file_name' => $file_name,
			];
        ...
        $id_translation = Db::getInstance()->getValue(
            ...
            ->where('`iso_code` = \'' . pSQL(trim($iso_code)) . '\'')
            ->where('`origin` = \'' . pSQL(trim($origin)) . '\'')
			->where(!is_null($file) ? '`file_name` = \'' . pSQL(trim($file_name)) . '\'' : 1)
        );
    ...
    public function getExistingTranslationByIso($iso_code) {
		...
		if($this->context->company->domain_ssl != 'ephenyx.io' && $this->context->company->target_domain != 'ephenyx.io')  {
			...
			$data_array = [
				'crypto_key' => $crypto_key,
				'action' => 'getExistingTranslationByIso',
            	'iso_code' => $iso_code
			];
     ...
     * @return bool
     */
    public function addTranslation($object) {

        $data        = Tools::jsonDecode(Tools::jsonEncode($object), true);
        $translation = new Translation();

        foreach ($data as $key => $value) {
            if (property_exists($translation, $key) && $key != 'id_translation') {
	...
	public function updateTranslation($object) {

        $data        = Tools::jsonDecode(Tools::jsonEncode($object), true);
        $translation = new Translation();

        foreach ($data as $key => $value) {
            if (property_exists($translation, $key) && $key) {
                $translation->{$key} = $value;
            }
        }
