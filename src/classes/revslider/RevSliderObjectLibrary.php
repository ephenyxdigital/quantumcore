...
class RevSliderObjectLibrary extends RevSliderFunction {
	...
	public $upload_dir;
	public $allowed_types = ['thumb', 'video', 'video_thumb'];
	...
	public function _get_list($force = false) {
		...
		if (time() - $last_check > 1296000 || $force == true) {
			//30 days
			RevLoader::update_option('revslider-library-check', time());
				...
				if ($response = RevLoader::maybe_unserialize($request['body'])) {
					...
					if ('actual' != $response) {
						...
						if (is_array($library)) {
							...
							if (isset($library['hash'])) {
								RevLoader::update_option('revslider-library-hash', $library['hash']);
							}
	...
	public function get_object_handle_by_id($id) {
		...
		$handle = '';
		$full = RevLoader::get_option('rs-library', []);
	...
	public function _get_object_thumb($object_handle, $type, $download = false) {
		...
		if ((int) ($object_handle) > 0) {
			$object_handle = $this->get_object_handle_by_id($object_handle);
		} else {
			//check if we are original image and if not change it to original image
		...
		if ($validated == 'false' && !in_array($type, $this->allowed_types, true)) {
			return ['error' => $this->l('Plugin not activated')];
		}
		...
		if ($_download && $download === true && RevLoader::wp_mkdir_p($this->upload_dir['basedir'] . $path)) {
			...
			if (!is_file($file)) {
				...
				if ($curl !== false) {
					...
					if ($validated == 'false' && !in_array($type, $this->allowed_types, true)) {
						$error = $this->l('Plugin not activated');
					} else {
						$rslb = new RevSliderLoadBalancer();
						$code = ($validated == 'false') ? '' : RevLoader::get_option('revslider-code', '');
						$rattr = [
						...
						$image_data = RevLoader::wp_remote_post($url, [
						...
						if (!RevLoader::is_wp_error($image_data) && isset($image_data['body']) && isset($image_data['response']) && isset($image_data['response']['code']) && $image_data['response']['code'] == '200') {
							...
							$image_data = false;
							$error = $this->l('Error downloading object');
						}
...
//use default image
	...
	public function _get_object_layers($object_id) {
		...
		if ((int) ($object_id) > 0) {
			...
			$error = $this->l('Error downloading layers');
			return ['error' => $error];
		}
		...
		if ($curl !== false) {
			...
			if ($validated == 'false' && !in_array($type, $this->allowed_types)) {
				...
				$url = 'http://revapi.smartdatasoft.net/v6/call/object.php?' . http_build_query($rattr);
				$layers_data = RevLoader::wp_remote_post($url, [
					'user-agent' => RevLoader::get_bloginfo('url'),
					'body'       => '',
					'timeout'    => 45,
				]);
				...
				if (!RevLoader::is_wp_error($layers_data) && isset($layers_data['body']) && isset($layers_data['response']) && isset($layers_data['response']['code']) && $layers_data['response']['code'] == '200') {
					...
					$layers_data = false;
					$error = $this->l('Error downloading layers data');
				}
		...
		//could not connect to server
		$error = ($layers_data === false && $error == '') ? $this->l('Error downloading layers data') : $error;
			...
			foreach ($data as $k => $v) {
				...
				if (!empty($svg_source)) {
					...
					if (is_array($t) && count($t) == 2) {
						$this->set_val($data, [$k, 'svg', 'source'], RS_PLUGIN_URL . $t[1]);
					}
	...
	public function load_objects_with_svg() {
		...
		$online = RevLoader::get_option('rs-library', []);
	...
	public function get_svg_categories() {
		...
		if (!empty($svgs)) {
			...
			foreach ($svgs as $cat => $svg) {
				...
				if (trim($cat) !== '' && !isset($svg_cat[$cat])) {
					$svg_cat[$cat] = ucwords($cat);
				}
	...
	public function get_correct_size_url($image_id, $size, $full = false) {
		...
		if (!in_array($size, $this->sizes) && $full === false) {
			return '';
		}
	...
	*/
	...
	public function retrieve_all_object_data() {
		...
		if (!empty($svgs) && is_array($svgs)) {
			...
			foreach ($svgs as $svghandle => $svgfiles) {
				$data['html'][] = ['type' => 'tag', 'handle' => $svghandle, 'name' => $svghandle];
				$data['html'][] = ['type' => 'inner'];
			...
			if (!empty($online) && is_array($online)) {
				...
				if (isset($obj['online']['tags'])) {
					...
					foreach ($obj['online']['tags'] as $t) {
						$data['html'][] = ['type' => 'tag', 'handle' => $t['handle'], 'name' => $t['name']];
					}
	...
	public function get_favorites() {
		...
		return RevLoader::get_option('rs_obj_favorites', []);
	}
	...
	public function get_svg_sets_url() {
		...
		$svg_sets['Actions'] = ['path' => $path . 'action/', 'url' => $url . 'action/'];
		$svg_sets['Alerts'] = ['path' => $path . 'alert/', 'url' => $url . 'alert/'];
		$svg_sets['AV'] = ['path' => $path . 'av/', 'url' => $url . 'av/'];
		$svg_sets['Communication'] = ['path' => $path . 'communication/', 'url' => $url . 'communication/'];
		$svg_sets['Content'] = ['path' => $path . 'content/', 'url' => $url . 'content/'];
		$svg_sets['Device'] = ['path' => $path . 'device/', 'url' => $url . 'device/'];
		$svg_sets['Editor'] = ['path' => $path . 'editor/', 'url' => $url . 'editor/'];
		$svg_sets['File'] = ['path' => $path . 'file/', 'url' => $url . 'file/'];
		$svg_sets['Hardware'] = ['path' => $path . 'hardware/', 'url' => $url . 'hardware/'];
		$svg_sets['Images'] = ['path' => $path . 'image/', 'url' => $url . 'image/'];
		$svg_sets['Maps'] = ['path' => $path . 'maps/', 'url' => $url . 'maps/'];
		$svg_sets['Navigation'] = ['path' => $path . 'navigation/', 'url' => $url . 'navigation/'];
		$svg_sets['Notifications'] = ['path' => $path . 'notification/', 'url' => $url . 'notification/'];
		$svg_sets['Places'] = ['path' => $path . 'places/', 'url' => $url . 'places/'];
		$svg_sets['Social'] = ['path' => $path . 'social/', 'url' => $url . 'social/'];
		$svg_sets['Toggle'] = ['path' => $path . 'toggle/', 'url' => $url . 'toggle/'];
					...
					while (false !== ($file = readdir($dir))) {
						...
						if ($file != '.' && $file != '..') {
							...
							if (isset($filetype['extension']) && $filetype['extension'] == 'svg') {
								...
								$title = str_replace(['px.svg', '.svg'], '', $title);
	...
	public function get_material_icons() {
		...
		return [
			...
			'verified_user', 'vertical_align_bottom', 'vertical_align_center', 'vertical_align_top', 'vertical_split', 'vibration', 'video_call', 'video_label', 'video_library', 'videocam', 'videocam_off', 'videogame_asset', 'view_agenda', 'view_array', 'view_carousel', 'view_column', 'view_comfy', 'view_compact', 'view_day', 'view_headline', 'view_list', 'view_plugin', 'view_quilt', 'view_stream', 'view_week', 'vignette', 'visibility', 'visibility_off', 'voice_chat', 'voice_over_off', 'voicemail', 'volume_down', 'volume_mute', 'volume_off', 'volume_up', 'vpn_key', 'vpn_lock',
