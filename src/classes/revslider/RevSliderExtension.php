...
class RevSliderExtension {
	...
	public function init_essential_grid_extensions() {
		...
		RevLoader::add_filter('essgrid_set_ajax_source_order', [$this, 'add_slider_to_eg_ajax']);
		RevLoader::add_filter('essgrid_handle_ajax_content', [$this, 'set_slider_values_to_eg_ajax'], 10, 4);
		RevLoader::add_action('essgrid_add_meta_options', [$this, 'add_eg_additional_meta_field']);
		RevLoader::add_action('essgrid_save_meta_options', [$this, 'save_eg_additional_meta_field'], 10, 2);

		//only do on frontend
		RevLoader::add_action('admin_head', [$this, 'add_eg_additional_inline_javascript']);
		RevLoader::add_action('wp_footer', [$this, 'add_eg_additional_inline_javascript']);
	}
	...
	public function add_slider_to_eg_ajax($media) {
		...
		$media['revslider'] = ['name' => $this->l('Slider Revolution'), 'type' => 'ccw'];
	...
	public function add_eg_additional_meta_field($values) {
		...
		?>
		<p>
			<strong style="font-size:14px"><?php _e('Choose Revolution Slider');?></strong>
		</p>
		<p>
			<select name="eg_sources_revslider" id="eg_sources_revslider">
				<option value=""<?php selected($selected_slider[0], '');?>><?php _e('--- Choose Slider ---');?></option>
				<?php
