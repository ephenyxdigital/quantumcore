...
class RevSliderOutput extends RevSliderFunction {
	...
	public $icon_sets = []; //RevSliderBase::set_icon_sets(array());
	 ...
	 **/
	public $rs_plugin_wrap_open = false;
	public $rs_plugin_open = false;
	public $rs_plugin_wrap_closed = false;
	public $rs_plugin_closed = false;
	public $rs_custom_navigation_css = '';

	/**
	 * ShortCode based Global Values
	 * usage : Is Plugin to emebed as Modal ?
	 * sc_layout : overwrite original plugin Layout,
	 * offset : padding and margin of the wrapping Plugin
	...
	public function set_html_id($html_id, $check_for_duplication = true) {
		...
		if ($check_for_duplication) {
			//check if it already exists, if yes change it and add attribute for console output
		...
		if (!in_array($html_id, $rs_ids_collection)) {
			$rs_ids_collection[] = $html_id;
		}
	...
	public function set_custom_settings($settings) {
		...
		$settings = ($settings !== '' && !is_array($settings)) ? json_decode(str_replace(['({', '})', "'"], ['[', ']', '"'], $settings), true) : $settings;
	...
	public function add_slider_base() {
		...
		try {
			...
			if (empty($this->slider)) {
				$slider = RevSliderSlider::getInstance();
				$slider_id = $slider->init_by_mixed($this->get_slider_id());
				$this->slider = new RevSliderSlider($slider_id);
			}

			$this->slider = RevLoader::apply_filters('revslider_add_slider_base', $this->slider);

			if ($this->slider->get_param(['general', 'disableOnMobile'], false) === true && RevLoader::wp_is_mobile()) {
				return false;
			}
			...
			if ($this->get_preview_mode()) {

				$this->modify_preview_mode_settings();
			...
			if ($this->ajax_loaded === true || $this->get_markup_export()) {
				...
				//if true, then we are loaded by ajax
			...
			if ($this->slider->get_param(['troubleshooting', 'jsInBody'], false) == true && $this->ajax_loaded === false) {
	...
	public function get_slider_wrapper_div() {
		...
		$position = $this->slider->get_param(['layout', 'position', 'align'], 'center');
		$bg_color = RevLoader::esc_attr(trim($this->slider->get_param(['layout', 'bg', 'color'])));
		$max_width = $this->slider->get_param(['size', 'maxWidth'], '0');
		...
		$style .= 'padding:' . RevLoader::esc_attr($this->slider->get_param(['layout', 'bg', 'padding'], '0')) . ';';
		...
		if ($type != 'fullscreen') {
			...
			if ($position != 'center') {
				$ma_l = $this->slider->get_param(['layout', 'position', 'marginLeft'], '0');
				$ma_r = $this->slider->get_param(['layout', 'position', 'marginRight'], '0');
				$style .= ($ma_l !== '') ? 'margin-left:' . RevLoader::esc_attr($ma_l) . ';' : '';
				$style .= ($ma_r !== '') ? 'margin-right:' . RevLoader::esc_attr($ma_r) . ';' : '';
			}

			$ma_t = $this->slider->get_param(['layout', 'position', 'marginTop'], '0');
			$ma_b = $this->slider->get_param(['layout', 'position', 'marginBottom'], '0');
		...
		if ($this->slider->get_param(['layout', 'bg', 'useImage'], false) == true) {
			$bg_img_id = RevLoader::esc_attr($this->slider->get_param(['layout', 'bg', 'imageId']));
			$bg_img_type = RevLoader::esc_attr($this->slider->get_param(['layout', 'bg', 'imageSourceType'], 'full'));
			$bg_img = RevLoader::esc_attr($this->slider->get_param(['layout', 'bg', 'image']));
			...
			if ($bg_img !== false) {
				...
				$style .= 'background-repeat:' . RevLoader::esc_attr($this->slider->get_param(['layout', 'bg', 'repeat'], 'no-repeat')) . ';'; //$this->slider->get_param(array('def', 'background', 'repeat'), 'no-repeat')
				$style .= 'background-size:' . RevLoader::esc_attr($this->slider->get_param(['layout', 'bg', 'fit'], 'cover')) . ';'; //$this->slider->get_param(array('def', 'background', 'fit'), 'cover')
				$style .= 'background-position:' . RevLoader::esc_attr($this->slider->get_param(['layout', 'bg', 'position'], 'center center')) . ';'; //$this->slider->get_param(array('def', 'background', 'position'), 'center center')
			}

		}

		if (!in_array($type, ['responsitive', 'fixed', 'auto', 'fullwidth', 'fullscreen'], true)) {
			$style .= 'height:' . $this->slider->get_param(['size', 'height', 'd'], 900) . ';';
			$style .= 'width:' . $this->slider->get_param(['size', 'width', 'd'], 1240) . ';';
		}

		if (!in_array($max_width, ['0', 0, '0px', '0%'], true) && $type == 'auto') {
			$style .= 'max-width:' . $max_width . ';';
		}

		$fixedOnTop = [
			'v' => $this->slider->get_param(['layout', 'position', 'fixedOnTop'], false),
			'd' => false,
		];
	...
	public function close_slider_wrapper_div() {
		...
		$this->rs_plugin_wrap_closed = true;
	...
	public function add_unfloat_html() {
		...
		if ($this->slider->get_param(['layout', 'position', 'addClear'], false) === true) {
	...
	public function get_slider_div() {
		...
		$class .= ($this->slider->get_param(['size', 'overflow'], true) == true) ? ' rs-ov-hidden' : '';

		if (!in_array($this->slider->get_param('layouttype'), ['responsitive', 'fixed', 'auto', 'fullwidth', 'fullscreen'], true)) {
			$style .= 'height:' . $this->slider->get_param(['size', 'width', 'd'], 1240) . ';';
			$style .= 'width:' . $this->slider->get_param(['size', 'height', 'd'], 900) . ';';
		}

		$r = RS_T4 . '<rs-plugin id="' . $this->get_html_id() . '"';
		$this->rs_plugin_open = true;
		$r .= ($class !== '') ? ' class="' . $class . '"' : '';
		$r .= ' style="' . $style . '"';
	...
	public function get_slides() {
		...
		if ($layouttype == 'hero' || !empty($order)) {
			$this->set_only_published(false);
		}
		...
		if ($this->get_preview_mode() === true) {
			...
			$lang = RevSliderWpml::getCurrentLang();
		...
		if ($layouttype == 'hero' && empty($order) && empty($gallery_ids)) {
	...
	public function get_timer_bar() {
		...
		$enable_progressbar = $this->slider->get_param(['general', 'progressbar', 'set'], true);

		$timer_bar = $this->slider->get_param(['general', 'progressbar', 'position'], 'top');
		$progress_height = $this->slider->get_param(['general', 'progressbar', 'height'], '5');
		$progressbar_color = RSColorpicker::get($this->slider->get_param(['general', 'progressbar', 'color'], '#000000'));
	...
	public function add_static_slide_html() {
		...
		if ($this->get_do_static() && !empty($static_slide)) {
			...
			if (!empty($layers)) {
				$sof = $static_slide->get_param(['static', 'overflow'], '');
				$sof = (!empty($sof) && $sof == 'hidden') ? ' style="overflow:hidden;width:100%;height:100%;top:0px;left:0px;"' : '';
				$slp = $static_slide->get_param(['static', 'position'], 'front');
	...
	public function get_image_data() {
		...
		if ($bg_type != 'external') {
			...
			switch ($slide->get_param(['attributes', 'altOption'], $slide->get_param(['attributes', 'titleOption'], 'media_library'))) {
			case 'media_library':
			default:
				//track
			...
			case 'file_name':
				...
				break;
			case 'custom':
				$img['alt'] = RevLoader::esc_attr($slide->get_param(['attributes', 'alt'], ''));
				break;
			}

			switch ($slide->get_param(['attributes', 'titleOption'], 'media_library')) {
			case 'media_library':
			default:
				//track
			...
			case 'file_name':
				...
				break;
			case 'custom':
				$img['title'] = RevLoader::esc_attr($slide->get_param(['attributes', 'title'], ''));
				break;
			}
			...
			if ($img['id'] !== false) {
				...
				if ($data !== false && !empty($data)) {
					$size = $slide->get_param(['bg', 'imageSourceType'], 'full'); //$this->slider->get_param(array('def', 'background', 'imageSourceType'), 'full')
			...
			$img['src'] = $slide->get_param(['bg', 'externalSrc'], '');
			$img['alt'] = RevLoader::esc_attr($slide->get_param(['attributes', 'alt'], ''));
			$img['title'] = RevLoader::esc_attr($slide->get_param(['attributes', 'title'], ''));
			$img['width'] = $slide->get_param(['bg', 'width'], '1920');
			$img['height'] = $slide->get_param(['bg', 'height'], '1080');
		}
		...
		case 'solid':
			...
			if (isset($img['alt']) && trim($img['alt']) === '') {
				...
				$img['alt'] = (empty($img['alt'])) ? $this->l('Slide Background') : $img['alt'];
			}
	...
	public function get_image_background_values() {
		...
		$c = ($slide->get_param(['bg', 'type'], 'trans') == 'solid') ? RSColorpicker::get($slide->get_param(['bg', 'color'], '#ffffff')) : '';
		$c = ($c == '' && $slide->get_param(['bg', 'type'], 'trans') == 'solid') ? '#ffffff' : $c; //force white here as we need it in frontend

		$bg .= (!in_array($pos, ['', '50%', '50% 50%', 'center center', 'center'], true)) ? 'p:' . $pos . ';' : '';
	...
	public function get_html_parallax() {
		...
		if ($this->slider->get_param(['parallax', 'set'], false) == true) {
			$slide_level = $slide->get_param(['effects', 'parallax'], '-');
	...
	public function get_html_pan_zoom() {
		...
		if ($slide->get_param(['panzoom', 'set'], false) == true && ($bg_type == 'image' || $bg_type == 'external')) {
			$d = $slide->get_param(['panzoom', 'duration'], '10000');
			$e = $slide->get_param(['panzoom', 'ease'], 'none');
			$ss = $slide->get_param(['panzoom', 'fitStart'], '100');
			$se = $slide->get_param(['panzoom', 'fitEnd'], '100');
			$rs = $slide->get_param(['panzoom', 'rotateStart'], '0');
			$re = $slide->get_param(['panzoom', 'rotateEnd'], '0');
			$bs = $slide->get_param(['panzoom', 'blurStart'], '0');
			$be = $slide->get_param(['panzoom', 'blurEnd'], '0');
			$os = $slide->get_param(['panzoom', 'xStart'], '0') . '/' . $slide->get_param(['panzoom', 'yStart'], '0');
			$oe = $slide->get_param(['panzoom', 'xEnd'], '0') . '/' . $slide->get_param(['panzoom', 'yEnd'], '0');
	...
	public function get_background_position() {
		...
		$pos = $slide->get_param(['bg', 'position'], 'center center'); //$this->slider->get_param(array('def', 'background', 'position'),
		$type = $slide->get_param(['bg', 'type'], 'trans');
		...
		$pos = ($pos == 'percentage') ? intval($slide->get_param(['bg', 'positionX'], '0')) . '% ' . intval($slide->get_param(['bg', 'positionY'], '0')) . '%' : $pos; //$this->slider->get_param(array('def', 'background', 'positionX'), '0') $this->slider->get_param(array('def', 'background', 'positionY'), '0')
	...
	public function get_background_fit_and_repeat() {
		...
		if (!($slide->get_param(['panzoom', 'set'], false) == true && ($bg_type == 'image' || $bg_type == 'external'))) {
			//only set if kenburner is off and not a background video //$this->slider->get_param(array('def', 'panZoom', 'set'), false)
			...
			if ($bg_type == 'youtube' || $bg_type == 'html5' || $bg_type == 'vimeo' || $bg_type == 'streamvimeo' || $bg_type == 'streamyoutube' || $bg_type == 'streaminstagram' || $bg_type == 'streamtwitter') {
				...
				$bgFit = $slide->get_param(['bg', 'fit'], 'cover'); //$this->slider->get_param(array('def', 'background', 'fit'), 'cover')
				$return['f'] = ($bgFit == 'percentage') ? intval($slide->get_param(['bg', 'fitX'], '100')) . '% ' . intval($slide->get_param(['bg', 'fitY'], '100')) . '%' : $bgFit; //$this->slider->get_param(array('def', 'background', 'fitX'), '100') $this->slider->get_param(array('def', 'background', 'fitY'), '100')
				$return['r'] = $slide->get_param(['bg', 'repeat'], 'no-repeat'); //$this->slider->get_param(array('def', 'background', 'repeat'), 'no-repeat')
			}
	...
	public function add_background_video() {
		...
		if ($type == 'streamvimeoboth' || $type == 'streamyoutubeboth' || $type == 'streaminstagramboth' || $type == 'streamtwitterboth') {
			...
			if ($this->check_if_stream_video_exists()) {
				$this->add_html_background_video();
			}
	...
	public function add_zones() {
		...
		$this->container_mode = '';
		$zones = ['t' => 'top', 'm' => 'middle', 'b' => 'bottom'];
		$enabled_sizes = ['d' => true, 'n' => false, 't' => false, 'm' => false];

		foreach ($zones as $zs => $zone) {
			//go through all three zones
			...
			foreach ($layers as $layer) {
				...
				$layer_zone = $this->get_val($layer, ['group', 'puid'], 'top');
				...
				$zi = $this->get_val($layers, [$zone, 'position', 'zIndex'], $this->row_zindex);
	...
	public function add_groups() {
		...
		foreach ($layers as $layer) {
			...
			if ($this->get_val($layer, 'type', 'text') !== 'group') {
				continue;
			}
	...
	private function add_creative_layer() {
		...
		foreach ($layers as $layer) {
			...
			if ((string) $this->get_val($layer, ['group', 'puid'], '-1') !== '-1') {
				continue;
			}
	...
	public function add_group_layer($u_id) {
		...
		foreach ($layers as $layer) {
			$p_uid = $this->get_val($layer, ['group', 'puid']);
			$uid = $this->get_val($layer, ['group', 'uid']);

			if ((string) $u_id !== (string) $p_uid) {
				continue;
			}

			if ((string) $u_id === (string) $uid) {
				continue;
			}
	...
	public function add_rows($current_zone) {
		...
		foreach ($layers as $layer) {
			...
			if ($this->get_val($layer, ['group', 'puid'], 'top') !== $current_zone) {
				continue;
			}

			//wrong zones, so continue

			$order = $this->get_val($layer, ['group', 'groupOrder']);

			if ($order === '') {
				// || isset($row_layer[$order])
				$order = $go;
				$go++;
			}

			$zi = $this->get_val($layer, ['position', 'zIndex'], false); // set the z-index so that the wrapper gains the highest one
	...
	public function add_column($uid) {
		...
		foreach ($layers as $layer) {
			...
			if ($this->get_val($layer, 'type', 'text') !== 'column') {
				continue;
			}

			//we only want to handle columns here of the current row

			if ((string) $this->get_val($layer, ['group', 'puid'], -1) !== (string) $uid) {
				continue;
			}

			//has the wrong row ID

			$_go = $this->get_val($layer, ['group', 'groupOrder']);

			if ($_go === '') {
				// || isset($column_layers[$_go])
				$_go = $go;
				$go++;
			}
		...
		if (!empty($column_layers)) {
			...
			foreach ($column_layers as $layer) {
				...
				foreach ($layers as $nlayer) {
					...
					if ((string) $this->get_val($nlayer, ['group', 'puid'], -1) !== $cuid) {
						continue;
					}

					$_go = $this->get_val($nlayer, ['group', 'groupOrder']);

					if ($_go === '') {
						// || isset($group_layers[$_go])
						$_go = $go;
						$go++;
					}
	...
	public function add_layer($row_group_uid = false, $special_type = false) {
		...
		if (in_array($this->get_layer_unique_id(), ['top', 'middle', 'bottom'], true)) {
			return '';
		}
	...
	public function check_layer_video_continue() {
		...
		$video_type = trim($this->get_val($layer, ['media', 'mediaType']));
		$video_type = ($video_type === '') ? 'html5' : $video_type;

		if (!in_array($video_type, ['streamyoutube', 'streamyoutubeboth', 'youtube', 'streamvimeo', 'streamvimeoboth', 'vimeo', 'streaminstagram', 'streaminstagramboth', 'html5'], true)) {
			return true;
		}

		$vid = trim($this->get_val($layer, ['media', 'id']));

		switch ($video_type) {
		case 'streaminstagram':
		case 'streaminstagramboth':
		case 'html5':
			$ogv = trim($this->get_val($layer, ['media', 'ogvUrl'], ''));
			$webm = trim($this->get_val($layer, ['media', 'webmUrl'], ''));
			$mp4 = trim($this->remove_http($this->get_val($layer, ['media', 'mp4Url'], '')));
		...
		case 'streamyoutubeboth':
			$vid = (in_array($video_type, ['streamyoutube', 'streamyoutubeboth'], true)) ? $this->slide->get_param(['bg', 'youtube'], '') : $vid; //change $vid to the stream!
			$vid = ($this->get_val($layer, ['media', 'videoFromStream'], false) === true) ? $this->slide->get_param(['bg', 'youtube'], '') : $vid;
		...
		case 'streamvimeoboth':
			$vid = (in_array($video_type, ['streamvimeo', 'streamvimeoboth'], true)) ? $this->slide->get_param(['bg', 'vimeo'], '') : $vid;
			$vid = ($this->get_val($layer, ['media', 'videoFromStream'], false) === true) ? $this->slide->get_param(['bg', 'vimeo'], '') : $vid;
	...
	public function get_layer_tag($html_simple_link, $special_type = false) {
		...
		//if we are special type, only allow div to be the structure, as we will close with a div outside of this function
	...
	public function get_layer_class() {
		...
		$acs = $this->get_val($layer, ['attributes', 'classes'], '');
		...
		$idle_class = $this->get_val($layer, ['idle', 'style'], '');
		$internal_class = $this->get_val($layer, ['runtime', 'internalClass'], '');
		$selectable = $this->get_val($layer, ['idle', 'selectable'], 'default');
		...
		if ($selectable !== 'default') {
			...
			if ($this->_truefalse($selectable) == true) {
				$class[] = 'rs-selectable';
			}

		} else {

			if ($this->slider->get_param(['general', 'layerSelection'], false) == true) {
				$class[] = 'rs-selectable';
			}

		}

		if ($this->get_val($layer, ['hover', 'pointerEvents'], 'auto') == 'none') {
			$class[] = 'rs-noevents';
		}
		...
		if ($this->get_val($layer, ['idle', 'overflow']) === 'hidden') {
			$class[] = 'rs-ov-hidden';
		}
		...
		if ($type == 'video') {
			...
			switch (trim($this->get_val($layer, ['media', 'mediaType']))) {
			case 'streaminstagram':
			case 'streaminstagramboth':
			case 'html5':

				if ($this->get_val($layer, ['media', 'largeControls'], true) === false) {
					$class[] = 'rs-nolc';
				}
		...
		if ($this->slider->get_param(['parallax', 'set', false]) == true) {
			$level = $this->get_val($layer, ['effects', 'parallax'], '-');
			$level = ($this->slider->get_param(['parallax', 'setDDD'], false) == true && $level == '-' && $this->get_val($layer, ['effects', 'attachToBg'], '') === true) ? 'tobggroup' : $level;
		...
		if ($type == 'audio') {
			...
			$visible = $this->get_val($layer, ['media', 'controls'], false);

			if ($visible === false) {
				$class[] = 'rs-layer-hidden';
			}

		}

		if ($this->get_val($layer, ['visibility', 'onlyOnSlideHover'], false) === true) {
			$class[] = 'rs-on-sh';
		}

		if ($this->slider->get_param('type', 'standard') === 'carousel') {

			if ($this->get_val($layer, ['visibility', 'alwaysOnCarousel'], false) === true) {
				$class[] = 'rs-on-car';
			}
		...
		if (strpos($text, '<iframe') !== false || strpos($text_toggle, '<iframe') !== false) {
			$add_intrinsic = true;
		}

		if ($tag == 'iframe' && array_search('rs-layer', $class) !== false) {
			$add_intrinsic = true;
		}
	...
	public function create_style_hover() {
		...
		if ($this->get_val($layer, ['hover', 'usehover'], false) === false || $this->get_val($layer, ['hover', 'usehover'], false) === 'false') {
			return false;
		}
		...
		if (!empty($custom_css)) {
			...
			$_nec = $this->get_val($custom_css, 'nec', []);
	...
	public function get_html_inline_style() {
		...
		$layer = $this->get_layer();
		$type = $this->get_val($layer, 'type', 'text');
		$img = trim($this->get_val($layer, ['idle', 'backgroundImage'], ''));
		$img_id = $this->get_val($layer, ['idle', 'backgroundImageId']);
		$img_t = $this->get_val($layer, ['behavior', 'imageSourceType'], 'full');
		$zi = $this->get_val($layer, ['position', 'zIndex'], false);
		$zi = ($zi === false) ? $this->zIndex : $zi;
		...
		if ($this->get_val($layer, ['idle', 'bgFromStream']) === true) {
		...
		if ($img !== '' && !in_array($type, ['group', 'shape', 'row'], true)) {
			...
			$style['background'] .= ' ' . $this->get_val($layer, ['idle', 'backgroundRepeat'], 'no-repeat');
			$style['background'] .= ' ' . $this->get_val($layer, ['idle', 'backgroundPosition'], 'center center');
			$bgs = $this->get_val($layer, ['idle', 'backgroundSize'], 'cover');
			$bgs = ($bgs === 'percentage') ? $this->get_val($layer, ['idle', 'backgroundSizePerc'], '100') . '%' : $bgs;
			$bgs = ($bgs === 'pixel') ? $this->get_val($layer, ['idle', 'backgroundSizePix'], '100') . 'px' : $bgs;
			$style['background-size'] = $bgs;
		}

		$bgcolor = $this->get_val($layer, ['idle', 'backgroundColor'], 'transparent');
		...
		if (!in_array($type, ['image', 'video', 'row', 'column', 'group', 'shape', 'audio'], true)) {
			$style['font-family'] = str_replace('"', "'", $this->get_val($layer, ['idle', 'fontFamily'], 'Roboto'));
		}

		$text_transform = $this->get_val($layer, ['idle', 'textTransform'], 'none');

		if ($text_transform !== 'none') {
			$style['text-transform'] = $text_transform;
		}

		$fs = $this->get_val($layer, ['idle', 'fontStyle'], 'off');

		if ($fs == 'on' || $fs == 'italic') {
			$style['font-style'] = 'italic';
		}

		$mc = $this->get_val($layer, ['idle', 'cursor'], 'auto');
		...
		if ($this->container_mode === 'column' && $type !== 'row' && $this->get_val($layer, ['idle', 'display'], 'block') !== 'block') {
			$style['display'] = $this->get_val($layer, ['idle', 'display']);
		}
	...
	public function get_material_icons_css() {
		...
		if ($rs_material_icons_css === false) {
			return '';
		}

		if ($rs_material_icons_css_parsed === true) {
			return '';
		}
...
rs-plugin .material-icons {
  ...
  -moz-osx-font-smoothing: grayscale;
	...
	public function add_custom_navigation_css() {
		...
		$enable_arrows = $this->slider->get_param(['nav', 'arrows', 'set'], false);
		$enable_bullets = $this->slider->get_param(['nav', 'bullets', 'set'], false);
		$enable_tabs = $this->slider->get_param(['nav', 'tabs', 'set'], false);
		$enable_thumbnails = $this->slider->get_param(['nav', 'thumbs', 'set'], false);
			...
			foreach ($slides as $slide) {
				...
				if (!empty($all_navs)) {
					...
					foreach ($all_navs as $cur_nav) {
						...
						if ($enable_arrows == true && $cur_nav['id'] == $this->slider->get_param(['nav', 'arrows', 'style'], 'new-bullet-bar')) {
						...
						if ($enable_bullets == true && $cur_nav['id'] == $this->slider->get_param(['nav', 'bullets', 'style'], 'round')) {
						...
						if ($enable_tabs == true && $cur_nav['id'] == $this->slider->get_param(['nav', 'tabs', 'style'], 'round')) {
						...
						if ($enable_thumbnails == true && $cur_nav['id'] == $this->slider->get_param(['nav', 'thumbs', 'style'], 'new-bullet-bar')) {
	...
	public function get_html_transform() {
		...
		$rx = intval($this->get_val($layer, ['idle', 'rotationX'], 0));
		$ry = intval($this->get_val($layer, ['idle', 'rotationY'], 0));
		$rz = intval($this->get_val($layer, ['idle', 'rotationZ'], 0));
		$op = $this->get_val($layer, ['idle', 'opacity'], 1);
	...
	public function get_html_responsive() {
		...
		if (in_array($this->get_val($layer, 'type', 'text'), ['row', 'column'], true)) {
			return $html;
		}

		if ($this->get_val($layer, ['behavior', 'autoResponsive'], true) === true) {
			$html .= ($this->get_val($layer, ['behavior', 'responsiveChilds'], true)) ? 'data-rsp_ch="on"' : '';
		}
	...
	public function get_html_layer_ids($raw = false) {
		...
		$layer = $this->get_layer();
		$ids = $this->get_val($layer, ['attributes', 'id']);
	...
	public function get_html_title() {
		...
		$layer = $this->get_layer();
		$title = $this->get_val($layer, ['attributes', 'title']);
	...
	public function get_html_tab_index() {
		...
		$layer = $this->get_layer();
		$tabindex = $this->get_val($layer, ['attributes', 'tabIndex']);

		return (!in_array($tabindex, ['', '0', 0], true)) ? 'tabindex="' . $tabindex . '"' : '';
	}
	...
	public function get_html_rel() {
		...
		$layer = $this->get_layer();
		$rel = $this->get_val($layer, ['attributes', 'rel']);
	...
	public function get_html_layer_position() {
		...
		$f = ['top', 'right', 'bottom', 'left', 'center', 'middle'];
		$t = ['t', 'r', 'b', 'l', 'c', 'm'];
		...
		if ($this->is_full_width_video() == true) {
			...
			$layer = $this->get_layer();
			$alignHor = $this->get_val($layer, ['position', 'horizontal']);
			$alignVert = $this->get_val($layer, ['position', 'vertical']);
			$left = $this->get_val($layer, ['position', 'x']);
			$top = $this->get_val($layer, ['position', 'y']);

			if ($this->adv_resp_sizes == true) {
				//remove from myTop and myLeft 0 and 0px
				$myHor = $this->normalize_device_settings($alignHor, $this->enabled_sizes, 'html-array', ['l']);
				$myLeft = $this->normalize_device_settings($left, $this->enabled_sizes, 'html-array', ['0', '0px']);
				$myVer = $this->normalize_device_settings($alignVert, $this->enabled_sizes, 'html-array', ['t']);
				$myTop = $this->normalize_device_settings($top, $this->enabled_sizes, 'html-array', ['0', '0px']);
				...
				$xy .= (!in_array($myHor, ['', 0, '0', '0px'], true)) ? 'x:' . $myHor . ';' : '';
				$xy .= (!in_array($myLeft, ['', 0, '0', '0px'], true)) ? 'xo:' . $myLeft . ';' : '';
				$xy .= (!in_array($myVer, ['', 0, '0', '0px'], true)) ? 'y:' . $myVer . ';' : '';
				$xy .= (!in_array($myTop, ['', 0, '0', '0px'], true)) ? 'yo:' . $myTop . ';' : '';
			} else {
				...
				case 'left':
					...
					break;
				case 'center':
					$left = (in_array($left, ['', 0, '0', '0px'], true)) ? '' : $left;
					$xy .= 'x:c;';
					$xy .= ($left !== '') ? 'xo:' . $left . ';' : '';
					break;
				case 'right':
				...
				case 'top':
					...
					break;
				case 'middle':
					$top = (in_array($top, ['', 0, '0', '0px'], true)) ? '' : $top;
					$xy .= 'y:c;';
					$xy .= ($top !== '') ? 'yo:' . $top . ';' : '';
					break;
				case 'bottom':
	...
	public function get_html_text_data() {
		...
		$de = [
			's'  => [20, '20', '20px'],
			'l'  => (in_array($type, ['text', 'button'])) ? [25, '25', '25px'] : [0, '0', '0px'],
			'ls' => [0, '0', '0px'],
			'fw' => [400, '400'],
			'w'  => ['nowrap'],
			'a'  => ['left'],
		...
		if ($this->adv_resp_sizes == true) {
			$ws = $this->normalize_device_settings($this->get_val($layer, ['idle', 'whiteSpace']), $this->enabled_sizes, 'html-array', $de['w']);
		} else {
			$ws = $this->get_biggest_device_setting($this->get_val($layer, ['idle', 'whiteSpace'], $de['w'][0]), $this->enabled_sizes);
		}
		...
		if ($this->adv_resp_sizes == true) {
			$data['s'] = $this->normalize_device_settings($this->get_val($layer, ['idle', 'fontSize']), $this->enabled_sizes, 'html-array', $de['s']);
			$data['f'] = $this->normalize_device_settings($this->get_val($layer, ['idle', 'float']), $this->enabled_sizes, 'html-array', $de['f']);
			$data['c'] = $this->normalize_device_settings($this->get_val($layer, ['idle', 'clear']), $this->enabled_sizes, 'html-array', $de['c']);
			$data['l'] = $this->normalize_device_settings($this->get_val($layer, ['idle', 'lineHeight']), $this->enabled_sizes, 'html-array', $de['l']);
			$data['ls'] = $this->normalize_device_settings($this->get_val($layer, ['idle', 'letterSpacing']), $this->enabled_sizes, 'html-array', $de['ls']);
			$data['fw'] = $this->normalize_device_settings($this->get_val($layer, ['idle', 'fontWeight']), $this->enabled_sizes, 'html-array', $de['fw']);
			$data['a'] = $this->normalize_device_settings($this->get_val($layer, ['idle', 'textAlign']), $this->enabled_sizes, 'html-array', $de['a']);
		} else {
			$data['s'] = $this->get_biggest_device_setting($this->get_val($layer, ['idle', 'fontSize']), $this->enabled_sizes);
			$data['f'] = $this->get_biggest_device_setting($this->get_val($layer, ['idle', 'float']), $this->enabled_sizes);
			$data['c'] = $this->get_biggest_device_setting($this->get_val($layer, ['idle', 'clear']), $this->enabled_sizes);
			$data['l'] = $this->get_biggest_device_setting($this->get_val($layer, ['idle', 'lineHeight']), $this->enabled_sizes);
			$data['ls'] = $this->get_biggest_device_setting($this->get_val($layer, ['idle', 'letterSpacing']), $this->enabled_sizes);
			$data['fw'] = $this->get_biggest_device_setting($this->get_val($layer, ['idle', 'fontWeight']), $this->enabled_sizes);
			$data['a'] = $this->get_biggest_device_setting($this->get_val($layer, ['idle', 'textAlign']), $this->enabled_sizes);
		}
		...
		$textDecoration = $this->get_val($layer, ['idle', 'textDecoration']);
	...
	public function get_html_color_data() {
		...
		if (in_array($type, ['text', 'svg', 'button'], true)) {

			if ($this->adv_resp_sizes == true) {
				$color = $this->normalize_device_settings($this->get_val($layer, ['idle', 'color'], '#ffffff'), $this->enabled_sizes, 'html-array', ['#ffffff'], ['' => '#ffffff'], '||');
			} else {
				$color = $this->get_biggest_device_setting($this->get_val($layer, ['idle', 'color'], '#ffffff'), $this->enabled_sizes);
	...
	public function get_html_box_shadow_data() {
		...
		if ($this->get_val($layer, ['idle', 'boxShadow', 'inuse'], false) === true) {
			$color = str_replace(' ', '', $this->get_val($layer, ['idle', 'boxShadow', 'color'], 'rgba(0,0,0,0)'));

			if ($this->get_val($layer, ['idle', 'boxShadow', 'container'], 'content') !== 'content') {
				$text .= 'e:w';
			}

			//w for wrapper

			if (!in_array($color, ['rgba(0,0,0,0)', '#000000'])) {
				$text .= 'c:' . $color . ';';
			}
			...
			if ($this->adv_resp_sizes == true) {
				$data['h'] = $this->normalize_device_settings($this->get_val($layer, ['idle', 'boxShadow', 'hoffset']), $this->enabled_sizes, 'html-array', [0]);
				$data['v'] = $this->normalize_device_settings($this->get_val($layer, ['idle', 'boxShadow', 'voffset']), $this->enabled_sizes, 'html-array', [0]);
				$data['b'] = $this->normalize_device_settings($this->get_val($layer, ['idle', 'boxShadow', 'blur']), $this->enabled_sizes, 'html-array', [0]);
				$data['s'] = $this->normalize_device_settings($this->get_val($layer, ['idle', 'boxShadow', 'spread']), $this->enabled_sizes, 'html-array', [0]);
			} else {
				$data['h'] = $this->get_biggest_device_setting($this->get_val($layer, ['idle', 'boxShadow', 'hoffset']), $this->enabled_sizes, 0);
				$data['v'] = $this->get_biggest_device_setting($this->get_val($layer, ['idle', 'boxShadow', 'voffset']), $this->enabled_sizes, 0);
				$data['b'] = $this->get_biggest_device_setting($this->get_val($layer, ['idle', 'boxShadow', 'blur']), $this->enabled_sizes, 0);
				$data['s'] = $this->get_biggest_device_setting($this->get_val($layer, ['idle', 'boxShadow', 'spread']), $this->enabled_sizes, 0);
			}

			if (!empty($data)) {

				foreach ($data as $k => $v) {
					$text .= (!in_array(trim($v), [0, '0', '0px', ''], true)) ? $k . ':' . $v . ';' : '';
				}
	...
	public function get_html_text_shadow_data() {
		...
		if ($this->get_val($layer, 'type', 'text') === 'text') {
			...
			if ($this->get_val($layer, ['idle', 'textShadow', 'inuse'], false) === true) {
				$color = str_replace(' ', '', $this->get_val($layer, ['idle', 'textShadow', 'color'], 'rgba(0,0,0,0.25)'));

				if ($this->get_val($layer, ['idle', 'textShadow', 'container'], 'content') !== 'content') {
					$text .= 'e:w';
				}

				//w for wrapper

				if (!in_array($color, ['rgba(0,0,0,0.25)'])) {
					$text .= 'c:' . $color . ';';
				}
				...
				if ($this->adv_resp_sizes == true) {
					$data['h'] = $this->normalize_device_settings($this->get_val($layer, ['idle', 'textShadow', 'hoffset']), $this->enabled_sizes, 'html-array', [0]);
					$data['v'] = $this->normalize_device_settings($this->get_val($layer, ['idle', 'textShadow', 'voffset']), $this->enabled_sizes, 'html-array', [0]);
					$data['b'] = $this->normalize_device_settings($this->get_val($layer, ['idle', 'textShadow', 'blur']), $this->enabled_sizes, 'html-array', [0]);
				} else {
					$data['h'] = $this->get_biggest_device_setting($this->get_val($layer, ['idle', 'textShadow', 'hoffset']), $this->enabled_sizes, 0);
					$data['v'] = $this->get_biggest_device_setting($this->get_val($layer, ['idle', 'textShadow', 'voffset']), $this->enabled_sizes, 0);
					$data['b'] = $this->get_biggest_device_setting($this->get_val($layer, ['idle', 'textShadow', 'blur']), $this->enabled_sizes, 0);
				}
	...
	public function get_html_dim_data() {
		...
		if ($type !== 'column') {
			...
			if ($this->adv_resp_sizes == true) {
				$data['w'] = $this->normalize_device_settings($this->get_val($layer, ['size', 'width']), $this->enabled_sizes, 'html-array', ['auto']);
				$data['h'] = $this->normalize_device_settings($this->get_val($layer, ['size', 'height']), $this->enabled_sizes, 'html-array', ['auto']);

				$data['maxw'] = $this->normalize_device_settings($this->get_val($layer, ['size', 'maxWidth'], 'auto'), $this->enabled_sizes, 'html-array', ['none'], ['none' => 'n']);
				$data['maxh'] = $this->normalize_device_settings($this->get_val($layer, ['size', 'maxHeight'], 'auto'), $this->enabled_sizes, 'html-array', ['none'], ['none' => 'n']);
				$data['minw'] = $this->normalize_device_settings($this->get_val($layer, ['size', 'minWidth'], 'auto'), $this->enabled_sizes, 'html-array', ['none'], ['none' => 'n']);
				$data['minh'] = $this->normalize_device_settings($this->get_val($layer, ['size', 'minHeight'], 'auto'), $this->enabled_sizes, 'html-array', ['none'], ['none' => 'n']);
			} else {

				$data['w'] = $this->get_biggest_device_setting($this->get_val($layer, ['size', 'width']), $this->enabled_sizes);
				$data['h'] = $this->get_biggest_device_setting($this->get_val($layer, ['size', 'height']), $this->enabled_sizes);

				$data['maxw'] = $this->get_biggest_device_setting($this->get_val($layer, ['size', 'maxWidth'], 'auto'), $this->enabled_sizes);
				$data['maxh'] = $this->get_biggest_device_setting($this->get_val($layer, ['size', 'maxHeight'], 'auto'), $this->enabled_sizes);
				$data['minw'] = $this->get_biggest_device_setting($this->get_val($layer, ['size', 'minWidth'], 'auto'), $this->enabled_sizes);
				$data['minh'] = $this->get_biggest_device_setting($this->get_val($layer, ['size', 'minHeight'], 'auto'), $this->enabled_sizes);
			}
			...
			if ($type === 'image') {
				$scaleX = $this->get_val($layer, ['size', 'width']);
				$scaleY = $this->get_val($layer, ['size', 'height']);
				$cover_mode = $this->get_val($layer, ['size', 'covermode'], []);
				$cover_mode = (is_string($cover_mode)) ? ['d' => $cover_mode, 'n' => $cover_mode, 't' => $cover_mode, 'm' => $cover_mode] : (array) $cover_mode;
				...
				if ($this->adv_resp_sizes == true) {
					...
					foreach ($cover_mode as $cvmk => $cvmv) {
						...
						if ($cvmv !== 'custom' && $cvmv !== 'fullheight') {
							$this->set_val($scaleX, [$cvmk, 'v'], '100%');
						}

						if ($cvmv !== 'custom' && $cvmv !== 'fullwidth') {
							$this->set_val($scaleY, [$cvmk, 'v'], '100%');
						}

					}

					$myScaleX = $this->normalize_device_settings($scaleX, $this->enabled_sizes, 'html-array', ['auto'], ['NaNpx' => '', 'auto' => '']);
					$myScaleY = $this->normalize_device_settings($scaleY, $this->enabled_sizes, 'html-array', ['auto'], ['NaNpx' => '', 'auto' => '']);
			...
			if (!empty($data)) {
				...
				foreach ($data as $k => $v) {
					$dim .= (!in_array(trim($v), [-1, '-1', '', 'auto'], true)) ? $k . ':' . $v . ';' : '';
				}
	...
	public function get_html_column_break() {
		...
		if ($this->get_val($layer, 'type', 'text') === 'row') {
			$break = $this->get_val($layer, ['group', 'columnbreakat'], 'tablet');
			...
			//as default, dont write it, so set it back to empty (was 2)
	...
	public function get_html_layer_action(&$html_simple_link) {
		...
		$html = "data-actions='";
		$events = [];
		$all_actions = $this->get_val($layer, 'actions', []);
		$actions = $this->get_val($all_actions, 'action', []);
			...
			foreach ($actions as $num => $action) {
				...
				case 'open_modal':
					...
					if (!empty($_modal)) {
						...
						if ($this->modal_sliders[$_modal]->get_param(['modal', 'cover'], true) === true) {
							$_event['bg'] = $this->modal_sliders[$_modal]->get_param(['modal', 'coverColor'], 'rgba(0,0,0,0.5)');
						}

						if ($this->modal_sliders[$_modal]->get_param(['layout', 'spinner', 'type'], '0') !== 'off') {
							$_event['spin'] = $this->modal_sliders[$_modal]->get_param(['layout', 'spinner', 'type'], '0');
							$_event['spinc'] = $this->modal_sliders[$_modal]->get_param(['layout', 'spinner', 'color'], '#FFFFFF');
						}
	...
	public function get_html_class($class, $layer_tag) {
		...
		if (!in_array($layer_tag, ['rs-row', 'rs-column', 'rs-layer', 'rs-group', 'rs-bgvideo'], true)) {
			$c[] = 'rs-layer';
		}
		...
		if ($this->slider->get_param(['parallax', 'set'], false) === true) {
			...
			if ($this->get_val($layer, ['effects', 'parallax'], '-') !== '-') {

				if ($this->get_val($layer, ['effects', 'pxmask'], false) === true) {
					$c[] = 'rs-pxmask';
				}
	...
	public function get_html_svg() {
		...
		$svg_html = '';
		$svg = [];
		$svg_source = $this->get_val($layer, ['svg', 'source']);
		...
		if (!empty($svg_source)) {
			...
			if ($this->get_val($layer, ['hover', 'usehover'], false) === true || $this->get_val($layer, ['hover', 'usehover'], false) === 'true' || $this->get_val($layer, ['hover', 'usehover'], false) === 'desktop') {
				$push['svgh'] = 'hover';
			}
			...
			foreach ($push as $tag => $path) {
				...
				$c = $this->get_val($layer, [$path, 'svg', 'color'], '#ffffff');
				$sc = $this->get_val($layer, [$path, 'svg', 'strokeColor'], 'transparent');
				$sw = $this->get_val($layer, [$path, 'svg', 'strokeWidth'], 0);
				$sa = $this->get_val($layer, [$path, 'svg', 'strokeDashArray'], '');
				$so = $this->get_val($layer, [$path, 'svg', 'strokeDashOffset'], '');
				...
				*/
				...
				if ($path === 'idle') {
					...
					if ($this->adv_resp_sizes == true) {
						$c = $this->normalize_device_settings($c, $this->enabled_sizes, 'html-array', ['#ffffff'], [], '||');
					} else {
				...
				if (!in_array(strtolower($c), ['#fff', '#ffffff']) && $c !== '') {
					$svg[$tag]['c'] = $c;
				}
	...
	public function get_html_base_align() {
		...
		$layer = $this->get_layer();
		$base_align = $this->get_val($layer, ['behavior', 'baseAlign'], 'grid');
	...
	public function get_html_responsive_offset() {
		...
		return ($this->get_val($layer, ['behavior', 'responsiveOffset'], true) === false) ? 'data-rsp_o="off"' : '';
	}
	...
	public function get_html_wrapper_ids() {
		...
		$layer = $this->get_layer();
		$wrapper_ids = $this->get_val($layer, ['attributes', 'wrapperId']);
	...
	public function get_html_wrapper_classes() {
		...
		$layer = $this->get_layer();
		$class = $this->get_val($layer, ['attributes', 'wrapperClasses']);
	...
	public function get_html_responsive_data() {
		...
		$layer = $this->get_layer();
		$default = (in_array($this->get_val($layer, 'type', 'text'), ['row', 'column'], true)) ? false : true;

		return ($this->get_val($layer, ['behavior', 'autoResponsive'], $default) === false) ? 'data-rsp_bd="off"' : '';
	}
	...
	public function get_html_scrollbased_data() {
		...
		$html = 'data-sba="';
		$sd = $sli->get_param(['scrolltimeline', 'set'], false);
		$se = $sli->get_param(['scrolleffects', 'set'], false);
		$s = $this->get_val($layer, ['timeline', 'scrollBased'], 'default');
		$so = $this->get_val($layer, ['timeline', 'scrollBasedOffset'], 0);
		$e = $this->get_val($layer, ['effects', 'effect'], 'default');
		...
		if (!in_array($so, ['0', 0, '0px'], true) && $sd === true) {
			$html .= 'so:' . $so;
		}
	...
	public function get_html_static_layer() {
		...
		if ($this->slider->get_param('type') !== 'hero') {
			$s = intval($this->get_val($layer, ['timeline', 'static', 'start'], 1));
			$e = $this->get_val($layer, ['timeline', 'static', 'end'], 'last');
		} else {
			$s = '-1';
			$e = '-1';
		}
	...
	public function get_html_trigger() {
		...
		$trigger_memory = ($has_trigger) ? $this->get_val($layer, ['actions', 'triggerMemory'], 'reset') : 'keep';
	...
	public function get_frames() {
		...
		$_base = [
			'grayscale'            => ['n' => 'gra', 'd' => ['frame_0' => 0, 'frame_1' => 0, 'default' => 'inherit'], 'depth' => ['filter', 'grayscale']], //0
			'brightness'           => ['n' => 'bri', 'd' => ['frame_0' => 100, 'frame_1' => 100, 'default' => 'inherit'], 'depth' => ['filter', 'brightness']], //100
			'blur'                 => ['n' => 'blu', 'd' => ['frame_0' => 0, 'frame_1' => 0, 'default' => 'inherit'], 'depth' => ['filter', 'blur']], //100
			'color'                => ['n' => 'c', 'd' => 'inherit', 'depth' => ['color', 'color']], //'#ffffff'
			'backgroundColor'      => ['n' => 'bgc', 'd' => 'inherit', 'depth' => ['bgcolor', 'backgroundColor']], //'transparent'

			//transform
			'x'                    => ['n' => 'x', 'd' => ['frame_0' => [0, '0', '0px', ''], 'frame_1' => [0, '0', '0px', ''], 'default' => 'inherit'], 'depth' => ['transform', 'x']], //0
			'y'                    => ['n' => 'y', 'd' => ['frame_0' => [0, '0', '0px', ''], 'frame_1' => [0, '0', '0px', ''], 'default' => 'inherit'], 'depth' => ['transform', 'y']], //0
			'z'                    => ['n' => 'z', 'd' => ['frame_0' => [0, '0', '0px', ''], 'frame_1' => [0, '0', '0px', ''], 'default' => 'inherit'], 'depth' => ['transform', 'z']], //0
			'scaleX'               => ['n' => 'sX', 'd' => ['frame_0' => 1, 'frame_1' => 1, 'default' => 'inherit'], 'depth' => ['transform', 'scaleX']], //0
			'scaleY'               => ['n' => 'sY', 'd' => ['frame_0' => 1, 'frame_1' => 1, 'default' => 'inherit'], 'depth' => ['transform', 'scaleY']], //0
			'opacity'              => ['n' => 'o', 'd' => ['frame_0' => 0, 'frame_1' => 1, 'default' => 'inherit'], 'depth' => ['transform', 'opacity']], //1
			'rotationX'            => ['n' => 'rX', 'd' => ['frame_0' => [0, '0', '0deg', ''], 'frame_1' => [0, '0', '0deg', ''], 'default' => 'inherit'], 'depth' => ['transform', 'rotationX']], //0
			'rotationY'            => ['n' => 'rY', 'd' => ['frame_0' => [0, '0', '0deg', ''], 'frame_1' => [0, '0', '0deg', ''], 'default' => 'inherit'], 'depth' => ['transform', 'rotationY']], //0
			'rotationZ'            => ['n' => 'rZ', 'd' => ['frame_0' => [0, '0', '0deg', ''], 'frame_1' => [0, '0', '0deg', ''], 'default' => 'inherit'], 'depth' => ['transform', 'rotationZ']], //0
			'skewX'                => ['n' => 'skX', 'd' => ['frame_0' => [0, '0', '0px', ''], 'frame_1' => [0, '0', '0px', ''], 'default' => 'inherit'], 'depth' => ['transform', 'skewX']], //0
			'skewY'                => ['n' => 'skY', 'd' => ['frame_0' => [0, '0', '0px', ''], 'frame_1' => [0, '0', '0px', ''], 'default' => 'inherit'], 'depth' => ['transform', 'skewY']], //0
			'originX'              => ['n' => 'oX', 'd' => ['frame_0' => '50%', 'frame_1' => '50%', 'default' => 'inherit'], 'depth' => ['transform', 'originX']], //'50%'
			'originY'              => ['n' => 'oY', 'd' => ['frame_0' => '50%', 'frame_1' => '50%', 'default' => 'inherit'], 'depth' => ['transform', 'originY']], //'50%'
			'originZ'              => ['n' => 'oZ', 'd' => ['frame_0' => 0, 'frame_1' => 0, 'default' => 'inherit'], 'depth' => ['transform', 'originZ']], //'0'
			'transformPerspective' => ['n' => 'tp', 'd' => true, 'depth' => ['transform', 'transformPerspective']], //'600px'
			'clip'                 => ['n' => 'cp', 'd' => ['frame_0' => 100, 'frame_1' => 100, 'default' => 'inherit'], 'depth' => ['transform', 'clip']], //100
			'clipB'                => ['n' => 'cpb', 'd' => ['frame_0' => 100, 'frame_1' => 100, 'default' => 'inherit'], 'depth' => ['transform', 'clipB']], //100

			//timeline
			'ease'                 => ['n' => 'e', 'd' => ['frame_0' => false, 'default' => 'power3.inOut'], 'depth' => ['timeline', 'ease']], //'power3.inOut'
			'start'                => ['n' => 'st', 'd' => ['frame_0' => false, 'frame_1' => 10, 'default' => true], 'depth' => ['timeline', 'start']], //0
			'speed'                => ['n' => 'sp', 'd' => ['frame_0' => false, 'default' => 300], 'depth' => ['timeline', 'speed']], //300
			'startRelative'        => ['n' => 'sR', 'd' => 0, 'depth' => ['timeline', 'startRelative']], //0
		];

		$_split = [
			'ease'       => ['n' => 'e', 'd' => ['frame_0' => false, 'default' => 'inherit']],
			'direction'  => ['n' => 'dir', 'd' => ['frame_0' => false, 'default' => 'forward']], //'forward'
			'delay'      => ['n' => 'd', 'd' => ['frame_0' => false, 'default' => 5]], //5
			'x'          => ['n' => 'x', 'd' => ['frame_0' => [0, '0', '0px', ''], 'frame_1' => [0, '0', '0px', ''], 'default' => 'inherit']],
			'y'          => ['n' => 'y', 'd' => ['frame_0' => [0, '0', '0px', ''], 'frame_1' => [0, '0', '0px', ''], 'default' => 'inherit']],
			'z'          => ['n' => 'z', 'd' => ['frame_0' => [0, '0', '0px', ''], 'frame_1' => [0, '0', '0px', ''], 'default' => 'inherit']],
			'scaleX'     => ['n' => 'sX', 'd' => ['frame_0' => 1, 'frame_1' => 1, 'default' => 'inherit']],
			'scaleY'     => ['n' => 'sY', 'd' => ['frame_0' => 1, 'frame_1' => 1, 'default' => 'inherit']],
			'opacity'    => ['n' => 'o', 'd' => 'inherit'],
			'rotationX'  => ['n' => 'rX', 'd' => ['frame_0' => [0, '0', '0px', ''], 'frame_1' => [0, '0', '0px', ''], 'default' => 'inherit']],
			'rotationY'  => ['n' => 'rY', 'd' => ['frame_0' => [0, '0', '0px', ''], 'frame_1' => [0, '0', '0px', ''], 'default' => 'inherit']],
			'rotationZ'  => ['n' => 'rZ', 'd' => ['frame_0' => [0, '0', '0px', ''], 'frame_1' => [0, '0', '0px', ''], 'default' => 'inherit']],
			'skewX'      => ['n' => 'skX', 'd' => ['frame_0' => [0, '0', '0px', ''], 'frame_1' => [0, '0', '0px', ''], 'default' => 'inherit']],
			'skewY'      => ['n' => 'skY', 'd' => ['frame_0' => [0, '0', '0px', ''], 'frame_1' => [0, '0', '0px', ''], 'default' => 'inherit']],
			'originX'    => ['n' => 'oX', 'd' => ['frame_0' => '50%', 'frame_1' => '50%', 'default' => 'inherit']], //'50%'
			'originY'    => ['n' => 'oY', 'd' => ['frame_0' => '50%', 'frame_1' => '50%', 'default' => 'inherit']], //'50%'
			'originZ'    => ['n' => 'oZ', 'd' => ['frame_0' => 0, 'frame_1' => 0, 'default' => 'inherit']],
			'fuse'       => ['n' => 'fuse', 'd' => ['default' => false]],
			'grayscale'  => ['n' => 'gra', 'd' => ['frame_0' => 0, 'frame_1' => 0, 'default' => 'inherit']], //0
			'brightness' => ['n' => 'bri', 'd' => ['frame_0' => 100, 'frame_1' => 100, 'default' => 'inherit']], //100
			'blur'       => ['n' => 'blu', 'd' => ['frame_0' => 0, 'frame_1' => 0, 'default' => 'inherit']], //100
		];

		$_mask = [
			'x' => ['n' => 'x', 'd' => ['frame_0' => [0, '0', '0px', ''], 'frame_1' => [0, '0', '0px', ''], 'default' => 'inherit'], 'depth' => ['mask', 'x']],
			'y' => ['n' => 'y', 'd' => ['frame_0' => [0, '0', '0px', ''], 'frame_1' => [0, '0', '0px', ''], 'default' => 'inherit'], 'depth' => ['mask', 'y']],
		];

		$_sfx = [
			'effect' => ['n' => 'se', 'd' => '', 'depth' => ['sfx', 'effect']],
			'color'  => ['n' => 'fxc', 'd' => '#ffffff', 'depth' => ['sfx', 'color']],
		];

		$_reverse = [
			'x'              => ['n' => 'x', 'd' => false, 'depth' => ['reverseDirection', 'x']],
			'y'              => ['n' => 'y', 'd' => false, 'depth' => ['reverseDirection', 'y']],
			'rotationX'      => ['n' => 'rX', 'd' => false, 'depth' => ['reverseDirection', 'rotationX']],
			'rotationY'      => ['n' => 'rY', 'd' => false, 'depth' => ['reverseDirection', 'rotationY']],
			'rotationZ'      => ['n' => 'rZ', 'd' => false, 'depth' => ['reverseDirection', 'rotationZ']],
			'skewX'          => ['n' => 'sX', 'd' => false, 'depth' => ['reverseDirection', 'skewX']],
			'skewY'          => ['n' => 'sY', 'd' => false, 'depth' => ['reverseDirection', 'skewY']],
			'maskX'          => ['n' => 'mX', 'd' => false, 'depth' => ['reverseDirection', 'maskX']],
			'maskY'          => ['n' => 'mY', 'd' => false, 'depth' => ['reverseDirection', 'maskY']],
			'charsX'         => ['n' => 'cX', 'd' => false, 'depth' => ['reverseDirection', 'charsX']],
			'charsY'         => ['n' => 'cY', 'd' => false, 'depth' => ['reverseDirection', 'charsY']],
			'charsDirection' => ['n' => 'cD', 'd' => false, 'depth' => ['reverseDirection', 'charsDirection']],
			'wordsX'         => ['n' => 'wX', 'd' => false, 'depth' => ['reverseDirection', 'wordsX']],
			'wordsY'         => ['n' => 'wY', 'd' => false, 'depth' => ['reverseDirection', 'wordsY']],
			'wordsDirection' => ['n' => 'wD', 'd' => false, 'depth' => ['reverseDirection', 'wordsDirection']],
			'linesX'         => ['n' => 'lX', 'd' => false, 'depth' => ['reverseDirection', 'linesX']],
			'linesY'         => ['n' => 'lY', 'd' => false, 'depth' => ['reverseDirection', 'linesY']],
			'linesDirection' => ['n' => 'lD', 'd' => false, 'depth' => ['reverseDirection', 'linesDirection']],
		];
		...
		if (!empty($frames)) {
			...
			foreach ($frames as $fk => $frame) {
				...
				if ($this->get_val($frame, ['filter', 'use']) === true) {
					$use['grayscale'] = $_base['grayscale'];
					$use['brightness'] = $_base['brightness'];
					$use['blur'] = $_base['blur'];
				}

				if ($this->get_val($frame, ['color', 'use']) === true) {
					$use['color'] = $_base['color'];
				}

				if ($this->get_val($frame, ['bgcolor', 'use']) === true) {
					$use['backgroundColor'] = $_base['backgroundColor'];
				}

				if ($this->get_val($layer, ['timeline', 'clipPath', 'use']) === true) {
					$use['clip'] = $_base['clip'];
				...
				foreach ($use as $key => $v) {
					...
					if ($fk === 'frame_0' && $nv === 'inherit') {
						continue;
					}

					//inherit is ignored in frame_0
				...
				if ($this->get_val($frame, ['chars', 'use']) === true) {
					$split[] = 'chars';
				}

				if ($this->get_val($frame, ['words', 'use']) === true) {
					$split[] = 'words';
				}

				if ($this->get_val($frame, ['lines', 'use']) === true) {
					$split[] = 'lines';
				}
				...
				if (!empty($split)) {
					...
					foreach ($split as $splt) {
						...
						if ($this->get_val($frame, [$splt, 'fuse'], false) === true) {
				...
				if ($this->get_val($frame, ['mask', 'use']) === true) {
					$push['mask'] = [
				...
				if (!in_array($this->get_val($frame, ['sfx', 'effect']), ['', 'none'], true)) {
					$push['sfx'] = [
						'effect' => $_sfx['effect'],
						'color'  => $_sfx['color'],
					];
		...
		if ($this->get_val($layer, ['hover', 'usehover'], false) === true || $this->get_val($layer, ['hover', 'usehover'], false) === 'true' || $this->get_val($layer, ['hover', 'usehover'], false) === 'desktop') {
			$_frames['frame_hover'] = ['base' => []];

			$idle_v = $this->get_val($layer, 'idle', []);
			$hover_v = $this->get_val($layer, 'hover', []);
			...
			$hv = [
				...
				'z'                    => ['n' => 'z', 'd' => 0],
				'color'                => ['n' => 'c', 'd' => '#fff'],
				'backgroundColor'      => ['n' => 'bgc', 'd' => 'transparent'],
				'borderColor'          => ['n' => 'boc', 'd' => 'transparent'],
				'borderRadius'         => ['n' => 'bor', 'd' => '0,0,0,0', 'depth' => ['borderRadius', 'v']], //check further as it is stored in v
				'borderStyle'          => ['n' => 'bos', 'd' => 'none'],
				'borderWidth'          => ['n' => 'bow', 'd' => '0,0,0,0'],
				'transformPerspective' => ['n' => 'tp', 'd' => '600'],
				'originX'              => ['n' => 'oX', 'd' => '50%'],
				'originY'              => ['n' => 'oY', 'd' => '50%'],
				'originZ'              => ['n' => 'oZ', 'd' => '0'],
				'textDecoration'       => ['n' => 'td', 'd' => 'none'],
				'speed'                => ['n' => 'sp', 'd' => 300],
				'ease'                 => ['n' => 'e', 'd' => 'power3.inOut'],
				'zIndex'               => ['n' => 'zI', 'd' => 'auto'],
				'pointerEvents'        => ['n' => 'pE', 'd' => 'auto'],
				'grayscale'            => ['n' => 'gra', 'd' => 0, 'depth' => ['filter', 'grayscale']],
				'brightness'           => ['n' => 'bri', 'd' => 100, 'depth' => ['filter', 'brightness']],
				'blur'                 => ['n' => 'blu', 'd' => 0, 'depth' => ['filter', 'blur']],
				'usehovermask'         => ['n' => 'm', 'd' => false],
			];

			if ($this->get_val($layer, ['hover', 'usehover'], false) === 'desktop') {
				$hv['instantClick'] = ['n' => 'iC', 'd' => 'true'];
			}
			...
			foreach ($hv as $key => $v) {
				...
				if (is_object($nv) || is_array($nv)) {
					...
					if ($devices_exist) {
						...
						if ($this->adv_resp_sizes == true) {
							$nv = $this->normalize_device_settings($nv, $this->enabled_sizes, 'html-array', [$v['d']]);
						} else {
							$nv = $this->get_biggest_device_setting($nv, $this->enabled_sizes);
						}
				...
				*/
				...
				if (is_object($idle) || is_array($idle)) {
					...
					if ($devices_exist) {
						...
						if ($this->adv_resp_sizes == true) {
							$idle = $this->normalize_device_settings($idle, $this->enabled_sizes, 'html-array', [$v['d']]);
						} else {
		...
		if ($this->get_val($layer, ['timeline', 'tloop', 'use'], false) === true) {
			$_frames['tloop'] = ['base' => []];
			$t_from = $this->get_val($layer, ['timeline', 'tloop', 'from'], 'frame_1');
			$t_to = $this->get_val($layer, ['timeline', 'tloop', 'to'], 'frame_999');
			$t_repeat = $this->get_val($layer, ['timeline', 'tloop', 'repeat'], -1);
			$t_keep = $this->get_val($layer, ['timeline', 'tloop', 'keep'], true);
			$t_child = $this->get_val($layer, ['timeline', 'tloop', 'children'], true);

			if ($t_from !== 'frame_1') {
				$_frames['tloop']['base']['f'] = $t_from;
			}

			if ($t_to !== 'frame_999') {
				$_frames['tloop']['base']['t'] = $t_to;
			}

			if ($t_keep === false) {
				$_frames['tloop']['base']['k'] = 'false';
			}

			if (!in_array($t_repeat, [-1, '-1'])) {
				$_frames['tloop']['base']['r'] = $t_repeat;
			}

			if ($t_child === false && in_array($this->get_val($layer, 'type', 'text'), ['group', 'row', 'column'], true)) {
				$_frames['tloop']['base']['c'] = $t_child;
			}

			if (empty($_frames['tloop']['base'])) {
				$_frames['tloop']['base']['u'] = true;
			}

			//if empty, set u to true so that frontend knows that it is set
		}
		...
		if (!empty($_frames)) {
			...
			$start_cache_999 = $this->get_val($_frames, ['frame_999', 'base', 'st']);

			if ($this->get_val($frames, ['frame_999', 'timeline', 'endWithSlide'], false)) {
			...
			foreach ($frames as $frame => $zone) {
				$start_cache[$frame] = $this->get_val($_frames, [$frame, 'base', 'st']);

				$at = $this->get_val($zone, ['timeline', 'actionTriggered'], false);
				$trg = $this->layer_frame_triggered($uid, $frame);
				$ign = (!in_array($frame, ['frame_hover', 'frame_0'], true)) ? true : false;
			...
			foreach ($_frames as $frame => $zone) {
				...
				if ($frame !== 'frame_0' && in_array($this->get_val($layer, 'type', 'text'), ['group', 'row', 'column'], true)) {

					if (!isset($start_cache[$frame])) {
						$start_cache[$frame] = $this->get_val($_frames, [$frame, 'base', 'st']);
					}
			...
			if ($this->get_val($frames, ['frame_999', 'timeline', 'auto'], false)) {
				$_frames['frame_999']['base'] = [
					'st'   => $this->get_val($_frames, ['frame_999', 'base', 'st']),
					'sp'   => $this->get_val($_frames, ['frame_999', 'base', 'sp']),
					'auto' => 'true',
				];
			}
		...
		if ($this->get_val($layer, ['visibility', 'onlyOnSlideHover'], false) === true) {
			...
			if (!isset($_frames['frame_1'])) {
				$_frames['frame_1'] = [];
			}

			if (!isset($_frames['frame_999'])) {
				$_frames['frame_999'] = [];
			}

			if (!isset($_frames['frame_1']['base'])) {
				$_frames['frame_1']['base'] = [];
			}

			if (!isset($_frames['frame_999']['base'])) {
				$_frames['frame_999']['base'] = [];
			}
	...
	public function get_html_clip() {
		...
		if ($this->get_val($layer, ['timeline', 'clipPath', 'use']) === true) {
			$html .= 'u:true;';
			$type = $this->get_val($layer, ['timeline', 'clipPath', 'type'], 'rectangle');
			$origin = $this->get_val($layer, ['timeline', 'clipPath', 'origin'], 'l');
	...
	public function get_html_blendmode() {
		...
		$layer = $this->get_layer();
		$blendmode = $this->get_val($layer, ['idle', 'filter', 'blendMode'], 'normal');
	...
	public function get_html_spike_data() {
		...
		if ($this->get_val($layer, ['idle', 'spikeUse'], false) === true) {
			$l = $this->get_val($layer, ['idle', 'spikeLeft'], 'none');
			$r = $this->get_val($layer, ['idle', 'spikeRight'], 'none');
			$lw = $this->get_val($layer, ['idle', 'spikeLeftWidth'], 10);
			$rw = $this->get_val($layer, ['idle', 'spikeRightWidth'], 10);
			...
			if (!in_array($lw, [10, '10', '10%'], true)) {
				$html .= 'lw:' . $lw . ';';
			}

			if (!in_array($rw, [10, '10', '10%'], true)) {
				$html .= 'rw:' . $rw . ';';
			}
	...
	public function get_html_text_stroke() {
		...
		if ($this->get_val($layer, ['idle', 'textStroke', 'inuse'], false) === true) {
			$w = $this->get_val($layer, ['idle', 'textStroke', 'width'], '1px');
			$c = $this->get_val($layer, ['idle', 'textStroke', 'color'], 'rgba(0,0,0,0.25)');

			if (!in_array($w, [1, '1', '1px'], true)) {
				$html .= 'w:' . $w . ';';
			}

			if ($c !== 'rgba(0,0,0,0.25)') {
				$html .= 'c:' . $c . ';';
			}
	...
	public function get_html_frameorder() {
		...
		$layer = $this->get_layer();
		$frameorder = $this->get_val($layer, ['timeline', 'frameOrder'], '');
	...
	public function get_html_hideunder() {
		...
		$layer = $this->get_layer();
		return ($this->get_val($layer, ['visibility', 'hideunder']) === true) ? 'data-layeronlimit="on"' : '';
	}
	...
	public function get_html_audio_data() {
		...
		if ($this->get_val($layer, 'type', 'text') !== 'audio') {
			return '';
		}

		$vw = $this->get_val($layer, ['size', 'width']);
		$vh = $this->get_val($layer, ['size', 'height']);
		$vpl = $this->get_val($layer, ['media', 'preloadAudio'], 'auto');
		$sta = $this->get_val($layer, ['media', 'startAt'], -1);
		$end = $this->get_val($layer, ['media', 'endAt'], -1);
		$mp4 = RevLoader::esc_attr($this->remove_http($this->get_val($layer, ['media', 'audioUrl'])));
		$vl = $this->get_val($layer, ['media', 'loop'], true);
		$vpt = $this->get_val($layer, ['media', 'pausetimer'], false);
		$vpt = (in_array($vl, ['loop', 'none'], true)) ? true : $vpt;
		$ap = $this->get_val($layer, ['media', 'autoPlay'], 'true');
		$frw = $this->get_val($layer, ['media', 'forceRewind'], true);
		$vc = $this->get_val($layer, ['media', 'controls'], false);
		$nse = $this->get_val($layer, ['media', 'nextSlideAtEnd'], true);
		$sav = $this->get_val($layer, ['media', 'stopAllVideo'], true);
		$volume = $this->get_val($layer, ['media', 'volume'], 100);
		$mute = $this->get_val($layer, ['media', 'mute'], true);

		if ($this->adv_resp_sizes == true) {
			$data['video']['w'] = $this->normalize_device_settings($vw, $this->enabled_sizes, 'html-array', [54]);
			$data['video']['h'] = $this->normalize_device_settings($vh, $this->enabled_sizes, 'html-array', [54]);
		} else {
		...
		if (!in_array($vpl, ['', 'auto'], true)) {
			$data['video']['p'] = $vpl;
			$plw = intval($this->get_val($layer, ['media', 'preloadWait'], 5));

			if (!in_array($plw, ['5', 5], true)) {
				$data['video']['pw'] = $plw;
			}

		}

		if (!in_array($sta, ['', '-1', -1], true)) {
			$data['video']['sta'] = $sta;
		}

		if (!in_array($end, ['', '-1', -1], true)) {
			$data['video']['end'] = $end;
		}
		...
		if ($nse === false) {
			$data['video']['nse'] = false;
		}

		if ($sav === false) {
			$data['video']['sav'] = false;
		}
	...
	public function get_html_video_data() {
		...
		$mute = $this->get_val($layer, ['media', 'mute'], true);
		$volume = $this->get_val($layer, ['media', 'volume'], 100);
		$vargs = trim($this->get_val($layer, ['media', 'args']));
		$control = $this->get_val($layer, ['media', 'controls'], false);
		$sta = $this->get_val($layer, ['media', 'startAt']);
		$end = $this->get_val($layer, ['media', 'endAt']);
		$vl = $this->get_val($layer, ['media', 'loop'], true);
		$vpt = $this->get_val($layer, ['media', 'pausetimer'], false);
		$vpt = (in_array($vl, ['loop', 'none'], true)) ? true : $vpt;
		$autoplay = $this->get_val($layer, ['media', 'autoPlay'], 'true');
		$nextslide = $this->get_val($layer, ['media', 'nextSlideAtEnd'], true);
		$poster = $this->remove_http($this->get_val($layer, ['media', 'posterUrl'], ''));
		$poster_change = $this->get_val($layer, ['behavior', 'imageSourceType'], 'full');
		$poster_id = $this->remove_http($this->get_val($layer, ['media', 'posterId']));
		$mute = (!in_array($autoplay, ['false', false], true)) ? true : $mute;

		if (!in_array($autoplay, ['true', true], true)) {
			$data['video']['ap'] = $autoplay;
		}
		...
		if ($this->get_val($layer, ['media', 'posterOnPause'], false) !== false) {
			$data['video']['scop'] = 't';
		}

		if ($this->get_val($layer, ['media', 'forceRewind'], true) !== true) {
			$data['video']['rwd'] = 'f';
		}

		if ($this->get_val($layer, ['media', 'nointeraction'], false) !== false) {
			$data['video']['noint'] = 't';
		}

		if ($this->get_val($layer, ['size', 'covermode'], 'custom') === 'cover-proportional') {
			$ratio = $this->get_val($layer, ['media', 'ratio']);
			$data['video']['fc'] = true;

			if (!in_array($ratio, ['16:9', ''], true)) {
				$data['video']['ar'] = $ratio;
			}

		}

		$dotted = $this->get_val($layer, ['media', 'dotted']);

		if (!in_array($dotted, ['none', ''], true)) {
		...
		if ($this->get_val($layer, ['media', 'stopAllVideo'], true) === false) {
			$data['video']['sav'] = 'f';
		}

		if ($this->get_val($layer, ['media', 'allowFullscreen'], true) === false) {
			$data['video']['afs'] = 'f';
		}
		...
		if (!empty($poster)) {
			...
			if ($this->get_val($layer, ['media', 'disableOnMobile'], false) === true) {
				$data['video']['npom'] = 't';
			}

			if ($this->get_val($layer, ['media', 'posterOnMobile'], false) === true) {
				$data['video']['opom'] = 't';
			}
		...
		case 'youtube':
			...
			$sp = $this->get_val($layer, ['media', 'speed'], 1);
			$inl = $this->get_val($layer, ['media', 'playInline'], true);

			if (!$mute) {
				$vargs = 'volume=' . (int) ($volume) . '&' . $vargs;
			}
		...
		case 'streaminstagramboth':
		case 'html5':
			$ogv = $this->get_val($layer, ['media', 'ogvUrl'], '');
			$webm = $this->get_val($layer, ['media', 'webmUrl'], '');
			$mp4 = $this->remove_http($this->get_val($layer, ['media', 'mp4Url'], ''));
			$vpr = $this->get_val($layer, ['media', 'preload'], 'auto');
			$inl = $this->get_val($layer, ['media', 'playInline'], true);
	...
	public function get_html_column_data() {
		...
		if ($type == 'column') {
			$size_raw = explode('/', $this->get_val($layer, ['group', 'columnSize'], '1/3'));
			$size = (count($size_raw) !== 2) ? '100' : round(100 * ((int) $size_raw[0] / (int) $size_raw[1]), 2);
			$va = $this->get_val($layer, ['idle', 'verticalAlign'], 'top');

			$data .= (!in_array($size, ['', '33,33', '33.33', 33.33], true)) ? 'w:' . $size . '%;' : '';
			$data .= ($va !== 'top') ? 'a:' . $va . ';' : '';
		}
	...
	public function get_html_margin_data($row_group_uid) {
		...
		if ($type == 'row' || $type == 'column' || $row_group_uid !== false) {
			$margins = $this->get_val($layer, ['idle', 'margin']);
			$rl_margin = [];

			$margin_vals = $this->normalize_device_settings($margins, $this->enabled_sizes, 'obj', [0]);
	...
	public function get_html_padding_data() {
		...
		$layer = $this->get_layer();
		$paddings = $this->get_val($layer, ['idle', 'padding']);
		$data = 'data-padding="';
		$rl_padding = [];

		$padding_vals = $this->normalize_device_settings($paddings, $this->enabled_sizes, 'obj', [0]);
		...
		foreach ($padding_vals as $padding) {
			...
			if (!empty($padding)) {
				...
				foreach ($padding as $mkey => $mar) {
					$rl_padding[$mkey][] = str_replace(['px', '%'], '', $mar);
				}
			...
			for ($i = 0; $i < 4; $i++) {
				...
				if (isset($rl_padding[$i])) {
					...
					if (is_array($rl_padding[$i])) {
						...
						$mm = str_replace(['%', 'px'], '', $rl_padding[$i]);
	...
	public function get_html_border_data() {
		...
		if ($this->adv_resp_sizes == true) {
			$style['bos'] = $this->normalize_device_settings($this->get_val($layer, ['idle', 'borderStyle'], 'none'), $this->enabled_sizes, 'html-array', ['none']);
		} else {
			$style['bos'] = $this->get_biggest_device_setting($this->get_val($layer, ['idle', 'borderStyle'], 'none'), $this->enabled_sizes);
		}

		$style['boc'] = $this->get_val($layer, ['idle', 'borderColor']);
		$style['bow'] = $this->get_val($layer, ['idle', 'borderWidth'], '0px');
		$style['bow'] = (is_array($style['bow']) || is_object($style['bow'])) ? implode(',', (array) $style['bow']) : $style['bow'];
		$style['bor'] = $this->get_val($layer, ['idle', 'borderRadius', 'v']);
		$style['bor'] = (is_array($style['bor']) || is_object($style['bor'])) ? implode(',', (array) $style['bor']) : $style['bor'];

		if (in_array($style['bow'], [0, '0px', '0px,0px,0px,0px', '0', '0,0,0,0'], true) || $style['bos'] === 'none') {
			unset($style['bow']);
			unset($style['boc']);
			unset($style['bos']);
		}

		if (in_array($style['bor'], [0, '0px', '0px,0px,0px,0px', '0', '0,0,0,0'], true)) {
			unset($style['bor']);
		}
	...
	public function get_background_image() {
		...
		if (in_array($type, ['shape', 'row', 'group'], true)) {
			$url_image = $this->get_val($layer, ['idle', 'backgroundImage'], '');
			...
			if ($url_image !== '') {
				...
				if (in_array($type, ['group', 'shape', 'row'])) {
					$image .= "background: url('" . $this->remove_http($url_image) . "')";
					$image .= ' ' . $this->get_val($layer, ['idle', 'backgroundRepeat'], 'no-repeat');
					$image .= ' ' . $this->get_val($layer, ['idle', 'backgroundPosition'], 'center center');
					$image .= ';';
					$bgs = $this->get_val($layer, ['idle', 'backgroundSize'], 'cover');
					$bgs = ($bgs === 'percentage') ? $this->get_val($layer, ['idle', 'backgroundSizePerc'], '100') . '%' : $bgs;
					$bgs = ($bgs === 'pixel') ? $this->get_val($layer, ['idle', 'backgroundSizePix'], '100') . 'px' : $bgs;
	...
	public function get_stream_background_image($layer) {
		...
		if ($this->slider->get_param('sourcetype') !== 'gallery') {
			...
			if ($this->slider->get_param('sourcetype') === 'post') {
				...
				if (!empty($bgi['id'])) {
					$bgi['size'] = $this->get_val($layer, ['behavior', 'streamSourceType'], 'full');
				...
				$bgi['id'] = $slide->get_id();
				$bgi['size'] = 'full';
				$bgi['url'] = $this->get_val($layer, ['media', 'imageUrl'], '');
			}
	...
	public function get_loop_data() {
		...
		$layer = $this->get_layer();
		$loop = ['loop_0' => '', 'loop_999' => '']; //needs to be pushed as loop_* in frontend
		$loop_keys = ['frame_0' => '', 'frame_999' => '']; //stored as frame_* in database

		if ($this->get_val($layer, ['timeline', 'loop', 'use'], false) === true) {

			$e = $this->get_val($layer, ['timeline', 'loop', 'ease'], 'none');
			$sp = $this->get_val($layer, ['timeline', 'loop', 'speed'], 1000);
			$rA = $this->get_val($layer, ['timeline', 'loop', 'radiusAngle'], 0);
			$crns = $this->get_val($layer, ['timeline', 'loop', 'curviness'], 2);
			$crd = $this->get_val($layer, ['timeline', 'loop', 'curved'], false);
			$yym = $this->get_val($layer, ['timeline', 'loop', 'yoyo_move'], false);
			$yyr = $this->get_val($layer, ['timeline', 'loop', 'yoyo_rotate'], false);
			$yys = $this->get_val($layer, ['timeline', 'loop', 'yoyo_scale'], false);
			$yyf = $this->get_val($layer, ['timeline', 'loop', 'yoyo_filter'], false);
			$rep = $this->get_val($layer, ['timeline', 'loop', 'repeat'], '-1');
			$st = $this->get_val($layer, ['timeline', 'loop', 'start'], 740);
			$aR = $this->get_val($layer, ['timeline', 'loop', 'autoRotate'], false);
			$oX = $this->get_val($layer, ['timeline', 'loop', 'originX'], '50%');
			$oY = $this->get_val($layer, ['timeline', 'loop', 'originY'], '50%');
			$oZ = $this->get_val($layer, ['timeline', 'loop', 'originZ'], '0');
			...
			$all_keys = [
				...
				'scaleY'     => ['n' => 'sY', 'd' => 1],
				'opacity'    => ['n' => 'o', 'd' => 1],
				'rotationX'  => ['n' => 'rX', 'd' => [0, '0', '0deg']],
				'rotationY'  => ['n' => 'rY', 'd' => [0, '0', '0deg']],
				'rotationZ'  => ['n' => 'rZ', 'd' => [0, '0', '0deg']],
				'skewX'      => ['n' => 'skX', 'd' => [0, '0', '0px', '']],
				'skewY'      => ['n' => 'skY', 'd' => [0, '0', '0px', '']],
				'blur'       => ['n' => 'blu', 'd' => 0],
				'brightness' => ['n' => 'bri', 'd' => 100],
				'grayscale'  => ['n' => 'gra', 'd' => 0],
			];
			...
			foreach ($loop_keys as $l => $lv) {
				...
				foreach ($all_keys as $key => $v) {
					...
					$nv = $this->get_val($layer, ['timeline', 'loop', $l, $key], $d);
	...
	public function get_toggle_data() {
		...
		$type = $this->get_val($layer, ['type', 'text']);
		$text_toggle = $this->get_val($layer, ['toggle', 'text']);
		$toggle['allow'] = $this->get_val($layer, ['toggle', 'set'], false);
		$toggle['inverse_content'] = $this->get_val($layer, ['toggle', 'inverse'], false);
		$toggle['html'] = '';

		if (!in_array($type, ['shape', 'svg', 'image'], true)) {
		...
		foreach ($this->icon_sets as $is) {
			...
			if (strpos($toggle['html'], $is) !== false) {
		...
		if ($this->adv_resp_sizes == true) {
			$ws = $this->normalize_device_settings($this->get_val($layer, ['idle', 'whiteSpace']), $this->enabled_sizes, 'html-array', ['nowrap']);
		} else {
			$ws = $this->get_biggest_device_setting($this->get_val($layer, ['idle', 'whiteSpace'], 'nowrap'), $this->enabled_sizes);
		}
	...
	public function get_html_corners() {
		...
		if (in_array($this->get_val($layer, 'type', 'text'), ['text', 'button', 'shape'], true)) {
			$cl = $this->get_val($layer, ['idle', 'cornerLeft'], 'none');
			$cr = $this->get_val($layer, ['idle', 'cornerRight'], 'none');

			$html .= (!in_array($cl, ['', 'none'], true)) ? $cl . ';' : '';
			$html .= (!in_array($cr, ['', 'none'], true)) ? $cr . ';' : '';
		}
	...
	public function get_html_disp() {
		...
		if ($this->container_mode === 'column' && $type !== 'row' && $this->get_val($layer, ['idle', 'display'], 'block') !== 'block') {
			$html .= $this->get_val($layer, ['idle', 'display']);
		}
	...
	public function get_html_layer() {
		...
		case 'image':
			$additional = '';
			$cover_mode = $this->get_val($layer, ['size', 'covermode'], 'custom');
			$urlImage = $this->get_val($layer, ['media', 'imageUrl']);
			$cur_img_id = $this->get_val($layer, ['media', 'imageId']);
			$img_change = $this->get_val($layer, ['behavior', 'imageSourceType'], 'auto');
			$img_size = 'full';
			$img_w = '';
			$img_h = '';
			$alt = $this->get_val($layer, ['attributes', 'alt']);
			$alt_option = $this->get_val($layer, ['attributes', 'altOption'], 'media_library');
			...
			if ($cur_img_id !== false && !empty($cur_img_id)) {
				...
				//we might be from image library
				$objlib = new RevSliderObjectLibrary();
			...
			$urlImage = str_replace("revslider//", "revslider/", $urlImage);
	...
	public function set_placeholders($text) {
		...
		$text = str_replace(['%home_url%', '{{home_url}}'], RevLoader::esc_url(RevLoader::home_url('/')), $text);
		//track
		//if(isset($post->ID)) $text = str_replace(array('%current_page_link%', '{{current_page_link}}'), get_permalink($post->ID), $text);

		if (isset($post->ID)) {
			$text = RevLoader::apply_filters('revslider_gallery_set_placeholders', $text, $post->ID);
		}
	...
	public function get_html_layer_device_visibility() {
		...
		$layer = $this->get_layer();
		$vis_desktop = ($this->get_val($layer, ['visibility', 'd'], true) === true) ? 't' : 'f';
		$vis_notebook = ($this->get_val($layer, ['visibility', 'n'], true) === true) ? 't' : 'f';
		$vis_tablet = ($this->get_val($layer, ['visibility', 't'], true) === true) ? 't' : 'f';
		$vis_mobile = ($this->get_val($layer, ['visibility', 'm'], true) === true) ? 't' : 'f';
	...
	public function is_full_width_video() {
		...
		$layer = $this->get_layer();
		return ($this->get_val($layer, 'type', 'text') == 'video' && $this->get_val($layer, ['size', 'covermode']) == 'cover-proportional') ? true : false;
	}
	...
	public function is_in_group_or_row() {
		...
		$layer = $this->get_layer();
		$puid = $this->get_val($layer, ['group', 'puid']);
		return ((int) ($puid) > 0) ? true : false;
	}
	...
	public function check_if_stream_video_exists() {
		...
		switch ($slide->get_param(['bg', 'type'], 'trans')) {
		case 'streamyoutubeboth': //youtube
			$vid = $slide->get_param(['bg', 'youtube'], '');
			break;
		case 'streamvimeoboth': //vimeo
			$vid = $slide->get_param(['bg', 'vimeo'], '');
			break;
		case 'streaminstagramboth': //instagram
			$vid = $slide->get_param(['bg', 'mpeg'], '');
			break;
	...
	public function add_html_background_video() {
		...
		$mute_video = $slide->get_param(['bg', 'video', 'mute'], true);
		//if($mute_video === true) $data['video']['v'] = 'mute'; //dont write, as mute is default
		$volume = $slide->get_param(['bg', 'video', 'volume'], '100');
		$video_type = $slide->get_param(['bg', 'type'], 'trans');
		$http = (RevLoader::is_ssl()) ? 'https://' : 'http://';
		...
		case 'twitter':
			$youtube_id = $slide->get_param(['bg', 'youtube'], '');
			$vimeo_id = $slide->get_param(['bg', 'vimeo'], '');
			$html_mpeg = $this->remove_http($slide->get_param(['bg', 'mpeg'], ''));

			if ($youtube_id === '' && $vimeo_id === '' && $html_mpeg === '') {
				return false;
			}

			if ($youtube_id !== '') {
				$arguments = $slide->get_param(['bg', 'video', 'args'], RevSliderFront::YOUTUBE_ARGUMENTS);
				$arguments = (empty($arguments)) ? RevSliderFront::YOUTUBE_ARGUMENTS : $arguments;

				if ($mute_video === false) {
					$data['video']['v'] = (int) ($volume);
					$arguments = 'volume=' . (int) ($volume) . '&amp;' . $arguments;
				}
				...
				$sp = $slide->get_param(['bg', 'video', 'speed'], 1);
			...
			if ($vimeo_id !== '') {
				$arguments = $slide->get_param(['bg', 'video', 'argsVimeo'], RevSliderFront::VIMEO_ARGUMENTS);
		...
		case 'html5':
			...
			if ($video_type == 'streaminstagram' || $video_type == 'streaminstagramboth') {
				...
				$html_webm = $slide->get_param(['bg', 'webm'], '');
				$html_ogv = $slide->get_param(['bg', 'ogv'], '');
			}

			//$data['video']['p'] = 'auto'; //default is auto, so do not write

			if (!empty($html_ogv)) {
				$data['videoogv'] = $html_ogv;
			}

			if (!empty($html_webm)) {
				$data['videowebm'] = $html_webm;
			}
		...
		$ratio = $slide->get_param(['bg', 'video', 'ratio'], '16:9');
		$loop = $slide->get_param(['bg', 'video', 'loop'], true);
		$vpt = $slide->get_param(['bg', 'video', 'pausetimer'], false);

		if ($loop === 'loop') {
			$vpt = true;
		}

		$nsae = $slide->get_param(['bg', 'video', 'nextSlideAtEnd'], false);
		$sat = $slide->get_param(['bg', 'video', 'startAfterTransition'], false);
		$vsa = $slide->get_param(['bg', 'video', 'startAt'], '');
		$vea = $slide->get_param(['bg', 'video', 'endAt'], '');

		if (!in_array($vsa, ['', '-1', -1], true)) {
			$data['video']['sta'] = $vsa;
		}

		if (!in_array($vea, ['', '-1', -1], true)) {
			$data['video']['end'] = $vea;
		}
		...
		if ($slide->get_param(['bg', 'video', 'forceRewind'], true) === false) {
			$data['video']['rwd'] = false;
		}
		...
		if ($slide->get_param(['bg', 'video', 'forceCover'], true) == false) {$data['video']['fc'] = false;}

		$do = $slide->get_param(['bg', 'video', 'dottedOverlay'], 'none');
	...
	public function get_html_slide_description() {
		...
		if ($this->slider->is_posts()) {
			...
			$description = $slide->get_param(['info', 'description'], '');
		} else {
			$description = $slide->get_param(['info', 'description'], '');
		}

		$description = trim(str_replace(["\'", '\"'], ["'", '"'], RevLoader::esc_attr($description)));
	...
	public function get_thumb_url() {
		...
		$active = ($this->slider->get_param(['nav', 'bullets', 'set'], false) == true || $this->slider->get_param(['nav', 'thumbs', 'set'], false) == true || $this->slider->get_param(['nav', 'arrows', 'set'], false) == true || $this->slider->get_param(['nav', 'tabs', 'set'], false) == true) ? true : false;
		$special = (
			in_array($this->slider->get_param(['nav', 'arrows', 'style'], 'round'), ['preview1', 'preview2', 'preview3', 'preview4', 'custom'], true) ||
			in_array($this->slider->get_param(['nav', 'bullets', 'style'], 'round'), ['preview1', 'preview2', 'preview3', 'preview4', 'custom'], true)
		) ? true : false;

		if ($active === false && $special == false) {
			return '';
		}

		$slide = $this->get_slide();
		$url = ($this->slider->is_posts() && $slide->get_param(['bg', 'imageFromStream'], false) === true) ? '' : $slide->get_param(['thumb', 'customThumbSrc'], '');

		if (
			$slide->get_param(['thumb', 'dimension'], 'slider') == 'slider' &&
			(in_array($this->slider->get_param('sourcetype'), ['youtube', 'vimeo'], true) ||
				in_array($slide->get_param(['bg', 'type'], 'trans'), ['image', 'vimeo', 'youtube', 'html5', 'streamvimeo', 'streamyoutube', 'streaminstagram', 'streamtwitter', 'streamvimeoboth', 'streamyoutubeboth', 'streaminstagramboth', 'streamtwitterboth'], true))
		) {
			//use the slider settings for width / height
			$w = intval($this->slider->get_param(['nav', 'preview', 'width'], $this->slider->get_param(['nav', 'thumbs', 'width'], 100)));
			$h = intval($this->slider->get_param(['nav', 'preview', 'height'], $this->slider->get_param(['nav', 'thumbs', 'height'], 50)));
	...
	public function get_slide_link() {
		...
		if ($slide->get_param(['seo', 'set'], false) == true) {
			...
			switch ($slide->get_param(['seo', 'type'], 'regular')) {
			default: //---- normal link
			case 'regular':
				$target = ' data-target="' . $slide->get_param(['seo', 'target'], '_self') . '"';
				$http = $slide->get_param(['seo', 'linkHelp'], 'auto');
				$l = $this->remove_http($slide->get_param(['seo', 'link'], ''), $http);
				//$link	= ($l !== '') ? ' data-link="'.do_shortcode($l).'"'.$target : $link;
				$link = ($l !== '') ? ' data-link="' . $l . '"' . $target : $link;
				break;
			case 'slide': //---- link to slide
				$slide_link = $this->get_val($params, ['seo', 'slideLink'], 'nothing');
			...
			//set link position:
			$link .= ' data-seoz="' . $this->get_val($params, ['seo', 'z'], 'front') . '"';
		}
	...
	public function get_html_delay() {
		...
		$slide = $this->get_slide();
		$delay = $slide->get_param(['timeline', 'delay'], 'default');
		$delay = strtolower($delay);

		return (!in_array($delay, ['default', ''], true)) ? ' data-duration="' . $delay . '"' : '';
	}
	...
	public function get_html_scrollbased_slidedata() {
		...
		$slide = $this->get_slide();
		$html = 'data-sba="';
		$sd = $this->slider->get_param(['scrolltimeline', 'set'], false);
		$es = $this->slider->get_param(['scrolleffects', 'set'], false);

		//$s = $slide->get_param(array('timeline', 'scrollBased'), 'default');
		//if ($s !== 'default' && $sd != false) $html .='t:'.($s=='true' ? 'true' : 'false').';';

		if ($es === true) {
			$fa = $slide->get_param(['effects', 'fade'], 'default');
			$bl = $slide->get_param(['effects', 'blur'], 'default');
			$gr = $slide->get_param(['effects', 'grayscale'], 'default');
	...
	public function get_html_stop_slide() {
		...
		$slide = $this->get_slide();
		return ($this->_truefalse($slide->get_param(['timeline', 'stopOnPurpose'], false)) === true) ? ' data-ssop="true"' : '';
	}
	...
	public function get_html_invisible() {
		...
		$slide = $this->get_slide();
		return ($this->_truefalse($slide->get_param(['visibility', 'hideFromNavigation'], false)) === true) ? ' data-invisible="true"' : '';
	}
	...
	public function get_html_duration() {
		...
		$slide = $this->get_slide();
		$duration = $slide->get_param(['timeline', 'duration'], ''); //$this->slider->get_param(array('def', 'transitionDuration'), '')
	...
	public function get_html_rotation() {
		...
		$slide = $this->get_slide();
		$rotation = (array) $slide->get_param(['timeline', 'rotation'], '');
		$html_rotation = '';
	...
	public function get_html_ease_in() {
		...
		$slide = $this->get_slide();
		$easein = $slide->get_param(['timeline', 'easeIn'], ['default']);
	...
	public function get_html_ease_out() {
		...
		$slide = $this->get_slide();
		$easeout = $slide->get_param(['timeline', 'easeOut'], ['default']);
	...
	public function get_html_transitions() {
		...
		$slide = $this->get_slide();
		$transition = $slide->get_param(['timeline', 'transition'], 'fade');
	...
	public function get_html_random_animations() {
		...
		$sl = $this->get_slide();
		$t = $sl->get_param(['timeline', 'transition'], 'fade');
	...
	public function get_html_slide_loop() {
		...
		if ($slide->get_param(['timeline', 'loop', 'set'], false) === true) {
			$html .= 's:' . $slide->get_param(['timeline', 'loop', 'start'], '2500') . ';';

			$lend = $slide->get_param(['timeline', 'loop', 'end'], '4500');

			if (!empty($lend) && is_numeric($lend)) {
				$html .= 'e:' . $lend . ';';
			}

			$rpt = $slide->get_param(['timeline', 'loop', 'repeat'], 'unlimited');

			if (!empty($rpt) && $rpt !== 'unlimited') {
				$html .= 'r:' . $rpt . ';';
			}
	...
	public function get_first_anim_data() {
		...
		if ($this->slider->get_param(['general', 'firstSlide', 'set'], false) == true && $this->slider->get_param('type') !== 'hero') {
			$html .= 't:' . $this->slider->get_param(['general', 'firstSlide', 'type'], 'fade') . ';';

			$duration = $this->slider->get_param(['general', 'firstSlide', 'duration'], '300');

			if (!empty($duration) && is_numeric($duration)) {
				$html .= 's:' . $duration . ';';
			}

			$amount = $this->slider->get_param(['general', 'firstSlide', 'slotAmount'], '7');
	...
	public function get_html_media_filter() {
		...
		$slide = $this->get_slide();
		$filter = $slide->get_param(['bg', 'mediaFilter'], 'none');
	...
	public function get_html_slide_class() {
		...
		$slide = $this->get_slide();
		$class = $slide->get_param(['attributes', 'class'], '');
	...
	public function get_html_slide_id() {
		...
		$slide = $this->get_slide();
		$id = $slide->get_param(['attributes', 'id'], '');
		return ($id != '') ? ' id="' . $id . '"' : '';
	}
	...
	public function get_html_extra_data() {
		...
		$slide = $this->get_slide();
		$data = stripslashes($slide->get_param(['attributes', 'data'], ''));
		echo ($data != '') ? ' ' . $data : '';
	}
	...
	public function get_html_hide_after_loop() {
		...
		$slide = $this->get_slide();
		$hal = $slide->get_param(['visibility', 'hideAfterLoop'], 0);
	...
	public function get_html_hide_slide_mobile() {
		...
		$slide = $this->get_slide();
		$hsom = $slide->get_param(['visibility', 'hideOnMobile'], false);
	...
	public function get_html_extra_params() {
		...
		for ($mi = 0; $mi < 10; $mi++) {
			$pa = $slide->get_param(['info', 'params', $mi, 'v'], '');

			if ($pa !== '') {
				$pa_limit = $slide->get_param(['info', 'params', $mi, 'l'], 10);
				$pa = strip_tags($pa);
	...
	public function get_html_image_video_ratio() {
		...
		if ($s->get_param('type', 'standard') !== 'carousel') {
			return '';
		}

		if ($s->get_param(['carousel', 'justify'], false) !== true) {
			return '';
		}
		...
		case 'image':
			...
			if (!empty($data)) {
				$size = $slide->get_param(['bg', 'imageSourceType'], 'full');

				if ($size !== 'full') {

					if (isset($data['sizes']) && isset($data['sizes'][$size])) {
						$width = $this->get_val($data, ['sizes', $size, 'width'], '1');
						$height = $this->get_val($data, ['sizes', $size, 'height'], '1');
		...
		case 'youtube':
			...
			switch ($slide->get_param(['bg', 'video', 'ratio'], '16:9')) {
			case '16:9':
				$ratio = round(16 / 9, 5);
				break;
			case '4:3':
	...
	public function get_html_slot_amount() {
		...
		$slots = $slide->get_param(['timeline', 'slots'], '7');

		if (is_array($slots) || is_object($slots)) {
			$slots = implode(',', (array) $slots);
		}
	...
	public function remove_navigation() {
		...
		$this->slider->set_param(['nav', 'arrows', 'set'], false);
		$this->slider->set_param(['nav', 'bullets', 'set'], false);
		$this->slider->set_param(['nav', 'tabs', 'set'], false);
		$this->slider->set_param(['nav', 'thumbs', 'set'], false);
	}
	...
	public function set_gallery_slides($slides) {
		...
		if (count($slides) > 0) {
				...
				if (count($gallery_ids) < count($slides)) {
					...
					while (count($slides) < count($gallery_ids)) {
						...
						foreach ($slides as $slide) {
							...
							if (count($slides) >= count($gallery_ids)) {
								break;
							}
			...
			$size = $this->slider->get_param(['def', 'background', 'imageSourceType'], 'full');
			$gi = 0;
	...
	public function remove_slide_if_mobile($slides) {
		...
		if ($mobile && !empty($slides)) {
			...
			foreach ($slides as $ss => $sv) {
				...
				if ($sv->get_param(['visibility', 'hideOnMobile'], false) === true) {
					unset($slides[$ss]);
				}
	...
	private function get_hero_slide($slides) {
		...
		$hero_id = $this->slider->get_param(['hero', 'activeSlide'], -1);
		...
		foreach ($slides as $slide) {
			...
			if ($this->get_language() !== 'all') {
				...
				if ($slide->get_param(['child', 'parentId'], '') == $hero_id) {
					return $slide;
				}
		...
		foreach ($order as $order_slideid) {
			...
			foreach ($temp_slides as $temp_slide) {
				...
				if ($temp_slide->get_id() == $order_slideid) {
					$temp_slide->set_param(['publish', 'state'], 'published'); //set to published
					$slides[] = $temp_slide;
					break;
				}
	...
	public function check_add_to($empty_is_false = false) {
		...
		if ($empty_is_false && empty($add_to)) {
			return false;
		}

		if ($add_to == 'homepage') {
			//only add if we are the homepage
		...
		if (!empty($add_to)) {
			...
			if (!empty($add_to_pages)) {
				...
				if (array_search($cp_id, $add_to_pages) === false) {
					return false;
				}
	...
	public function set_general_params_for_layers() {
		...
		$this->enabled_sizes = [
			'd' => true,
			'n' => $this->slider->get_param(['size', 'custom', 'n'], false),
			't' => $this->slider->get_param(['size', 'custom', 't'], false),
			'm' => $this->slider->get_param(['size', 'custom', 'm'], false),
		];
		$this->adv_resp_sizes = ($this->enabled_sizes['n'] == true || $this->enabled_sizes['t'] == true || $this->enabled_sizes['m'] == true) ? true : false;

		$this->icon_sets = $this->set_icon_sets([]);
	}
	...
	private function modify_slide_by_skin() {
		...
		if (empty($this->custom_skin)) {
			return true;
		}

		//1 + 2

		if (!isset($this->custom_skin_data[$this->custom_skin])) {
			$this->custom_skin_data[$this->custom_skin] = [];
		}

		$_mod = $this->get_val($this->custom_skin_data, [$this->custom_skin, 'slide'], []);
	...
	private function modify_layers_by_skin() {
		...
		$_mod = $this->get_val($this->custom_skin_data, [$this->custom_skin, 'layers'], []);
	...
	private function modify_preview_mode_settings() {
		...
		$this->slider->set_param(['troubleshooting', 'jsInBody'], $js_to_body);
	}
	...
	private function set_fonts() {
		...
		$gfsub = $this->slider->get_param('subsets', []);
	...
	public function set_clean_font_import($font, $class = '', $url = '', $variants = [], $subsets = []) {
		...
		if (!empty($variants) || !empty($subsets)) {
			...
			if (!isset($revslider_fonts['queue'][$font])) {
				$revslider_fonts['queue'][$font] = [];
			}

			if (!isset($revslider_fonts['queue'][$font]['variants'])) {
				$revslider_fonts['queue'][$font]['variants'] = [];
			}

			if (!isset($revslider_fonts['queue'][$font]['subsets'])) {
				$revslider_fonts['queue'][$font]['subsets'] = [];
			}
			...
			if (!empty($variants)) {
				...
				foreach ($variants as $k => $v) {
					...
					if (!in_array($v, $revslider_fonts['queue'][$font]['variants'], true)) {
						...
						//already included somewhere, so do not call it anymore
						unset($variants[$k]);
					}
	...
	public function add_no_slides_markup() {
		...
		if ($this->slider->is_posts()) {
			$text = $this->l('No slides found, please add at least one Slide Template to the choosen language.');
		} else {
			$text = $this->l('No slides found, please add some slides');
		}
	...
	public function set_slide_loop($slides) {
		...
		$loop = $this->slider->get_param(['general', 'slideshow', 'loopSingle'], true);
	...
	public function is_in_timeframe() {
		...
		if ($this->get_preview_mode() === false) {
			...
			$date_from = $slide->get_param(['publish', 'from'], '');
			$date_to = $slide->get_param(['publish', 'to'], '');
	...
	public function add_inline_double_jquery_error($do_check = false) {
		...
		if ($rs_double_jquery_script === false || $do_check === true) {
			...
			echo RS_T4 . 'err += "To fix this, you can:<br>&nbsp;&nbsp;&nbsp; 1. Set \'Plugin General Options\' -> \'Advanced\' -> \'jQuery & OutPut Filters\' -> \'Put JS to Body\' to on";' . "\n";
	...
	public function get_html_js_start_size($optFullWidth, $optFullScreen) {
		...
		$jus = $this->slider->get_param(['carousel', 'justify'], false);
		...
		if (!$this->get_markup_export()) {
			...
			if ($this->slider->get_param('type', 'standard') !== 'hero') {
				...
				foreach ($check as $nk => $nav) {
					...
					if ($this->slider->get_param(['nav', $nav, 'set'], false) !== true) {
						continue;
					}

					if ($this->slider->get_param(['nav', $nav, 'innerOuter'], 'inner') === 'outer-vertical') {
						$html .= $nk . 'w:"' . $this->slider->get_param(['nav', $nav, 'widthMin'], 100) . '",';
						$do = true;
					}

					if ($this->slider->get_param(['nav', $nav, 'innerOuter'], 'inner') === 'outer-horizontal') {
						$wp = intval($this->slider->get_param(['nav', $nav, 'padding'], $wpd[$nav]));
						$h = $this->slider->get_param(['nav', $nav, 'height'], 50);
						$h = ($wp > 0) ? $h + $wp * 2 : $h;
					...
					if ($this->slider->get_param(['nav', $nav, 'hideUnder'], false) === false) {
						continue;
					}

					$html .= $nk . 'hide:"' . $this->slider->get_param(['nav', $nav, 'hideUnderLimit'], 0) . '",';
				}

			}

			if ($this->slider->get_param('layouttype') == 'fullscreen') {
				$html .= "offsetContainer:'" . RevLoader::esc_attr($this->slider->get_param(['size', 'fullScreenOffsetContainer'], '')) . "',";
				$html .= "offset:'" . RevLoader::esc_attr($this->slider->get_param(['size', 'fullScreenOffset'], '')) . "',";
			}

			$mheight = ($this->slider->get_param('layouttype') !== 'fullscreen') ? $this->slider->get_param(['size', 'minHeight'], 0) : $this->slider->get_param(['size', 'minHeightFullScreen'], '0');
	...
	public function print_error_message($message, $open_page = false) {
		...
		$url = '';
		$page_url = '';
		$html .= ($this->rs_plugin_wrap_open === false) ? RS_T3 . '<rs-plugin-wrap id="' . $html_id . '_wrapper">' . "\n" : '';
		$html .= ($this->rs_plugin_open === false) ? RS_T4 . '<rs-plugin id="' . $html_id . '">' . "\n" : '';
		...
		$html .= (!empty($url)) ? '<br>' . $this->l('Please follow this link to edit the Slider:') : '';
		$html .= '</div>' . "\n";
		$html .= (!empty($url)) ? RS_T6 . '<a href="' . $url . '" target="_blank" rel="noopener" class="rs_error_message_button">Edit Plugin : "' . $this->slider->get_alias() . '"</a>' . "\n" : '';
		$html .= (!empty($page_url)) ? RS_T6 . '<a href="' . $page_url . '" target="_blank" rel="noopener" class="rs_error_message_button">Edit Page</a>' . "\n" : '';
		$html .= RS_T5 . '</div>' . "\n";
		$html .= ($this->rs_plugin_wrap_closed === false) ? RS_T4 . '</rs-plugin>' . "\n" : '';
		$html .= ($this->rs_plugin_closed === false) ? RS_T3 . '</rs-plugin-wrap>' . "\n" : '';
	...
	private function add_js() {
		...
		$js_to_footer = $this->_truefalse($this->get_val($global, ['script', 'footer'], false));
		...
		if ($js_to_footer === true && $this->preview_mode == false && $me == false) {
			...
			RevLoader::add_action('wp_print_footer_scripts', [$this, 'add_inline_js'], 100);
		}

		if ($me === true) {
			//for html markup export
	...
	public function js_get_base_pre() {
		...
		$html .= ($this->slider->get_param(['troubleshooting', 'jsNoConflict'], true) === true) ? RS_T6 . 'tpj.noConflict();' . "\n" : '';
	...
	public function js_get_custom_js() {
		...
		$html = '';
		$js = $this->slider->get_param(['codes', 'javascript'], '');
	...
	public function js_get_custom_css() {
		...
		$html = '';
		$css = $this->slider->get_param(['codes', 'css'], '');
	...
	public function get_spinner_markup() {
		...
		$spinner = (string) $this->slider->get_param(['layout', 'spinner', 'type'], '0');
		$color = $this->slider->get_param(['layout', 'spinner', 'color'], '#FFFFFF');
	...
	public function get_notices() {
		...
		if ($this->orig_html_id !== false) {
			$html .= RS_T4 . '<script type="text/javascript">' . "\n";
			$html .= RS_T5 . 'console.log("' . sprintf($this->l('Warning - ID: %s exists already and was converted to: %s'), $this->orig_html_id, $this->get_html_id()) . '")' . "\n";
			$html .= RS_T4 . '</script>' . "\n";
		}
	...
	public function js_get_fallback() {
		...
		$dpz = $s->get_param(['general', 'disablePanZoomMobile'], false);
		$sii = $s->get_param(['troubleshooting', 'simplify_ie8_ios4'], true); //was false
		$dfl = $s->get_param(['general', 'disableFocusListener'], false);
		$apvom = $s->get_param(['general', 'autoPlayVideoOnMobile'], true);

		if ($dpz !== false) {
			$fb['panZoomDisableOnMobile'] = $dpz;
		}
		...
		if ($s->get_param('type', 'standard') !== 'hero') {
			$nsof = $s->get_param(['general', 'nextSlideOnFocus'], false);

			if ($nsof !== false) {
				$fb['nextSlideOnWindowFocus'] = $nsof;
			}

		}

		if ($dfl !== false) {
			$fb['disableFocusListener'] = $dfl;
		}

		if ($apvom !== false) {
			$fb['allowHTML5AutoPlayOnAndroid'] = $apvom;
		}
	...
	public function js_get_progressbar() {
		...
		if ($s->get_param(['general', 'progressbar', 'set'], true) === false || $s_type === 'hero') {
			$html = RS_T8 . 'progressBar:{disableProgressBar:true},' . "\n";
		} else {

			$pb = [];

			$pb_basedon = $s->get_param(['general', 'progressbar', 'basedon'], 'slide');
			$pb_bgcolor = RSColorpicker::get($s->get_param(['general', 'progressbar', 'bgcolor'], 'transparent'));
			$pb_color = RSColorpicker::get($s->get_param(['general', 'progressbar', 'color'], 'rgba(255,255,255,0.5)'));
			$pb_gapcolor = RSColorpicker::get($s->get_param(['general', 'progressbar', 'gapcolor'], 'rgba(255,255,255,0.5)'));
			$pb_gap = $s->get_param(['general', 'progressbar', 'gap'], false);

			$pb_gaps = $s->get_param(['general', 'progressbar', 'gapsize'], '0');
			$pb_reset = $s->get_param(['general', 'progressbar', 'reset'], 'reset');
			$pb_horizontal = $s->get_param(['general', 'progressbar', 'horizontal'], 'left');
			$pb_ond = $s->get_param(['general', 'progressbar', 'visibility', 'd'], true);
			$pb_onn = $s->get_param(['general', 'progressbar', 'visibility', 'n'], true);
			$pb_ont = $s->get_param(['general', 'progressbar', 'visibility', 't'], true);
			$pb_onm = $s->get_param(['general', 'progressbar', 'visibility', 'm'], true);

			// take care about fall back on old vertical position if still exists
			$pb_vertical = $s->get_param(['general', 'progressbar', 'vertical'], 'bottom');
			$pb_old_position = $s->get_param(['general', 'progressbar', 'position'], 'bottom');

			if ($pb_old_position !== "bottom" && $pb_vertical === "bottom") {
				$pb_vertical = $pb_old_position;
			}

			// take care about fall back on old height if still exists
			$pb_size = $s->get_param(['general', 'progressbar', 'size'], '5px');
			$pb_old_height = $s->get_param(['general', 'progressbar', 'height'], 5);

			if ($pb_old_height !== "5px" && $pb_size === "5px") {
				$pb_size = $pb_old_height;
			}

			$pb_style = $s->get_param(['general', 'progressbar', 'style'], 'horizontal');
			$pb_radius = $s->get_param(['general', 'progressbar', 'radius'], 10);
			$pb_xof = $s->get_param(['general', 'progressbar', 'x'], '0px');
			$pb_yof = $s->get_param(['general', 'progressbar', 'y'], '0px');

			$pb_alignby = $s->get_param(['general', 'progressbar', 'alignby'], 'slider');

			if ($pb_basedon !== "slide") {
				$pb['basedon'] = $pb_basedon;
			}

			if ($pb_alignby !== "slider") {
				$pb['alignby'] = $pb_alignby;
			}

			if ($pb_bgcolor !== "transparent") {
				$pb["bgcolor"] = $pb_bgcolor;
			}

			if ($pb_color !== "rgba(255,255,255,0.5)") {
				$pb["color"] = $pb_color;
			}

			if ($pb_basedon === "plugin") {

				if ($pb_gaps !== 0 && $pb_gaps !== 0) {
					$pb['gapsize'] = $pb_gaps;
				}

				if ($pb_gapcolor !== "rgba(255,255,255,0.5)") {
					$pb['gapcolor'] = $pb_gapcolor;
				}
			...
			if ($pb_style !== "horizontal") {
				$pb['style'] = $pb_style;
			}

			if ($pb_horizontal !== "left") {
				$pb['horizontal'] = $pb_horizontal;
			}

			if ($pb_vertical !== "bottom") {
				$pb['vertical'] = $pb_vertical;
			}
			...
			if (($pb_style == "ccw" || $pb_style == "cw") && $pb_radius !== 10) {
				$pb['radius'] = $pb_radius;
			}
	...
	public function js_get_viewport() {
		...
		$evp = $s->get_param(['general', 'slideshow', 'viewPort'], false);

		if ($evp === false) {
			return $html;
		}

		$vps = $s->get_param(['general', 'slideshow', 'viewPortStart'], 'wait');
		$psh = $s->get_param(['general', 'slideshow', 'presetSliderHeight'], false);
		$vpa = $s->get_param(['general', 'slideshow', 'viewPortArea'], 200);

		if ($this->adv_resp_sizes == true) {
			$vpa = $this->normalize_device_settings($vpa, $this->enabled_sizes, 'html-array', [200]);
		} else {

			if (is_array($vpa) || is_object($vpa)) {
				$vpa = $this->get_biggest_device_setting($vpa, $this->enabled_sizes);
			}

			//vpa was before only on one level, so it can be a string or integer in the past
		}
		...
		if (!in_array($vpa, [200, '200', '200px'], true)) {
			$vp['visible_area'] = $vpa;
		}
	...
	public function js_get_scrolleffect() {
		...
		$ge = $s->get_param(['scrolleffects', 'set'], false);

		if ($ge === false) {
			return $html;
		}

		$fa = $s->get_param(['scrolleffects', 'setFade'], false);
		$bl = $s->get_param(['scrolleffects', 'setBlur'], false);
		$sgs = $s->get_param(['scrolleffects', 'setGrayScale'], false);
		$mb = $s->get_param(['scrolleffects', 'maxBlur'], 10);
		$ol = $s->get_param(['scrolleffects', 'layers'], false);
		$bg = $s->get_param(['scrolleffects', 'bg'], false);
		$d = $s->get_param(['scrolleffects', 'direction'], 'both');
		$mp = $s->get_param(['scrolleffects', 'multiplicator'], '1.35'); //was 1.3
		$mpl = $s->get_param(['scrolleffects', 'multiplicatorLayers'], '0.5'); //was 1.3
		$ti = $s->get_param(['scrolleffects', 'tilt'], '30');
		$dom = $s->get_param(['scrolleffects', 'disableOnMobile'], false);
		...
		if (!in_array($mb, [10, '10', '10px'], true)) {
			$se['maxblur'] = $mb;
		}
		...
		if (!in_array($mp, [1.35, '1.35'], true)) {
			$se['multiplicator'] = $mp;
		}

		if (!in_array($mpl, [0.5, '0.5'], true)) {
			$se['multiplicator_layers'] = $mpl;
		}

		if (!in_array($ti, [30, '30'], true)) {
			$se['tilt'] = $ti;
		}

		if ($dom !== false) {
			$se['disable_onmobile'] = $dom;
		}
	...
	public function js_get_scrollbased_timeline() {
		...
		$fa = $s->get_param(['scrolltimeline', 'set'], false);

		if ($fa === false) {
			return $html;
		}

		$ol = $s->get_param(['scrolltimeline', 'layers'], false);
		$ea = $s->get_param(['scrolltimeline', 'ease'], 'none');
		$sp = $s->get_param(['scrolltimeline', 'speed'], 500);

		$sfix = $s->get_param(['scrolltimeline', 'fixed'], false);
		$sfixs = $s->get_param(['scrolltimeline', 'fixedStart'], 0);
		$sfixe = $s->get_param(['scrolltimeline', 'fixedEnd'], 0);
	...
	public function js_get_parallax() {
		...
		if ($s->get_param(['parallax', 'set'], false) === false) {
			return $html;
		}

		$sd = $s->get_param(['parallax', 'setDDD'], false);
		$pt = ($sd === true) ? '3D' : $s->get_param(['parallax', 'mouse', 'type'], 'off');
		$pl = [];

		for ($i = 0; $i <= 15; $i++) {
			$pl[] = intval($s->get_param(['parallax', 'levels', $i], ($i + 1) * 5));
		}

		$pl = implode(',', $pl);
		$or = ($sd === true) ? 'slidercenter' : $s->get_param(['parallax', 'mouse', 'origo'], 'enterpoint');
		$sp = $s->get_param(['parallax', 'mouse', 'speed'], 400);
		$dpm = $s->get_param(['parallax', 'disableOnMobile'], false);
		$bgs = $s->get_param(['parallax', 'mouse', 'bgSpeed'], 0);
		$ls = $s->get_param(['parallax', 'mouse', 'layersSpeed'], 0);
		...
		if (!in_array($sp, [400, '400', '400ms'], true)) {
			$p['speed'] = $sp;
		}

		if ($dpm !== false) {
			$p['disable_onmobile'] = $dpm;
		}

		if ($pt === '3D') {
			$sh = $s->get_param(['parallax', 'ddd', 'shadow'], false);
			$bgf = $s->get_param(['parallax', 'ddd', 'BGFreeze'], false);
			$of = $s->get_param(['parallax', 'ddd', 'overflow'], false);
			$lof = $s->get_param(['parallax', 'ddd', 'layerOverflow'], false);
			$zc = $s->get_param(['parallax', 'ddd', 'zCorrection'], 400);
			...
			if ($of !== false) {
				$p['ddd_overflow'] = ($of === false) ? 'visible' : 'hidden';
			}

			if ($lof !== false) {
				$p['ddd_layer_overflow'] = $lof;
			}

			if (!in_array($zc, [400, '400', '400px'], true)) {
				$p['ddd_z_correction'] = $zc;
			}

		}

		if (!in_array($bgs, [0, '0', '0ms'], true)) {
			$p['speedbg'] = $bgs;
		}

		if (!in_array($ls, [0, '0', '0ms'], true)) {
			$p['speedls'] = $ls;
		}
	...
	public function js_get_modal() {
		...
		$cover = $s->get_param(['modal', 'cover'], true);
		$bodyclass = $s->get_param(['modal', 'bodyclass'], '');
		$color = $s->get_param(['modal', 'coverColor'], 'rgba(0,0,0,0.5)');
		$h = $s->get_param(['modal', 'horizontal'], 'center');
		$v = $s->get_param(['modal', 'vertical'], 'middle');

		$c['useAsModal'] = true;
		$c['alias'] = RevLoader::esc_attr($this->slider->get_alias());

		if ($bodyclass !== '') {
			$c['bodyclass'] = $bodyclass;
		}
	...
	public function js_get_carousel() {
		...
		$ease = $s->get_param(['carousel', 'ease'], 'power3.inOut');
		$speed = $s->get_param(['carousel', 'speed'], 800);
		$sal = $s->get_param(['carousel', 'showAllLayers'], false);
		$ha = $s->get_param(['carousel', 'horizontal'], 'center');
		$va = $s->get_param(['carousel', 'vertical'], 'center');
		$in = $s->get_param(['carousel', 'infinity'], false);
		$jus = $s->get_param(['carousel', 'justify'], false);
		$jusmw = $s->get_param(['carousel', 'justifyMaxWidth'], false);

		$snap = $s->get_param(['carousel', 'snap'], true);
		$sp = $s->get_param(['carousel', 'space'], 0);
		$mvi = $s->get_param(['carousel', 'maxItems'], 3);
		$st = $s->get_param(['carousel', 'stretch'], false);
		$fo = $s->get_param(['carousel', 'fadeOut'], true);
		$cr = $s->get_param(['carousel', 'rotation'], false);
		$cs = $s->get_param(['carousel', 'scale'], false);
		$br = $s->get_param(['carousel', 'borderRadius'], 0);
		$pt = $s->get_param(['carousel', 'paddingTop'], 0);
		$pb = $s->get_param(['carousel', 'paddingBottom'], 0);
		$csd = $s->get_param(['carousel', 'scaleDown'], 50);
		$csd = ($csd > 100) ? 100 : $csd;

		if ($ease !== 'power3.inOut') {
			$c['easing'] = $ease;
		}

		if (!in_array($speed, [800, '800', '800ms'], true)) {
			$c['speed'] = $speed;
		}

		if (!in_array($sal, ['false', false], true)) {
			$c['showLayersAllTime'] = $sal;
		}

		if ($ha !== 'center') {
			$c['horizontal_align'] = $ha;
		}
		...
		if (!in_array($sp, [0, '0', '0px'], true)) {
			$c['space'] = $sp;
		}

		if (!in_array($mvi, [3, '3'], true)) {
			$c['maxVisibleItems'] = $mvi;
		}
		...
		if ($cr === true) {
			$mr = $s->get_param(['carousel', 'maxRotation'], 0);
			$vr = $s->get_param(['carousel', 'varyRotate'], false);

			if (!in_array($mr, [0, '0', '0deg'], true)) {
				$c['maxRotation'] = $mr;
			}
		...
		if ($cs === true) {
			$vs = $s->get_param(['carousel', 'varyScale'], false);
			$os = $s->get_param(['carousel', 'offsetScale'], false);
			$c['minScale'] = $csd;
		...
		if ($fo === true) {
			$vf = $s->get_param(['carousel', 'varyFade'], false);

			if ($vf !== false) {
				$c['vary_fade'] = $vf;
			}

			$mo = $s->get_param(['carousel', 'maxOpacity'], 100);
			$mo = ($mo > 100) ? 100 : $mo;

			if (!in_array($mo, [100, '100'], true)) {
				$c['maxOpacity'] = $mo;
			}

		}

		if (!in_array($br, [0, '0', '0px'], true)) {
			$c['border_radius'] = $br;
		}

		if (!in_array($pt, [0, '0', '0px'], true)) {
			$c['padding_top'] = $pt;
		}

		if (!in_array($pb, [0, '0', '0px'], true)) {
			$c['padding_bottom'] = $pb;
		}
	...
	public function js_get_root() {
		...
		$hsal = str_replace('px', '', $s->get_param(['visibility', 'hideSliderUnderLimit'], 0));
		$hlal = str_replace('px', '', $s->get_param(['visibility', 'hideSelectedLayersUnderLimit'], 0));
		$halul = str_replace('px', '', $s->get_param(['visibility', 'hideAllLayersUnderLimit'], 0));
		...
		$start_delay = $s->get_param(['general', 'slideshow', 'initDelay'], '0');
		$start_delay = RevLoader::apply_filters('revslider_add_js_delay', $start_delay);
		$spinner = $s->get_param(['layout', 'spinner', 'type'], '0');
		$spinner = (in_array($spinner, [-1, '-1'], true)) ? 'off' : $spinner;
		...
		$keys = [
			...
			'dottedOverlay'       => [
				'v' => $s->get_param(['layout', 'bg', 'dottedOverlay'], 'none'),
				'd' => 'none',
			],
			'duration'            => [
				'v' => $s->get_param(['def', 'delay'], '9000'),
				'd' => [9000, '9000', '9000ms'],
			],
			'visibilityLevels'    => [
			...
			'minHeight'           => [
				'v' => ($l_type !== 'fullscreen') ? $s->get_param(['size', 'minHeight'], 0) : $s->get_param(['size', 'minHeightFullScreen'], 0),
				'd' => [0, '0', '0px'],
			],
			'autoHeight'          => [
				'v' => $s->get_param(['size', 'respectAspectRatio'], false),
				'd' => false,
			],
			'enableUpscaling'     => [
				'v' => $s->get_param(['size', 'enableUpscaling'], false),
				'd' => false,
			],
			'hideSliderAtLimit'   => [
				'v' => $hsal,
			...
			'lazyType'            => [
				'v' => $s->get_param(['general', 'lazyLoad'], 'none'),
				'd' => 'none',
			],
			'spinner'             => [
				'v' => 'spinner' . $spinner,
			...
			'fixedOnTop'          => [
				'v' => $s->get_param(['layout', 'position', 'fixedOnTop'], false),
				'd' => false,
			],
			'forceOverflow'       => [
				'v' => $s->get_param(['size', 'forceOverflow'], false),
				'd' => false,
			],
			'overflowHidden'      => [
				'v' => $s->get_param(['size', 'overflowHidden'], false),
				'd' => false,
			]
			, 'useFullScreenHeight' => [
				'v' => $s->get_param(['size', 'useFullScreenHeight'], true),
				'd' => true,
			],
			'maxHeight'           => [
				'v' => $s->get_param(['size', 'maxHeight'], 'none'),
				'd' => ['', 0, '0', 'none'],
			],
			'perspective'         => [
				'v' => $s->get_param(['general', 'perspective'], '600px'),
				'd' => '600px',
			],
			'perspectiveType'     => [
				'v' => $s->get_param(['general', 'perspectiveType'], 'local'),
				'd' => ['none'],
			],
			'keepBPHeight'        => [
				'v' => $s->get_param(['size', 'keepBPHeight'], false),
				'd' => false,
			],
		];
		...
		if ($spinner !== 'off' && (int) ($spinner) > 5) {
			$keys['spinnerclr'] = [
				'v' => $s->get_param(['layout', 'spinner', 'color'], '#ffffff'),
				'd' => '#ffffff',
			];
		}
		...
		if (!in_array($imgcrossOrigin, ['', 'unset'])) {
			$keys['imgCrossOrigin'] = ['v' => $imgcrossOrigin, 'd' => 'unset'];
		}

		$lazyloaddata = $this->get_val($global, 'lazyloaddata', '');

		if ($lazyloaddata !== '') {
			$keys['lazyloaddata'] = ['v' => $lazyloaddata, 'd' => ''];
		}

		$cache_size = $this->slider->get_param(['size', 'editorCache'], false);

		if ($cache_size !== false) {
			$keys['editorheight'] = ['v' => implode(',', (array) $cache_size), 'd' => ''];
		}

		if ($this->get_val($csizes, 'level', '') !== '') {
			$keys['responsiveLevels'] = ['v' => $csizes['level'], 'd' => ''];
		}

		if ($l_type == 'fullscreen') {
			$keys['disableForceFullWidth'] = ['v' => $s->get_param(['size', 'disableForceFullWidth'], false), 'd' => false];
			$keys['ignoreHeightChange'] = ['v' => $s->get_param(['size', 'ignoreHeightChanges'], true), 'd' => true];
			$keys['gridEQPlugin'] = ['v' => $s->get_param(['size', 'gridEQPlugin'], false), 'd' => false];
			$keys['fullScreenOffsetContainer'] = ['v' => $s->get_param(['size', 'fullScreenOffsetContainer'], ''), 'd' => ''];
			$keys['fullScreenOffset'] = ['v' => $s->get_param(['size', 'fullScreenOffset'], ''), 'd' => ''];
		}
		...
		if ($s_type !== 'hero') {
			$stopSlider = $s->get_param(['general', 'slideshow', 'stopSlider'], false);
			$loopSingle = $s->get_param(['general', 'slideshow', 'loopSingle'], true);
			$stopAtSlide = $s->get_param(['general', 'slideshow', 'stopAtSlide'], -1);
			$stopAfterLoops = $s->get_param(['general', 'slideshow', 'stopAfterLoops'], 0);
			$slideShow = $s->get_param(['general', 'slideshow', 'slideShow'], true);
			...
			$keys['stopAtSlide'] = ['v' => $stopAtSlide, 'd' => [-1, '-1']];
			$keys['stopAfterLoops'] = ['v' => $stopAfterLoops, 'd' => [-1, '-1']];
			$keys['shuffle'] = ['v' => $s->get_param(['general', 'slideshow', 'shuffle'], false), 'd' => false];
		}

		$parallax = $s->get_param(['parallax', 'set'], false);
		$parallax_type = $s->get_param(['parallax', 'mouse', 'type'], 'mouse');

		if ($s->get_param(['parallax', 'setDDD'], false) == true) {
			$parallax_type = '3D';
		}

		if ($parallax != true || ($parallax == true && $parallax_type != '3D')) {
			$keys['shadow'] = ['v' => $s->get_param(['layout', 'bg', 'shadow'], 0), 'd' => [0, '0']];
		}

		if ($s_type !== 'hero') {
			$keys['stopLoop'] = ['v' => $s->get_param(['general', 'slideshow', 'stopSlider'], false), 'd' => false];

			if ($s->get_param(['general', 'firstSlide', 'alternativeFirstSlideSet'], false) === true) {
				$keys['startWithSlide'] = ['v' => $s->get_param(['general', 'firstSlide', 'alternativeFirstSlide'], 1), 'd' => '9999'];
			}

		}

		$keys['waitForInit'] = ['v' => $s->get_param(['general', 'slideshow', 'waitForInit'], false), 'd' => false];

		if ($this->frontend_action) {
			$keys['ajaxUrl'] = ['v' => RevLoader::admin_url(), 'd' => ''];
			$keys['ajaxNonce'] = ['v' => RevLoader::wp_create_nonce(), 'd' => ''];
		}
	...
	public function get_navigation_css() {
		...
		$navs = ['arrows', 'bullets', 'tabs', 'thumbs'];
		$_all_navs = array_merge($navs, ['swipe', 'keyboard', 'mouse']);
		$found = false;

		foreach ($_all_navs as $nav) {

			if ($s->get_param(['nav', $nav, 'set'], false) === true) {
				$found = true;
				break;
			}
		...
		foreach ($navs as $n) {
			...
			if ($s->get_param(['nav', $n, 'set'], true) === true) {
				$nar = $s->get_param(['nav', $n, 'style'], 'round');
	...
	public function js_get_navigation() {
		...
		$msn = $s->get_param(['nav', 'mouse', 'set'], false);
		$tod = $s->get_param(['nav', 'swipe', 'setOnDesktop'], false);

		if ($msn === 'on' || $msn === 'carousel' || $tod === true) {
			$found = true;
		}
		...
		if ($lot === 'hero' || $found === false) {
			...
			$ohs = $s->get_param(['general', 'slideshow', 'stopOnHover'], true);
			...
			//KEYBOARD
			$kbn = $s->get_param(['nav', 'keyboard', 'set'], false);
			$kbd = $s->get_param(['nav', 'keyboard', 'direction'], 'horizontal');

			$msr = $s->get_param(['nav', 'mouse', 'reverse'], false);
			$ohs = $s->get_param(['general', 'slideshow', 'stopOnHover'], true);

			if ($kbn === true) {
				$h['keyboardNavigation'] = true;
			}

			if ($kbd !== 'horizontal') {
				$h['keyboard_direction'] = $kbd;
			}

			if ($msn !== 'off') {
				$h['mouseScrollNavigation'] = $msn;
			}

			if ($msr !== 'default') {
				$h['mouseScrollReverse'] = $msr;
			}
			...
			//TOUCH
			$ctom = $s->get_param(['nav', 'swipe', 'setMobileCarousel'], true);
			$ctod = $s->get_param(['nav', 'swipe', 'setDesktopCarousel'], true);
			$te = ($tod === true) ? true : $s->get_param(['nav', 'swipe', 'set'], false);

			if ($te === true || ($lot === 'carousel' && ($ctod === false || $ctom === false))) {
				$sth = intval($s->get_param(['nav', 'swipe', 'velocity'], 75));
				$smt = intval($s->get_param(['nav', 'swipe', 'minTouch'], '1'));
				$sd = $s->get_param(['nav', 'swipe', 'direction'], 'horizontal');
				$dbv = $s->get_param(['nav', 'swipe', 'blockDragVertical'], false);

				$h['touch'] = [];
				$h['touch']['touchenabled'] = $te;

				if ($tod === true) {
					$h['touch']['touchOnDesktop'] = true;
				}

				if ($sth !== 75) {
					$h['touch']['swipe_threshold'] = $sth;
				}

				if ($smt !== 1) {
					$h['touch']['swipe_min_touches'] = $smt;
				}

				if ($sd !== 'horizontal') {
					$h['touch']['swipe_direction'] = $sd;
				}

				if ($dbv !== false) {
					$h['touch']['drag_block_vertical'] = $dbv;
				}

				if ($lot === 'carousel' && $ctod === false) {
					$h['touch']['desktopCarousel'] = false;
				}

				if ($lot === 'carousel' && $ctom === false) {
					$h['touch']['mobileCarousel'] = false;
				}
			...
			foreach ($navs as $n) {
				$as = $s->get_param(['nav', $n, 'set'], false);
				...
				if ($as === true) {
					...
					$tmp = preg_replace("/\r|\n/", "", $tmp);
					$tmp = str_replace('"', '\\"', $tmp);
					$hom = $s->get_param(['nav', $n, 'hideUnder'], false);
					$ho = $s->get_param(['nav', $n, 'hideOver'], false);
					$ao = $s->get_param(['nav', $n, 'alwaysOn'], true);
					$anim = $s->get_param(['nav', $n, 'anim'], 'fade');
					$aspeed = $s->get_param(['nav', $n, 'animSpeed'], '1000ms');
					$adelay = $s->get_param(['nav', $n, 'animDelay'], '1000ms');
					$rtl = $s->get_param(['nav', $n, 'rtl'], false);

					if ($tmp !== $defaults[$n]['tmp']) {
						$h[$n]['tmp'] = $tmp;
					}
					...
					if ($hom === true) {
						$h[$n]['hide_onmobile'] = $hom;
					}

					if ($hom === true) {
						$hu = $s->get_param(['nav', $n, 'hideUnderLimit'], 0);

						if (!in_array($hu, [0, '0', '0px'], true)) {
							$h[$n]['hide_under'] = $hu;
						}
					...
					if ($ho === true) {
						$hol = $s->get_param(['nav', $n, 'hideOverLimit'], 9999);

						if (!in_array($hol, [9999, '9999', '9999px'], true)) {
							$h[$n]['hide_over'] = $hol;
						}
					...
					if ($ao === false) {
						$h[$n]['hide_onleave'] = true;
					}

					if ($ao === false) {
						$hd = $s->get_param(['nav', $n, 'hideDelay'], 200);
						$hdm = $s->get_param(['nav', $n, 'hideDelayMobile'], 1200);

						if (!in_array($hd, [200, '200', '200px'], true)) {
							$h[$n]['hide_delay'] = $hd;
						}

						if (!in_array($hdm, [1200, '1200', '1200px'], true)) {
							$h[$n]['hide_delay_mobile'] = $hdm;
						}
					...
					if ($aspeed !== '1000ms') {
						$h[$n]['animSpeed'] = $aspeed;
					}

					if ($adelay !== '1000ms') {
						$h[$n]['animDelay'] = $adelay;
					}

					//left only at arrows

					if ($n === 'arrows') {
						$alc = (in_array($s->get_param(['nav', $n, 'left', 'align'], 'slider'), ['layergrid', 'grid'], true)) ? 'layergrid' : 'slider';
						$alha = $s->get_param(['nav', $n, 'left', 'horizontal'], 'left');
						$alva = $s->get_param(['nav', $n, 'left', 'vertical'], 'center');
						$alho = $s->get_param(['nav', $n, 'left', 'offsetX'], 20);
						$alvo = $s->get_param(['nav', $n, 'left', 'offsetY'], 0);
						$anil = $s->get_param(['nav', $n, 'left', 'anim'], 'fade');
						$arc = (in_array($s->get_param(['nav', $n, 'right', 'align'], 'slider'), ['layergrid', 'grid'], true)) ? 'layergrid' : 'slider';
						$arha = $s->get_param(['nav', $n, 'right', 'horizontal'], 'right');
						$arva = $s->get_param(['nav', $n, 'right', 'vertical'], 'center');
						$arho = $s->get_param(['nav', $n, 'right', 'offsetX'], 20);
						$arvo = $s->get_param(['nav', $n, 'right', 'offsetY'], 0);
						$anir = $s->get_param(['nav', $n, 'right', 'anim'], 'fade');

						$h[$n]['left'] = []; //left only at arrows
						$h[$n]['right'] = []; //right only at arrows
						...
						if ($alc !== 'slider') {
							$h[$n]['left']['container'] = $alc;
						}

						if ($alha !== 'left') {
							$h[$n]['left']['h_align'] = $alha;
						}

						if ($alva !== 'center') {
							$h[$n]['left']['v_align'] = $alva;
						}

						if (!in_array($alho, [20, '20', '20px'], true)) {
							$h[$n]['left']['h_offset'] = intval(str_replace('px', '', $alho));
						}

						if (!in_array($alvo, [0, '0', '0px'], true)) {
							$h[$n]['left']['v_offset'] = intval(str_replace('px', '', $alvo));
						}

						if ($arc !== 'slider') {
							$h[$n]['right']['container'] = $arc;
						}

						if ($arha !== 'right') {
							$h[$n]['right']['h_align'] = $arha;
						}

						if ($arva !== 'center') {
							$h[$n]['right']['v_align'] = $arva;
						}

						if (!in_array($arho, [20, '20', '20px'], true)) {
							$h[$n]['right']['h_offset'] = intval(str_replace('px', '', $arho));
						}

						if (!in_array($arvo, [0, '0', '0px'], true)) {
							$h[$n]['right']['v_offset'] = intval(str_replace('px', '', $arvo));
						}

					} else {
						//these are not in left/right, but directly added
						$arha = $s->get_param(['nav', $n, 'horizontal'], 'center');
						$arva = $s->get_param(['nav', $n, 'vertical'], 'bottom');
						$arho = $s->get_param(['nav', $n, 'offsetX'], 20);
						$arvo = $s->get_param(['nav', $n, 'offsetY'], 0);
						$dir = $s->get_param(['nav', $n, 'direction'], 'horizontal');
						$space = $s->get_param(['nav', $n, 'space'], $defaults[$n]['space']);
						...
						if (!in_array($arho, [0, '0', '0px'], true)) {
							$h[$n]['h_offset'] = intval(str_replace('px', '', $arho));
						}

						if (!in_array($arvo, [20, '20', '20px'], true)) {
							$h[$n]['v_offset'] = intval(str_replace('px', '', $arvo));
						}
						...
						//these exist not in arrows at all

						if (!in_array($space, [$defaults[$n]['space'], (string) $defaults[$n]['space'], $defaults[$n]['space'] . 'px'], true)) {
							$h[$n]['space'] = $space;
						}

						//only exist in thumbs and tabs

						if (in_array($n, ['thumbs', 'tabs'], true)) {
							$width = $s->get_param(['nav', $n, 'width'], 100);
							$height = $s->get_param(['nav', $n, 'height'], 50);
							$mw = $s->get_param(['nav', $n, 'widthMin'], 100);
							$wp = $s->get_param(['nav', $n, 'padding'], $defaults[$n]['wrapper_padding']);
							$wc = $s->get_param(['nav', $n, 'wrapperColor'], 'transparent');
							$va = $s->get_param(['nav', $n, 'amount'], 5);
							$span = $s->get_param(['nav', $n, 'spanWrapper'], false);
							$pos = $s->get_param(['nav', $n, 'innerOuter'], 'inner');

							if (!in_array($width, [100, '100', '100px'], true)) {
								$h[$n]['width'] = $width;
							}

							if (!in_array($height, [50, '50', '50px'], true)) {
								$h[$n]['height'] = $height;
							}

							if (!in_array($mw, [100, '100', '100px'], true)) {
								$h[$n]['min_width'] = $mw;
							}

							if (!in_array($wp, [$defaults[$n]['wrapper_padding'], (string) $defaults[$n]['wrapper_padding'], $defaults[$n]['wrapper_padding'] . 'px'], true)) {
								$h[$n]['wrapper_padding'] = $wp;
							}

							if (strtolower($wc) !== '#transparent') {
								$h[$n]['wrapper_color'] = $wc;
							}

							if (!in_array($va, [5, '5'], true)) {
								$h[$n]['visibleAmount'] = $va;
							}
							...
							if ($pos === 'inner') {
								$arc = (in_array($s->get_param(['nav', $n, 'align'], 'slider'), ['layergrid', 'grid'], true)) ? 'layergrid' : 'slider';
							...
							//only write in bullets like this
							$arc = (in_array($s->get_param(['nav', $n, 'align'], 'slider'), ['layergrid', 'grid'], true)) ? 'layergrid' : 'slider';
		...
		if (!empty($h)) {
			...
			foreach ($h as $key => $value) {
				...
				//change thumb to thumbnail here
				$html .= RS_T9 . $key . ':';
	...
	public function layer_frame_triggered($uid, $frame) {
		...
		if ($this->is_static) {
			//we have to push all layers of all slides
			$layers = [];
			$slides = $this->get_current_slides();
			$static_slide = $this->get_static_slide();
		...
		foreach ($layers as $layer) {
			...
			foreach ($actions as $action) {
				...
				if ($act === 'start_frame' && $frame == $this->get_val($action, 'gotoframe', '')) {
					$ret = true;
				} else
				if ($act === 'start_in' && $frame === 'frame_1') {
					$ret = true;
				} else
				if ($act === 'start_out' && $frame === 'frame_999') {
					$ret = true;
				} else
				if ($act === 'toggle_layer' && in_array($frame, ['frame_1', 'frame_999'])) {
					$ret = true;
				} else
				if ($act === 'toggle_frames' && ($frame == $this->get_val($action, 'gotoframeM', '') || $frame == $this->get_val($action, 'gotoframeN', ''))) {
					$ret = true;
				}
	...
	public function get_responsive_size($slider) {
		...
		$csn = $slider->slider->get_param(['size', 'custom', 'n'], false);
		$cst = $slider->slider->get_param(['size', 'custom', 't'], false);
		$csi = $slider->slider->get_param(['size', 'custom', 'm'], false);

		$w = $slider->slider->get_param(['size', 'width', 'd'], 1240);
		$h = $slider->slider->get_param(['size', 'height', 'd'], 1240);
		$r = $this->get_val($global, ['size', 'desktop'], 1240);
		$c = $this->slider->get_param(['size', 'editorCache', 'd'], false);

		if ($csn == true || $cst == true || $csi == true) {
			$d = $w;
			$w .= ',';
			$w .= ($csn == true) ? $slider->slider->get_param(['size', 'width', 'n'], 1024) : $d;
			$d = ($csn == true) ? $slider->slider->get_param(['size', 'width', 'n'], 1024) : $d;
			$w .= ',';
			$w .= ($cst == true) ? $slider->slider->get_param(['size', 'width', 't'], 778) : $d;
			$d = ($cst == true) ? $slider->slider->get_param(['size', 'width', 't'], 778) : $d;
			$w .= ',';
			$w .= ($csi == true) ? $slider->slider->get_param(['size', 'width', 'm'], 480) : $d;
			$d = ($csi == true) ? $slider->slider->get_param(['size', 'width', 'm'], 480) : $d;

			$d = $h;
			$h .= ',';
			$h .= ($csn == true) ? $slider->slider->get_param(['size', 'height', 'n'], 1024) : $d;
			$d = ($csn == true) ? $slider->slider->get_param(['size', 'height', 'n'], 1024) : $d;
			$h .= ',';
			$h .= ($cst == true) ? $slider->slider->get_param(['size', 'height', 't'], 778) : $d;
			$d = ($cst == true) ? $slider->slider->get_param(['size', 'height', 't'], 778) : $d;
			$h .= ',';
			$h .= ($csi == true) ? $slider->slider->get_param(['size', 'height', 'm'], 480) : $d;
			$d = ($csi == true) ? $slider->slider->get_param(['size', 'height', 'm'], 480) : $d;

			$d = $r;
			$r .= ',';
			$r .= ($csn == true) ? $this->get_val($global, ['size', 'notebook'], 1024) : $d;
			$d = ($csn == true) ? $this->get_val($global, ['size', 'notebook'], 1024) : $d;
			$r .= ',';
			$r .= ($cst == true) ? $this->get_val($global, ['size', 'tablet'], 778) : $d;
			$d = ($cst == true) ? $this->get_val($global, ['size', 'tablet'], 778) : $d;
			$r .= ',';
			$r .= ($csi == true) ? $this->get_val($global, ['size', 'mobile'], 480) : $d;
			$d = ($csi == true) ? $this->get_val($global, ['size', 'mobile'], 480) : $d;

			if ($c !== false) {
				$d = $c;
				$c .= ',';
				$c .= ($csn == true) ? $slider->slider->get_param(['size', 'editorCache', 'n'], 1024) : $d;
				$d = ($csn == true) ? $slider->slider->get_param(['size', 'editorCache', 'n'], 1024) : $d;
				$c .= ',';
				$c .= ($cst == true) ? $slider->slider->get_param(['size', 'editorCache', 't'], 778) : $d;
				$d = ($cst == true) ? $slider->slider->get_param(['size', 'editorCache', 't'], 778) : $d;
				$c .= ',';
				$c .= ($csi == true) ? $slider->slider->get_param(['size', 'editorCache', 'm'], 480) : $d;
				$d = ($csi == true) ? $slider->slider->get_param(['size', 'editorCache', 'm'], 480) : $d;
			}

		} else {
			$r .= ',';
			$r .= $this->get_val($global, ['size', 'notebook'], 1024);
			$r .= ',';
			$r .= $this->get_val($global, ['size', 'tablet'], 778);
			$r .= ',';
			$r .= $this->get_val($global, ['size', 'mobile'], 480);
		}
	...
	public function write_js_var($v, $pp = '"') {
		...
		return (is_numeric($v) || substr($v, 0, 1) === '[' || in_array($v, ['true', 'false'])) ? $v : $pp . $v . $pp;
	}
