...
class RevSliderPluginUpdate extends RevSliderFunction {
	...
	public $css_navigations = []; //holds css navigations
	public $googlefonts = []; //holds googlefonts
	public $upd_animations = []; //holds animations
	...
	public function __construct() {
		...
		if (empty($this->googlefonts)) {
			...
			include DIGITAL_CORE_DIR . '/vendor/googlefonts.php';
	...
	public function upgrade_next_slider() {
		...
		if (!empty($sliders)) {
			...
			foreach ($sliders as $slider) {
				...
				if (version_compare($this->get_val($slider, ['settings', 'version']), $this->revision, '<')) {
	...
	public function upgrade_slider_to_6_1_4($sliders = false) {
		...
		if (!empty($sliders) && is_array($sliders)) {
			...
			foreach ($sliders as $slider) {
				...
				if ($scale_down !== false) {
					$carousel['scaleDown'] = 100 - (int) ($scale_down);
					$slider->update_params(['carousel' => $carousel]);
				}

				$slider->update_settings(['version' => '6.1.4']);
			}
	...
	public function upgrade_slider_to_6_1_6($sliders = false) {
		...
		if (!empty($sliders) && is_array($sliders)) {
			...
			foreach ($sliders as $slider) {
				...
				$slider->update_settings(['version' => '6.1.6']);
			}
	...
	public function upgrade_slider_to_6_2_0($sliders = false) {
		...
		if (!empty($sliders) && is_array($sliders)) {
			...
			foreach ($sliders as $slider) {
				...
				$slider->update_settings(['version' => '6.2.0']);
			}
	...
	public function update_css_styles() {
		...
		foreach ($styles as $key => $attr) {
			$ret = $wpdb->update($wpdb->prefix . RevSliderFront::TABLE_CSS, ['settings' => $styles[$key]['settings'], 'params' => $styles[$key]['params'], 'hover' => $styles[$key]['hover'], 'advanced' => $styles[$key]['advanced']], ['id' => $attr['id']]);
		}
	...
	public function move_template_slider() {
		...
		$used = []; //will store all template IDs that are used by post based Sliders, these can be deleted after the progress.
		$sr = new RevSliderSlider();
		$sl = new RevSliderSlide();
						...
						if ($t_slider->get_id() === $template_id) {
							...
							if (!empty($slides) && is_array($slides)) {
								...
								foreach ($slides as $slide) {
									...
									$slider->copy_slide_to_slider(['slider_id' => $slider_id, 'slide_id' => $slide_id]);
								}
		...
		if (!empty($temp_sliders) && is_array($temp_sliders)) {
			...
			foreach ($temp_sliders as $slider) {
				$slider->update_params(['template' => 'false', 'source_type' => 'posts']);
			}
	...
	public function add_animation_settings_to_layer($sliders = false) {
		...
		$sr = new RevSliderSlider();
		$sl = new RevSliderSlide();
		$sliders = ($sliders === false) ? $sr->get_sliders() : [$sliders]; //false == do it on all Sliders
							...
							foreach ($layers as $lk => $layer) {
								...
								if ($link_slide != false && $link_slide !== 'nothing') {
									...
									case 'link':
										...
										$layers[$lk]['layer_action']->action = ['a' => 'link'];
										$layers[$lk]['layer_action']->link_type = ['a' => 'a'];
										$layers[$lk]['layer_action']->image_link = ['a' => $link];
										$layers[$lk]['layer_action']->link_open_in = ['a' => $link_open_in];

										unset($layers[$lk]['link']);
										unset($layers[$lk]['link_open_in']);
									case 'next':
										$layers[$lk]['layer_action']->action = ['a' => 'next'];
										break;
									case 'prev':
										$layers[$lk]['layer_action']->action = ['a' => 'prev'];
										break;
									case 'scroll_under':
										$scrollunder_offset = $this->get_val($layer, 'scrollunder_offset');
										$layers[$lk]['layer_action']->action = ['a' => 'scroll_under'];
										$layers[$lk]['layer_action']->scrollunder_offset = ['a' => $scrollunder_offset];

										unset($layers[$lk]['scrollunder_offset']);
										break;
									default: //its an ID, so its a slide ID
										$layers[$lk]['layer_action']->action = ['a' => 'jumpto'];
										$layers[$lk]['layer_action']->jump_to_slide = ['a' => $link_slide];
										break;
									}

									$layers[$lk]['layer_action']->tooltip_event = ['a' => 'click'];
				...
				if (!empty($slides) && is_array($slides)) {
					...
					foreach ($slides as $slide) {
						...
						if (!empty($layers) && is_array($layers)) {
							...
							foreach ($layers as $lk => $layer) {
								$static_styles = (array) $this->get_val($layer, 'static_styles', []);
								$def_val = (array) $this->get_val($layer, 'deformation', []);
								$defh_val = (array) $this->get_val($layer, 'deformation-hover', []);
								...
								if (empty($def_val)) {
									...
									if (isset($wws['params']['padding'])) {
										...
										case 1:
											$raw_pad = [$raw_pad[0], $raw_pad[0], $raw_pad[0], $raw_pad[0]];
											break;
										case 2:
											$raw_pad = [$raw_pad[0], $raw_pad[1], $raw_pad[0], $raw_pad[1]];
											break;
										case 3:
											$raw_pad = [$raw_pad[0], $raw_pad[1], $raw_pad[2], $raw_pad[1]];
											break;
										}
									...
									if (isset($wws['params']['border-radius'])) {
										...
										case 1:
											$raw_bor = [$raw_bor[0], $raw_bor[0], $raw_bor[0], $raw_bor[0]];
											break;
										case 2:
											$raw_bor = [$raw_bor[0], $raw_bor[1], $raw_bor[0], $raw_bor[1]];
											break;
										case 3:
											$raw_bor = [$raw_bor[0], $raw_bor[1], $raw_bor[2], $raw_bor[1]];
											break;
										}
			...
			foreach ($sliders as $slider) {
				...
				if (!isset($settings['version']) || version_compare($settings['version'], 5.0, '<')) {
					...
					if (empty($google_fonts) && !empty($fonts)) {
						...
						$slider->update_params(['google_font' => $google_fonts]);
					}

					$settings['version'] = 5.0;
					$slider->update_settings(['version' => 5.0]);
				}

				if ($bg_freeze == 'on') {
					//deprecated here, moved to slides so remove check here and add on to slides
					$slider->update_params(['parallax_bg_freeze' => 'off']);
				}
		...
		if (!empty($sliders) && is_array($sliders)) {
			...
			foreach ($sliders as $slider) {
				...
				if (version_compare($slider->get_setting('version', '1.0.0'), '6.0.0', '<')) {
					...
					$ms['sourcetype'] = (in_array($slider->get_param('source_type'), ['post', 'posts', 'specific_posts', 'current_post'])) ? 'post' : $slider->get_param('source_type', 'gallery');
					$ms['sourcetype'] = ($ms['sourcetype'] == 'woocommerce') ? 'woo' : $ms['sourcetype'];

					//$ms['title']		= $slider->get_param('title');
					$ms['googleFont'] = $slider->get_param('google_font', []);

					$postSubType = (in_array($slider->get_param('source_type'), ['post', 'posts', 'specific_posts', 'current_post'])) ? $slider->get_param('source_type', false) : 'post';
					...
					if ($amountparams > 5) {
						...
						$ms['size'] = [
							...
							'gridEQPlugin'              => $this->_truefalse($slider->get_param('full_screen_align_force', false)),
							'custom'                    => [
								'd' => true,
							...
							'width'                     => [
								'd' => (int) ($slider->get_param('width', 1240)),
								'n' => (int) ($slider->get_param('width_notebook', 1024)),
								't' => (int) ($slider->get_param('width_tablet', 778)),
								'm' => (int) ($slider->get_param('width_mobile', 480)),
							],
							'height'                    => [
								'd' => (int) ($slider->get_param('height', 900)),
								'n' => (int) ($slider->get_param('height_notebook', 768)),
								't' => (int) ($slider->get_param('height_tablet', 960)),
								'm' => (int) ($slider->get_param('height_mobile', 720)),
							],
						...
						$ms['general'] = [
							'slideshow'             => [
								...
								'viewPortArea'       => (100 - (int) ($slider->get_param('viewport_area', 60))) . "%",
						...
						if ($ms['scrolleffects']['set'] === true && $ms['type'] === 'hero') {
							//existed only for hero Slider
							/**
				...
				$slider->update_settings(['version' => '6.0.0']);
			}
	...
	public function transform_preset_to_6_0_0($params, $end = 'slide') {
		...
		$types = ['arrows', 'bullets', 'tabs', 'thumbs'];
		$preset = ['arrows' => [], 'bullets' => [], 'tabs' => [], 'thumbs' => []];
		$repl = ['color-rgba', 'custom', 'color', 'font_family']; //old existing types
		...
		if (!empty($params)) {
			...
			foreach ($params as $k => $v) {
				...
				foreach ($types as $type) {
					...
					//check if we are -arrows- ect

					$f = false;
					$new_k = '';

					foreach ($repl as $r) {
						//check if we end with -slide or -def
						$l = strlen('-' . $r . '-' . $end) * -1;
					...
					if ($this->_truefalse($v) !== true) {
						continue;
					}

					//it is set to true, so search for the counterpart
	...
	public function migrate_slide_to_6_0($slide, $slider = false, $slide_nr = false) {
		...
		$streamboth = (in_array($slide->get_param('background_type'), ['streamyoutubeboth', 'streamvimeoboth', 'streaminstagramboth', 'streamtwitterboth'])) ? true : false;
		$streamonlyvideo = (in_array($slide->get_param('background_type'), ['streamtwitter', 'streamyoutube', 'streamvimeo', 'streaminstagram'])) ? true : false;
		$streamimage = ($slide->get_param('background_type') === 'image' && $slider->get_param('sourcetype') !== 'gallery') ? true : false; //was 'settings', 'sourcetype'
		$streamanyvideo = (in_array($slide->get_param('background_type'), ['streamyoutubeboth', 'streamvimeoboth', 'streaminstagramboth', 'streamtwitterboth', 'streamtwitter', 'streamyoutube', 'streamvimeo', 'streaminstagram'])) ? true : false;

		if (strpos($slide->get_param('background_type'), 'youtube') !== false) {
			$type = 'youtube';
		} else
		if (strpos($slide->get_param('background_type'), 'vimeo') !== false) {
			$type = 'vimeo';
		} else
		if (strpos($slide->get_param('background_type'), 'instagram') !== false) {
			$type = 'html5';
		} else
		if (strpos($slide->get_param('background_type'), 'twitter') !== false) {
			$type = 'html5';
		} else {
		...
		$bg_image = ($this->_truefalse($slide->get_param('thumb_for_admin')) === true) ? $this->get_val($ms, ['bg', 'image']) : $slide->get_param('slide_thumb', $this->get_val($ms, ['bg', 'image']));
		$bg_image = ($stream === true) ? '' : $bg_image;

		$ms['thumb'] = [
			'customThumbSrc'        => $bg_image,
			'customThumbSrcId'      => ($this->get_val($ms, ['thumb', 'customThumbSrc'], '') !== '') ? $this->get_image_id_by_url($this->get_val($ms, ['thumb', 'customThumbSrc'], '')) : '',
			'customAdminThumbSrc'   => ($this->_truefalse($slide->get_param('thumb_for_admin')) === true) ? $slide->get_param('slide_thumb') : '',
			'customAdminThumbSrcId' => ($this->get_val($ms, ['thumb', 'customAdminThumbSrc'], '') !== '') ? $this->get_image_id_by_url($this->get_val($ms, ['thumb', 'customAdminThumbSrc'], '')) : '',
			'dimension'             => $slide->get_param('thumb_dimension', 'orig'),
		];

		//only do if we are not a stream!

		if (!in_array($slider->get_param('sourcetype'), ['youtube', 'vimeo', 'instagram', 'twitter', 'facebook', 'flickr'], true)) {

			if ($this->get_val($ms, ['thumb', 'customThumbSrc'], '') == '') {

				if ($this->get_val($ms, ['thumb', 'customAdminThumbSrc'], '') !== '') {
					$this->set_val($ms, ['thumb', 'customThumbSrc'], $this->get_val($ms, ['thumb', 'customAdminThumbSrc'], ''));
					$this->set_val($ms, ['thumb', 'customThumbSrcId'], $this->get_val($ms, ['thumb', 'customAdminThumbSrcId'], ''));
				}
		...
		$slide_transition = $slide->get_param('slide_transition', ['fade']);
		$add_transition = $this->get_val($this->add_to_transition_speed, $slide_transition, false);

		$duration = $slide->get_param('transition_duration', [1000]);
			...
			if (!empty($duration)) {
				...
				if (is_array($duration)) {
					...
					foreach ($duration as $dk => $dv) {
						...
						$duration[$dk] = (!in_array($dv, ['random', 'default'], true)) ? $dv + $add_transition : $dv;
					}

				} else {
					$duration = str_replace('ms', '', $duration);
					$duration = (!in_array($duration, ['random', 'default'], true)) ? $duration + $add_transition : $duration;
				}
		...
		$ms['timeline'] = [
			...
			'slots'         => $slide->get_param('slot_amount', [0]),
			'duration'      => $duration,
			'easeIn'        => $slide->get_param('transition_ease_in', ['default']),
			'easeOut'       => $slide->get_param('transition_ease_out', ['default']),
			'rotation'      => $slide->get_param('transition_rotation', [0]),
		];
		...
		foreach ($t_keys as $tk) {
			...
			$tlc = $this->get_val($ms, ['timeline', $tk, 0], '');
		...
		if ($slider->get_param('slicey_globals', false) !== false) {
			...
			$ms['panzoom']['blurStart'] = $this->get_val($slicey, 'blurgstart', $this->get_val($ms, ['panzoom', 'blurStart'], 0));
			$ms['panzoom']['blurEnd'] = $this->get_val($slicey, 'blurgend', $this->get_val($ms, ['panzoom', 'blurEnd'], 0));
			$ms['panzoom']['fitEnd'] = $this->get_val($slicey, 'scale', $this->get_val($ms, ['panzoom', 'fitEnd']));
			$ms['panzoom']['duration'] = $this->get_val($slicey, 'time', $this->get_val($ms, ['panzoom', 'duration']));
			$ms['panzoom']['ease'] = $this->get_val($slicey, 'easing', $this->get_val($ms, ['panzoom', 'ease'], 'none'));
		}
							...
							if (!empty($new_layers)) {
								...
								foreach ($new_layers as $nlk => $nlv) {
									...
									if (in_array((string) $nluid, $this->slide_action_map, true)) {
										$a_o = $this->get_val($nlv, ['actions', 'animationoverwrite'], 'default');

										if ($a_o === 'wait') {
											$this->set_val($new_layers, [$nlk, 'timeline', 'frames', 'frame_1', 'timeline', 'actionTriggered'], true);
										}

										if (in_array($a_o, ['wait', 'waitout'], true)) {
											$this->set_val($new_layers, [$nlk, 'timeline', 'frames', 'frame_999', 'timeline', 'actionTriggered'], true);
										}
	 ...
	 * @since: 6.0.0
	 **/
	public function migrate_layer_to_6_0($layer, $slide, $slider, $blank = false) {

		$color_picker = new RSColorpicker();

		$video_data = $this->get_val($layer, 'video_data', []);
		$deformation = (array) $this->get_val($layer, 'deformation', []);
		$deformation_hover = (array) $this->get_val($layer, 'deformation-hover', []);
		$static_styles = (array) $this->get_val($layer, 'static_styles', []);
		$layer_action = $this->get_val($layer, 'layer_action', []);
		$svg = $this->get_val($layer, 'svg', []);
		$frames = $this->get_val($layer, 'frames', []);
		$frame_0 = $this->get_val($frames, 'frame_0', []);
		$frame_999 = $this->get_val($frames, 'frame_999', []);
		...
		if (strpos($ml['text'], '<i class="pe-7s') !== false) {
			$ml['text'] = str_replace(['margin-top:-6px;', 'margin-top: -6px;'], '', $ml['text']);
		}

		if (strpos($toggleText, '<i class="pe-7s') !== false) {
			$toggleText = str_replace(['margin-top:-6px;', 'margin-top: -6px;'], '', $toggleText);
		}
		...
		case 'video':
			...
			$ml['media'] = [
				...
				'thumbs'           => [
					'veryBig' => $this->get_val($video_data, 'thumb_very_big', ['width' => 640, 'height' => 480, 'url' => $posterUrl]),
					'big'     => $this->get_val($video_data, 'thumb_big', ['width' => 640, 'height' => 480, 'url' => $posterUrl]),
					'large'   => $this->get_val($video_data, 'thumb_large', ['width' => 640, 'height' => 360, 'url' => $posterUrl]),
					'medium'  => $this->get_val($video_data, 'thumb_medium', ['width' => 320, 'height' => 240, 'url' => $posterUrl]),
					'small'   => $this->get_val($video_data, 'thumb_small', ['width' => 200, 'height' => 150, 'url' => $posterUrl]),
				],
		...
		case 'image':
			...
			if ($this->get_val($layer, 'scaleX', false) !== false) {
				...
				if ($get_dim === true) {
					$ow = '';
					$oh = '';
					$cur_img = $this->get_val($ml, ['media', 'imageUrl']);
					...
					if ($cur_img !== '') {
						...
						if ($img_data !== false && !empty($img_data)) {
							$this->set_val($ml, ['media', 'imageId'], $cur_id);
		...
		$defwidth = (!in_array($this->get_val($layer, 'type'), ['image', 'shape', 'video'])) ? 'auto' : '300';
		$defheight = (!in_array($this->get_val($layer, 'type'), ['image', 'shape', 'video'])) ? 'auto' : '180';
		$defproportion = (in_array($this->get_val($layer, 'type'), ['svg', 'image', 'video'])) ? true : false;
		$defaspectrat = (!in_array($this->get_val($layer, 'type'), ['image', 'shape', 'video'])) ? 'none' : 300 / 180;
		...
		$ml['size'] = [
			'width'             => $this->c_to_resp(['default' => $defwidth, 'val' => $ww]),
			'height'            => $this->c_to_resp(['default' => $defheight, 'val' => $hh]),
			'maxWidth'          => $this->c_to_resp(['default' => 'none', 'val' => 'none']),
			'maxHeight'         => $this->c_to_resp(['default' => 'none', 'val' => 'none']),
			'minWidth'          => $this->c_to_resp(['default' => 'none', 'val' => 'none']),
			'minHeight'         => $this->c_to_resp(['default' => 'none', 'val' => $minHH]),
		...
		$ml['size']['aspectRatio'] = ((int) ($ml['size']['originalWidth']) > 0 && (int) ($ml['size']['originalHeight']) > 0) ? $this->c_to_resp(['default' => $defaspectrat, 'val' => (int) ($ml['size']['originalWidth']) / (int) ($ml['size']['originalHeight'])]) : $this->c_to_resp(['default' => $defaspectrat, 'val' => $defaspectrat]);
		...
		if (!in_array($ml['size']['scaleProportional'], [true, false], true)) {

			if (in_array($this->get_val($layer, 'type'), ['svg', 'image', 'video'], true)) {
				$ml['size']['scaleProportional'] = true;
			} else {
		...
		if ($this->get_val($ml, ['size', 'originalWidth'], 0) === 0) {
			$ml['size']['originalWidth'] = $this->get_val($ml, ['size', 'width', 'd', 'v']);
		}

		if ($this->get_val($ml, ['size', 'originalHeight'], 0) === 0) {
			$ml['size']['originalHeight'] = $this->get_val($ml, ['size', 'height', 'd', 'v']);
		}

		$ml['position'] = [
			'x'          => $this->c_to_resp(['default' => 0, 'val' => $this->get_val($layer, 'left', 0), 'unit' => 'px'], true),
			'y'          => $this->c_to_resp(['default' => 0, 'val' => $this->get_val($layer, 'top', 0), 'unit' => 'px'], true),
			'horizontal' => $this->c_to_resp(['default' => 'left', 'val' => $this->get_val($layer, 'align_hor')]),
			'vertical'   => $this->c_to_resp(['default' => 'top', 'val' => $this->get_val($layer, 'align_vert')]),
		...
		if ($this->get_val($layer, 'p_uid', -1) == -1) {
			//only on layers that are not in row/group/column

			if (in_array($this->get_val($layer, 'type'), ['image', 'shape', 'text'], true)) {
				$mmw = $this->get_val($ml, ['size', 'width']);

				if ($this->get_val($mmw, ['d', 'v']) === '100%' ||
					$this->get_val($mmw, ['n', 'v']) === '100%' ||
					$this->get_val($mmw, ['t', 'v']) === '100%' ||
					$this->get_val($mmw, ['m', 'v']) === '100%'
				) {

					if ($this->get_val($ml, ['size', 'covermode']) === 'custom') {
						$base_align = 'slide';
					}
		...
		$ml['group'] = [
			...
			'columnSize'    => (in_array($column_size, [1, '1'], true)) ? '1/1' : $column_size,
		];

		$align_vert = $this->get_val($layer, 'align_vert', []);
		...
		*/
		...
		if ($version === false || (int) ($version) < 530) {
			//an additional check that we may not need, as checking if frame_999 is empty is already enough
			...
			'frames'      => [
				'frame_0'   => $this->default_frame(
					[
						...
						'x'                    => (!in_array($split, ['', 'none'])) ? 0 : $this->conv_perc_vals($this->get_val($layer, 'x_start')),
						'y'                    => (!in_array($split, ['', 'none'])) ? 0 : $this->conv_perc_vals($this->get_val($layer, 'y_start')),
						'z'                    => (!in_array($split, ['', 'none'])) ? 0 : $this->get_val($fr_0, 'z'),
						'scaleX'               => (!in_array($split, ['', 'none'])) ? 1 : $this->get_val($fr_0, 'scaleX'),
						'scaleY'               => (!in_array($split, ['', 'none'])) ? 1 : $this->get_val($fr_0, 'scaleY'),
						'opacity'              => (!in_array($split, ['', 'none'])) ? 1 : $this->get_val($fr_0, 'opacity'),
						'rotationX'            => (!in_array($split, ['', 'none'])) ? 0 : $this->get_val($fr_0, 'rotationX'),
						'rotationY'            => (!in_array($split, ['', 'none'])) ? 0 : $this->get_val($fr_0, 'rotationY'),
						'rotationZ'            => (!in_array($split, ['', 'none'])) ? 0 : $this->get_val($fr_0, 'rotationZ'),
						'skewX'                => (!in_array($split, ['', 'none'])) ? 0 : $this->get_val($fr_0, 'skewX'),
						'skewY'                => (!in_array($split, ['', 'none'])) ? 0 : $this->get_val($fr_0, 'skewY'),
				...
				'frame_999' => $this->default_frame(
					[
						...
						'x'                    => (!in_array($endsplit, ['', 'none'])) ? 0 : $this->conv_perc_vals($this->get_val($layer, 'x_end')),
						'y'                    => (!in_array($endsplit, ['', 'none'])) ? 0 : $this->conv_perc_vals($this->get_val($layer, 'y_end')),
						'z'                    => (!in_array($endsplit, ['', 'none'])) ? 0 : $this->conv_perc_vals($this->get_val($layer, 'z_end')),
						'scaleX'               => (!in_array($endsplit, ['', 'none'])) ? 1 : $this->conv_perc_vals($this->get_val($layer, 'scale_x_end')),
						'scaleY'               => (!in_array($endsplit, ['', 'none'])) ? 1 : $this->conv_perc_vals($this->get_val($layer, 'scale_y_end')),
						'opacity'              => (!in_array($endsplit, ['', 'none'])) ? 1 : $this->conv_perc_vals($this->get_val($layer, 'opacity_end')),
						'rotationX'            => (!in_array($endsplit, ['', 'none'])) ? 0 : $this->conv_perc_vals($this->get_val($layer, 'x_rotate_end')),
						'rotationY'            => (!in_array($endsplit, ['', 'none'])) ? 0 : $this->conv_perc_vals($this->get_val($layer, 'y_rotate_end')),
						'rotationZ'            => (!in_array($endsplit, ['', 'none'])) ? 0 : $this->conv_perc_vals($this->get_val($layer, 'z_rotate_end')),
						'skewX'                => (!in_array($endsplit, ['', 'none'])) ? 0 : $this->conv_perc_vals($this->get_val($layer, 'skew_x_end')),
						'skewY'                => (!in_array($endsplit, ['', 'none'])) ? 0 : $this->conv_perc_vals($this->get_val($layer, 'skew_y_end')),
		...
		*/
		...
		if ($blank === false) {
			//only do this if we are not creating a blank layer
		...
		if ($this->get_val($deformation, 'color-transparency', false) !== false) {
			$static_color = $this->get_val($static_styles, 'color', []);

			if (!empty($static_color)) {

				foreach ($static_color as $i => $s_color) {
					$this->set_val($static_styles, ['color', $i], $color_picker->convert($s_color, $this->get_val($deformation, 'color-transparency') * 100));
				}
		...
		$ml['idle'] = [
			...
			'color'              => $this->c_to_resp(['default' => '#ffffff', 'val' => $this->get_val($static_styles, 'color', '#ffffff')]),
			'margin'             => $this->c_to_resp(['default' => [0, 0, 0, 0], 'val' => $this->get_val($layer, 'margin')]),
			'padding'            => $this->c_to_resp(['debug' => true, 'default' => [0, 0, 0, 0], 'val' => $this->get_val($layer, 'padding')]),
			'marginLock'         => false,
			'paddingLock'        => false,
			...
			'fontStyle'          => $fs,
			'fontSize'           => $this->c_to_resp(['default' => '20', 'val' => $this->get_val($static_styles, 'font-size')]),
			'fontWeight'         => $this->c_to_resp(['default' => '400', 'val' => $this->get_val($static_styles, 'font-weight')]),
			'letterSpacing'      => $this->c_to_resp(['default' => '0', 'val' => $this->get_val($static_styles, 'letter-spacing')]),
			'lineHeight'         => $this->c_to_resp(['default' => '25', 'val' => $this->get_val($static_styles, 'line-height')]),
			'overflow'           => $this->get_val($deformation, 'overflow', 'visible'),
			'textAlign'          => $this->c_to_resp(['default' => 'left', 'val' => $this->get_val($layer, 'text-align', $this->get_val($deformation, 'text-align'))]),
			...
			'borderRadius'       => $this->c_to_v_and_u(['default' => [0, 0, 0, 0], 'val' => $this->get_val($deformation, 'border-radius', [0, 0, 0, 0]), 'u' => '%']),
			'borderStyle'        => $this->c_to_resp(['default' => 'none', 'val' => $this->get_val($deformation, 'border-style', 'none')]),
			...
			'whiteSpace'         => $this->c_to_resp(['default' => 'nowrap', 'val' => $this->get_val($layer, 'whitespace')]),
			'boxShadow'          => [
				'inuse'     => false,
				'container' => 'content',
				'hoffset'   => $this->c_to_resp(['default' => 0, 'val' => 0]),
				'voffset'   => $this->c_to_resp(['default' => 0, 'val' => 0]),
				'blur'      => $this->c_to_resp(['default' => 0, 'val' => 0]),
				'spread'    => $this->c_to_resp(['default' => 0, 'val' => 0]),
				'color'     => 'rgba(0,0,0,0)',
			],
			'textShadow'         => [
				'inuse'   => false,
		 ...
		 * 		-> If no, reset to 400
		 **/
		$_ff = str_replace(['"', '"'], '', $ml['idle']['fontFamily']);
		...
		if (isset($this->googlefonts[$_ff])) {
			...
			if (is_array($_fw)) {
				...
				foreach ($_fw as $device => $d_val) {
					...
					$fw = $this->get_val($this->googlefonts, [$_ff, 'variants'], []);
					$fw_found = false;
					...
					if ($fw_found === false) {
						...
						if (!empty($fw)) {
							...
							if ((int) ($_d_v) == $ml['idle']['fontWeight'][$device]['v']) {
		...
		if ($colorht !== false) {
			$colorch = $color_picker->convert($colorch, $colorht);
		}
		...
		$ml['hover'] = [
			...
			'borderRadius'         => $this->c_to_v_and_u(['default' => [0, 0, 0, 0], 'val' => $this->get_val($deformation_hover, 'border-radius', [0, 0, 0, 0]), 'u' => '%']),
		...
		if (!empty($layer_action)) {
			...
			$new_actions = $this->get_val($ml, ['actions', 'action'], []);

			if (!empty($new_actions)) {

				foreach ($new_actions as $a_k => $n_a) {

					if (in_array($this->get_val($n_a, 'action'), ['start_in', 'start_out', 'toggle_layer'], true)) {
						$this->slide_action_map[] = ($this->static_slide) ? 'static-' . $this->get_val($n_a, 'layer_target') : $this->get_val($n_a, 'layer_target');
					}

					//group row column

					if (in_array($this->get_val($ml, 'type', 'text'), ['row', 'group', 'column'], true)) {
		...
		$ml['runtime'] = $this->get_val($layer, 'runtime', [
		...
		$inline = $this->get_val($layer, 'inline', []);
		...
		$_style = $this->get_val($ml, ['idle', 'style'], '');

		if ($_style !== '') {
			$css = $this->get_css_navigations();
			$_adv = $this->get_val($css, '.tp-caption.' . $_style, []);

			if (!empty($_adv)) {
				$_idle = $this->get_val($_adv, ['advanced', 'idle'], []);
				$_hover = $this->get_val($_adv, ['advanced', 'hover'], []);
				...
				if ($this->get_val($static_styles, 'letter-spacing') === '') {
					$ls = $this->get_val($_adv, ['params', 'letter-spacing']);

					if ($ls !== '') {
						$ml['idle']['letterSpacing'] = $this->c_to_resp(['default' => '0', 'val' => $ls]);
					}
		...
		if (!empty($exploding)) {
			...
			if ($enabled_out === 'auto') {
				$enabled_out = $enabled_in === 'explodinglayers' ? 'explodinglayers' : false;
			}
	...
	public function remove_unneeded_slider_settings($sliders) {
		...
		if (!empty($sliders) && is_array($sliders)) {
			...
			foreach ($sliders as $slider) {
				...
				if ($this->get_val($params, ['addOns', 'revslider-particles-addon', 'enable'], false) !== false) {
					$this->set_val($params, ['addOns', 'revslider-particles-addon'], []);
					$this->set_val($params, ['addOns', 'revslider-particles-addon', 'enable'], true);
					$update = true;
				}
	...
	private function write_exploding_layer($exploding, $tpe) {
		...
		$color = $this->get_val($exploding, 'color_' . $tpe, ['#000000']);
		$density = $this->get_val($exploding, 'density_' . $tpe, ['1']);
		$direction = $this->get_val($exploding, 'direction_' . $tpe, ['left']);
		$padding = $this->get_val($exploding, 'padding_' . $tpe, ['150']);
		$power = $this->get_val($exploding, 'power_' . $tpe, ['2']);
		$randomsize = $this->get_val($exploding, 'randomsize_' . $tpe, [false]);
		$randomspeed = $this->get_val($exploding, 'randomspeed_' . $tpe, [false]);
		$size = $this->get_val($exploding, 'size_' . $tpe, ['5']);
		$speed = $this->get_val($exploding, 'speed_' . $tpe, ['1']);
		$style = $this->get_val($exploding, 'style_' . $tpe, ['fill']);
		$sync = $this->get_val($exploding, 'sync_' . $tpe, [false]);
		$type = $this->get_val($exploding, 'type_' . $tpe, ['circle']);
		...
		if (function_exists('is_plugin_active')) {
			...
			if (is_plugin_active('revslider-404-addon/revslider-404-addon.php')) {
				...
				if ($settings !== false) {
					...
					if (isset($options['revslider-404-addon-active']) && (int) ($options['revslider-404-addon-active'])) {
			...
			if (is_plugin_active('revslider-maintenance-addon/revslider-maintenance-addon.php')) {
				...
				if ($settings !== false) {
					...
					if (isset($options['revslider-maintenance-addon-active']) && (int) ($options['revslider-maintenance-addon-active'])) {
	...
	public function change_global_settings_to_6_2_0() {
		...
		if (is_array($global)) {
			//means we are not json, so we are on 5.x
		...
		if (!empty($anims)) {
			...
			foreach ($anims as $_) {
				...
				if ($_json_anim !== $json_anim) {
					...
					$result = $wpdb->update($wpdb->prefix . RevSliderFront::TABLE_LAYER_ANIMATIONS, $arr, ['id' => $id]);
				}
	...
	public function change_animations_settings_to_6_0($anims = false) {
		...
		if (!empty($anims)) {
			...
			foreach ($anims as $_) {
				...
				if ($this->get_val($_, 'settings') === 'in' || $this->get_val($_, 'settings') === 'out') {
					//ignore, as already converted to 6.0
					continue;
				} else {
					...
					$gid = ($this->get_val($_, ['params', 'type']) == 'customin') ? 'in' : 'out';
					$fr = ($gid === 'in') ? 'frame_0' : 'frame_999';
					$tr = ['name' => $this->get_val($_, 'handle')];
					$tr[$fr] = ['transform' => [], 'timeline' => []];

					if ($gid === 'in') {
						$tr['frame_1'] = ['timeline' => ['speed' => $this->get_val($_, ['params', 'speed']), 'ease' => $this->get_val($_, ['params', 'easing'])]];
					} else {
						$tr['frame_999']['timeline'] = ['speed' => $this->get_val($_, 'params', 'speed'), 'ease' => $this->get_val($_, ['params', 'easing'])];
					}

					// CHECK IF ANIMATION HAS MASK

					if ($this->_truefalse($this->get_val($_, ['params', 'mask'])) == true) {
						$tr[$fr]['mask'] = ['use' => true, 'x' => $this->get_val($_, ['params', 'mask_x']), 'y' => $this->get_val($_, ['params', 'mask_y'])];

						if ($gid === 'in') {
							$tr['frame_1']['mask'] = ['use' => true, 'x' => 0, 'y' => 0];
						}

					}

					$inside = $tr[$fr]['transform'];
					$splithelp = ['use' => true, 'delay' => $this->get_val($_, ['params', 'splitdelay'])];

					// SET TARGET ANIMATION TO SPLIT OR LAYER

					switch ($this->get_val($_, ['params', 'split'])) {
					case 'lines':
					case 'line':
					...
					case 'char':
						...
						if ($gid === 'in') {
							$tr['frame_1']['chars'] = $splithelp;
						}

						break;
					}

					$opacity = $this->get_val($_, ['params', 'captionopacity'], '######');

					if ($opacity !== '######') {
						$inside['opacity'] = $opacity;
					}
					...
					if (!empty($_['params'])) {
						...
						foreach ($_['params'] as $key => $val) {
							...
							if (in_array($val, ['inherit', '0', 0, '0px'])) {
								continue;
							}
					...
					$result = $wpdb->update($wpdb->prefix . RevSliderFront::TABLE_LAYER_ANIMATIONS, $arr, ['id' => $_['id']]);
				}
	...
	public function change_navigation_slider_to_6_0($sliders = false) {
		...
		$navs = ['arrows' => 'navigation_arrow_style', 'bullets' => 'navigation_bullets_style', 'tabs' => 'tabs_style', 'thumbs' => 'thumbnails_style'];
		...
		if (!empty($placeholders)) {
			...
			foreach ($placeholders as $placeholder) {
				...
				if ($this->get_val($placeholder, 'nav-type') === $t) {
					$n['settings']['placeholders']->{$this->get_val($placeholder, 'handle')}

					= [
						'title' => $this->get_val($placeholder, 'title'),
						'type'  => $this->get_val($placeholder, 'type'),
						'data'  => ($this->get_val($placeholder, 'type') === 'font-family') ? $this->get_val($placeholder, ['data', 'font_family']) : $this->get_val($placeholder, ['data', $this->get_val($placeholder, 'type')]),
					];
				}
	...
	public function migrate_slider_AddOn($_) {
		...
		if ($_->get_param('wb_enable', false) !== false) {
			$obj['revslider-whiteboard-addon'] = [
		...
		if ($_->get_param('beforeafter_enabled', false) !== false) {
			$obj['revslider-beforeafter-addon'] = [
		...
		if ($_->get_param('revealer_enabled', false) !== false) {
			...
			$obj['revslider-revealer-addon']['delay'] = (int) ($obj['revslider-revealer-addon']['delay']);
			...
			$obj['revslider-revealer-addon']['overlay']['delay'] = (int) ($obj['revslider-revealer-addon']['overlay']['delay']);
			...
			$obj['revslider-revealer-addon']['duration'] = (int) ($obj['revslider-revealer-addon']['duration']);
			...
			$obj['revslider-revealer-addon']['overlay']['duration'] = (int) ($obj['revslider-revealer-addon']['overlay']['duration']);
		...
		if ($_->get_param('bubblemorph_enabled', false) !== false) {
			$obj['revslider-bubblemorph-addon'] = ['enable' => $this->_truefalse($_->get_param('bubblemorph_enabled'))];
		}

		//DUALTONE MIGRATION (NO DEFAULTS NEEDED, LEAVE IT OUT !!)

		if ($_->get_param('duotonefilters_enabled', false) !== false) {
			$obj['revslider-duotonefilters-addon'] = [
		...
		if ($_->get_param('particles_enabled', false) !== false) {
			...
			*/
			...
			if ($partShape === 'star') {
				...
				$partSize = max(round((int) ($partSize) * 0.5), 1);
			...
			if ($partOpacityRandom && $partShape === 'edge' || $partShape === 'triangle' || $partShape === 'polygon' || $partShape === 'star') {
				$partOpacity = min((int) ($partOpacity) + 25, 100);
				$partOpacityMin = min((int) ($partOpacityMin) + 25, 100);
			}

			$obj['revslider-particles-addon'] = [
	...
	public function migrate_slide_AddOn($_, $_s, $slide_nr = false) {
		...
		if ($_->get_param('filmstrip_enabled', false) !== false) {
			...
			$obj['revslider-filmstrip-addon'] = [
				...
				'times'     => $this->c_to_resp(['default' => 40, 'val' => ['desktop' => $this->get_val($ftimes, 0), 'notebook' => $this->get_val($ftimes, 1), 'tablet' => $this->get_val($ftimes, 2), 'mobile' => $this->get_val($ftimes, 3)]]),
		...
		if ($_->get_param('paintbrush_enabled', false) !== false) {
			$obj['revslider-paintbrush-addon'] = [
			...
			$obj['revslider-paintbrush-addon']['image']['blur']['amount'] = (int) ($obj['revslider-paintbrush-addon']['image']['blur']['amount']);
			...
			$obj['revslider-paintbrush-addon']['image']['blur']['fixedges']['amount'] = (int) ($obj['revslider-paintbrush-addon']['image']['blur']['fixedges']['amount']);
			...
			$obj['revslider-paintbrush-addon']['brush']['size'] = (int) ($obj['revslider-paintbrush-addon']['brush']['size']);
			...
			$obj['revslider-paintbrush-addon']['brush']['disappear']['time'] = (int) ($obj['revslider-paintbrush-addon']['brush']['disappear']['time']);
		...
		if ($_->get_param('liquideffect_enabled', false) !== false) {
			$obj['revslider-liquideffect-addon'] = [
		...
		if ($_->get_param('duotonefilter_addon', false) !== false) {
			$obj['revslider-duotonefilters-addon'] = ['filter' => $_->get_param('duotonefilter_addon', 'rs-duotone-none')];
		}

		// BEFOREAFTER

		if ($_->get_param('beforeafter_enabled', false) !== false) {
			$globals = json_decode(str_replace('\\', '', $_->get_param('beforeafter_globals', ['moveto' => '30%|30%|30%|30%'])), true);
			...
			$obj['revslider-beforeafter-addon'] = [
				...
				'moveTo'     => $this->c_to_resp(['default' => 50, 'val' => ['desktop' => $this->get_val($movetos, 0), 'notebook' => $this->get_val($movetos, 1), 'tablet' => $this->get_val($movetos, 2), 'mobile' => $this->get_val($movetos, 3)]]),
				'teaser'     => [
		...
		if ($this->_truefalse($_s->get_param(['addOns', 'revslider-particles-addon', 'enable'], false)) !== false) {
			$p_start_at = $_s->get_param(['addOns', 'revslider-particles-addon', 'startSlide'], 'first');
			$p_end_at = $_s->get_param(['addOns', 'revslider-particles-addon', 'endSlide'], 'last');
			$add_particles = false;

			if ($p_start_at === 'first' && $p_end_at === 'last') {
				$add_particles = true;
			} else
			if ($p_start_at === 'first' && (int) ($p_end_at) >= $slide_nr) {
				$add_particles = true;
			} else
			if ($p_end_at === 'last' && (int) ($p_start_at) <= $slide_nr) {
				$add_particles = true;
			} else
			if ((int) ($p_start_at) <= $slide_nr && (int) ($p_end_at) >= $slide_nr) {
				$add_particles = true;
			}

			if ($add_particles === true) {
				$obj['revslider-particles-addon'] = [
					'enable'        => $this->_truefalse($_s->get_param(['addOns', 'revslider-particles-addon', 'enable'], false)),
					'hideOnMobile'  => $this->_truefalse($_s->get_param(['addOns', 'revslider-particles-addon', 'hideOnMobile'], false)),
					'particles'     => [
						'shape'   => $_s->get_param(['addOns', 'revslider-particles-addon', 'particles', 'shape'], 'circle'),
						'number'  => $_s->get_param(['addOns', 'revslider-particles-addon', 'particles', 'number'], 80),
						'size'    => $_s->get_param(['addOns', 'revslider-particles-addon', 'particles', 'size'], 6),
						'sizeMin' => $_s->get_param(['addOns', 'revslider-particles-addon', 'particles', 'sizeMin'], 1),
						'random'  => $this->_truefalse($_s->get_param(['addOns', 'revslider-particles-addon', 'particles', 'random'], true)),
					],
					'styles'        => [
						'border'   => [
							'enable'  => $this->_truefalse($_s->get_param(['addOns', 'revslider-particles-addon', 'styles', 'border', 'enable'], false)),
							'color'   => $_s->get_param(['addOns', 'revslider-particles-addon', 'styles', 'border', 'color'], '#ffffff'),
							'opacity' => $_s->get_param(['addOns', 'revslider-particles-addon', 'styles', 'border', 'opacity'], 100),
							'size'    => $_s->get_param(['addOns', 'revslider-particles-addon', 'styles', 'border', 'size'], 1),
						],
						'lines'    => [
							'enable'   => $this->_truefalse($_s->get_param(['addOns', 'revslider-particles-addon', 'styles', 'lines', 'enable'], false)),
							'color'    => $_s->get_param(['addOns', 'revslider-particles-addon', 'styles', 'lines', 'color'], '#ffffff'),
							'width'    => $_s->get_param(['addOns', 'revslider-particles-addon', 'styles', 'lines', 'width'], 1),
							'opacity'  => $_s->get_param(['addOns', 'revslider-particles-addon', 'styles', 'lines', 'opacity'], 100),
							'distance' => $_s->get_param(['addOns', 'revslider-particles-addon', 'styles', 'lines', 'distance'], 150),
						],
						'particle' => [
							'color'         => $_s->get_param(['addOns', 'revslider-particles-addon', 'styles', 'particle', 'color'], '#ffffff'),
							'opacity'       => $_s->get_param(['addOns', 'revslider-particles-addon', 'styles', 'particle', 'opacity'], 100),
							'opacityMin'    => $_s->get_param(['addOns', 'revslider-particles-addon', 'styles', 'particle', 'opacityMin'], 25),
							'opacityRandom' => $this->_truefalse($_s->get_param(['addOns', 'revslider-particles-addon', 'styles', 'particle', 'opacityRandom'], false)),
							'zIndex'        => $_s->get_param(['addOns', 'revslider-particles-addon', 'styles', 'particle', 'zIndex'], 'default'),
						],
					],
					'movement'      => [
						'enable'      => $this->_truefalse($_s->get_param(['addOns', 'revslider-particles-addon', 'movement', 'enable'], true)),
						'randomSpeed' => $this->_truefalse($_s->get_param(['addOns', 'revslider-particles-addon', 'movement', 'randomSpeed'], true)),
						'speed'       => $_s->get_param(['addOns', 'revslider-particles-addon', 'movement', 'speed'], 1),
						'speedMin'    => $_s->get_param(['addOns', 'revslider-particles-addon', 'movement', 'speedMin'], 1),
						'direction'   => $_s->get_param(['addOns', 'revslider-particles-addon', 'movement', 'direction'], 'none'),
						'straight'    => $this->_truefalse($_s->get_param(['addOns', 'revslider-particles-addon', 'movement', 'straight'], true)),
						'bounce'      => $this->_truefalse($_s->get_param(['addOns', 'revslider-particles-addon', 'movement', 'bounce'], false)),
					],
					'interactivity' => [
						'hoverMode' => ($this->_truefalse($_s->get_param(['addOns', 'revslider-particles-addon', 'interactivity', 'hoverMode']))) ? $_s->get_param(['addOns', 'revslider-particles-addon', 'interactivity', 'hoverMode'], 'repulse') : 'none',
						'clickMode' => ($this->_truefalse($_s->get_param(['addOns', 'revslider-particles-addon', 'interactivity', 'clickMode']))) ? $_s->get_param(['addOns', 'revslider-particles-addon', 'interactivity', 'clickMode'], 'repulse') : 'none',
					],
					'bubble'        => [
						'distance' => $_s->get_param(['addOns', 'revslider-particles-addon', 'bubble', 'distance'], 400),
						'size'     => $_s->get_param(['addOns', 'revslider-particles-addon', 'bubble', 'size'], 40),
						'opacity'  => $_s->get_param(['addOns', 'revslider-particles-addon', 'bubble', 'opacity'], 40),
					],
					'grab'          => [
						'distance' => $_s->get_param(['addOns', 'revslider-particles-addon', 'grab', 'distance'], 400),
						'opacity'  => $_s->get_param(['addOns', 'revslider-particles-addon', 'grab', 'opacity'], 50),
					],
					'repulse'       => [
						'distance' => $_s->get_param(['addOns', 'revslider-particles-addon', 'repulse', 'distance'], 200),
						'easing'   => 100, /* new option */
					],
					'pulse'         => [
						'size'    => [
							'enable' => $this->_truefalse($_s->get_param(['addOns', 'revslider-particles-addon', 'pulse', 'size', 'enable'], false)),
							'speed'  => $_s->get_param(['addOns', 'revslider-particles-addon', 'pulse', 'size', 'speed'], 40),
							'min'    => $_s->get_param(['addOns', 'revslider-particles-addon', 'pulse', 'size', 'min'], 1),
							'sync'   => $this->_truefalse($_s->get_param(['addOns', 'revslider-particles-addon', 'pulse', 'size', 'sync'], false)),
						],
						'opacity' => [
							'enable' => $this->_truefalse($_s->get_param(['addOns', 'revslider-particles-addon', 'pulse', 'opacity', 'enable'], false)),
							'speed'  => $_s->get_param(['addOns', 'revslider-particles-addon', 'pulse', 'opacity', 'speed'], 3),
							'min'    => $_s->get_param(['addOns', 'revslider-particles-addon', 'pulse', 'opacity', 'min'], 1),
							'sync'   => $this->_truefalse($_s->get_param(['addOns', 'revslider-particles-addon', 'pulse', 'opacity', 'sync'], false)),
						],
					],
				];
			}
	...
	public function migrate_layer_AddOn(&$_, $slide) {
		...
		if ($this->get_val($_, 'whiteboard', false) !== false) {
			...
			$wbenabled = $this->get_val($_, ['whiteboard', 'hand_function']);
			$obj['revslider-whiteboard-addon'] = [
				'enable' => $wbenabled !== 'off' ? true : false,
				'hand'   => [
					'angle'         => $this->get_val($_, ['whiteboard', 'hand_angle']),
					'angleRepeat'   => $this->get_val($_, ['whiteboard', 'hand_angle_repeat']),
					'direction'     => $this->get_val($_, ['whiteboard', 'hand_direction']),
					'rotation'      => $this->get_val($_, ['whiteboard', 'hand_full_rotation']),
					'rotationAngle' => $this->get_val($_, ['whiteboard', 'hand_full_rotation_angle']),
					'mode'          => $this->get_val($_, ['whiteboard', 'hand_function']),
					'gotoLayer'     => $this->get_val($_, ['whiteboard', 'hand_gotolayer']),
					'type'          => $this->get_val($_, ['whiteboard', 'hand_type']),
					'x'             => $this->get_val($_, ['whiteboard', 'hand_x_offset']),
					'y'             => $this->get_val($_, ['whiteboard', 'hand_y_offset']),
				],
				'jitter' => [
					'distance'           => $this->get_val($_, ['whiteboard', 'jitter_distance']),
					'distanceHorizontal' => $this->get_val($_, ['whiteboard', 'jitter_distance_horizontal']),
					'offset'             => $this->get_val($_, ['whiteboard', 'jitter_offset']),
					'offsetHorizontal'   => $this->get_val($_, ['whiteboard', 'jitter_offset_horizontal']),
					'repeat'             => $this->get_val($_, ['whiteboard', 'jitter_repeat']),
				],
			];
		}
		...
		if ($this->get_val($_, 'typewriter', false) !== false) {
			$obj['revslider-typewriter-addon'] = [
				'enable'          => $this->_truefalse($this->get_val($_, ['typewriter', 'enabled'])),
				'blinking_speed'  => $this->get_val($_, ['typewriter', 'blinking_speed']),
				'cursor_type'     => $this->get_val($_, ['typewriter', 'cursor_type']),
				'blinking'        => $this->_truefalse($this->get_val($_, ['typewriter', 'blinking'])),
				'delays'          => $this->get_val($_, ['typewriter', 'delays']),
				'deletion_delay'  => $this->get_val($_, ['typewriter', 'deletion_delay']),
				'deletion_speed'  => $this->get_val($_, ['typewriter', 'deletion_speed']),
				'hide_cursor'     => $this->_truefalse($this->get_val($_, ['typewriter', 'hide_cursor'])),
				'linebreak_delay' => $this->get_val($_, ['typewriter', 'linebreak_delay']),
				'lines'           => $this->get_val($_, ['typewriter', 'lines']),
				'looped'          => $this->_truefalse($this->get_val($_, ['typewriter', 'looped'])),
				'newline_delay'   => $this->get_val($_, ['typewriter', 'newline_delay']),
				'sequenced'       => $this->_truefalse($this->get_val($_, ['typewriter', 'sequenced'])),
				'speed'           => $this->get_val($_, ['typewriter', 'speed']),
				'start_delay'     => $this->get_val($_, ['typewriter', 'start_delay']),
				'word_delay'      => $this->get_val($_, ['typewriter', 'word_delay']),
			];
		}
		...
		if ($this->get_val($_, 'beforeafter', false) !== false) {
			$obj['revslider-beforeafter-addon'] = [
				'position' => $this->get_val($_, ['beforeafter', 'position'], 'before'),
			];
		}
		...
		if (strpos($layer_text, '{{weather_') !== false || strpos($layer_text, '%weather_') !== false) {
			...
			$params = $slide->get_params();
			$addons = $this->get_val($params, 'addOns', []);
			$weather = $this->get_val($addons, 'revslider-weather-addon', []);
			...
			if (strpos($layer_text, '%weather_') !== false) {
				$layer_text = preg_replace_callback(
					'/%weather.*?%/',
					[$this, 'preg_replace_callback_addon'],
					$layer_text
				);
		...
		if ($this->get_val($_, 'type') === 'shape' && $this->get_val($_, 'subtype') === 'slicey') {
			$obj['revslider-slicey-addon'] = [
				'scaleOffset' => $this->get_val($_, ['slicey', 'scale_offset'], 20),
				'blurStart'   => $this->get_val($_, ['slicey', 'blurlstart'], 'inherit'),
				'blurEnd'     => $this->get_val($_, ['slicey', 'blurlend'], 'inherit'),
			];
		}
		...
		if ($this->get_val($_, 'type') === 'shape' && $this->get_val($_, 'subtype') === 'bubblemorph') {
			...
			foreach ($bubblemorph as $prop => $bubbleVal) {
				...
				if (!is_array($bubbleVal)) {
					$bubbleVal = [$bubbleVal, $bubbleVal, $bubbleVal, $bubbleVal];
				}
				...
				$bubbleObj[$prop] = ['desktop' => $this->get_val($bubbleVal, 0), 'notebook' => $this->get_val($bubbleVal, 1), 'tablet' => $this->get_val($bubbleVal, 2), 'mobile' => $this->get_val($bubbleVal, 3)];

			}

			$obj['revslider-bubblemorph-addon'] = [
				'settings' => [
					'maxmorphs' => $this->c_to_resp(['default' => $bubbleDefaults['max'], 'val' => $this->get_val($bubbleObj, 'max')]),
					'speedx'    => $this->c_to_resp(['default' => $bubbleDefaults['speedx'], 'val' => $this->get_val($bubbleObj, 'speedx')]),
					'speedy'    => $this->c_to_resp(['default' => $bubbleDefaults['speedy'], 'val' => $this->get_val($bubbleObj, 'speedy')]),
					'bufferx'   => $this->c_to_resp(['default' => $bubbleDefaults['bufferx'], 'val' => $this->get_val($bubbleObj, 'bufferx')]),
					'buffery'   => $this->c_to_resp(['default' => $bubbleDefaults['buffery'], 'val' => $this->get_val($bubbleObj, 'buffery')]),
				],
				'shadow'   => [
					'strength' => $this->c_to_resp(['default' => $bubbleDefaults['blurstrength'], 'val' => $this->get_val($bubbleObj, 'blurstrength')]),
					'color'    => $this->c_to_resp(['default' => $bubbleDefaults['blurcolor'], 'val' => $this->get_val($bubbleObj, 'blurcolor')]),
					'offsetx'  => $this->c_to_resp(['default' => $bubbleDefaults['blurx'], 'val' => $this->get_val($bubbleObj, 'blurx')]),
					'offsety'  => $this->c_to_resp(['default' => $bubbleDefaults['blury'], 'val' => $this->get_val($bubbleObj, 'blury')]),
				],
				'border'   => [
					'size'  => $this->c_to_resp(['default' => $bubbleDefaults['bordersize'], 'val' => $this->get_val($bubbleObj, 'bordersize')]),
					'color' => $this->c_to_resp(['default' => $bubbleDefaults['bordercolor'], 'val' => $this->get_val($bubbleObj, 'bordercolor')]),
				],
			];
		}
	...
	*/
	public function c_to_resp($attr = ['default' => 0, 'unit' => '']) {
		...
		foreach ($this->_respsizes as $i => $rv) {
			...
			if (isset($val[$sold])) {
				...
				if (!is_object($val) && !is_array($val) && $val !== false) {
					/**
				...
				if ((!is_object($val) && !is_array($val)) || $this->get_val($val, $sold, false) === false) {
				...
				foreach ($v as $vi => $vval) {
					...
					if ($unit == '' && !is_numeric($newObj[$s]['v'][$vi])) {
						...
						if (strpos($newObj[$s]['v'][$vi], '%') !== false) {
							$newObj[$s]['v'][$vi] = (int) ($newObj[$s][$vi]) . '%';
						} else
						if (strpos($newObj[$s]['v'][$vi], 'px') !== false) {
							$newObj[$s]['v'][$vi] = (int) ($newObj[$s]['v'][$vi]) . 'px';
						}
				...
				if (is_array($newObj[$s]['v']) || is_object($newObj[$s]['v'])) {
						...
						if ($unit == '' && !is_numeric($nov) && $nov !== false && $nov !== true) {
							...
							if (strpos($nov, '%') !== false) {
								...
								if (is_object($newObj[$s]['v'])) {
									$newObj[$s]['v']->$nok = (int) ($nov) . '%';
								} else {
									$newObj[$s]['v'][$nok] = (int) ($nov) . '%';
								}

							} else
							if (strpos($nov, 'px') !== false) {

								if (is_object($newObj[$s]['v'])) {
									$newObj[$s]['v']->$nok = (int) ($nov) . 'px';
								} else {
									$newObj[$s]['v'][$nok] = (int) ($nov) . 'px';
								}
					...
					if ($unit == '' && !is_numeric($newObj[$s]['v']) && $newObj[$s]['v'] !== false && $newObj[$s]['v'] !== true) {
						...
						if (strpos($newObj[$s]['v'], '%') !== false) {
							$newObj[$s]['v'] = (int) ($newObj[$s]['v']) . '%';
						} else
						if (strpos($newObj[$s]['v'], 'px') !== false) {
							$newObj[$s]['v'] = (int) ($newObj[$s]['v']) . 'px';
						}
	 ...
	 **/
	public function default_frame($o = [], $overwrite_with = []) {

		$f = $this->get_val($o, 'fid', 'frame_0');

		$_base = [
			'grayscale'            => ['frame_0' => 0, 'frame_1' => 0, 'frame_999' => 'inherit'], //0
			'brightness'           => ['frame_0' => 100, 'frame_1' => 100, 'frame_999' => 'inherit'], //100
			'blur'                 => ['frame_0' => 0, 'frame_1' => 0, 'frame_999' => 'inherit'], //100
			'transformPerspective' => ['frame_0' => 600, 'frame_1' => 600, 'frame_999' => 600],
			//transform
			'x'                    => ['frame_0' => 0, 'frame_1' => 0, 'frame_999' => 'inherit'], //0
			'y'                    => ['frame_0' => 0, 'frame_1' => 0, 'frame_999' => 'inherit'], //0
			'z'                    => ['frame_0' => 0, 'frame_1' => 0, 'frame_999' => 'inherit'], //0
			'scaleX'               => ['frame_0' => 1, 'frame_1' => 1, 'frame_999' => 'inherit'], //0
			'scaleY'               => ['frame_0' => 1, 'frame_1' => 1, 'frame_999' => 'inherit'], //0
			'opacity'              => ['frame_0' => 0, 'frame_1' => 1, 'frame_999' => 'inherit'], //1
			'rotationX'            => ['frame_0' => 0, 'frame_1' => 0, 'frame_999' => 'inherit'], //0
			'rotationY'            => ['frame_0' => 0, 'frame_1' => 0, 'frame_999' => 'inherit'], //0
			'rotationZ'            => ['frame_0' => 0, 'frame_1' => 0, 'frame_999' => 'inherit'], //0
			'skewX'                => ['frame_0' => 0, 'frame_1' => 0, 'frame_999' => 'inherit'], //0
			'skewY'                => ['frame_0' => 0, 'frame_1' => 0, 'frame_999' => 'inherit'], //0
			'originX'              => ['frame_0' => '50%', 'frame_1' => '50%', 'frame_999' => 'inherit'], //'50%'
			'originY'              => ['frame_0' => '50%', 'frame_1' => '50%', 'frame_999' => 'inherit'], //'50%'
			'originZ'              => ['frame_0' => 0, 'frame_1' => 0, 'frame_999' => 'inherit'], //'0'

			//timeline
			'ease'                 => ['frame_0' => false, 'frame_999' => 'power3.inOut'], //'power3.inOut'
			'start'                => ['frame_0' => false, 'frame_1' => 10, 'frame_999' => true], //0
			'speed'                => ['frame_0' => false, 'frame_999' => 300], //300
		];

		$_split = [
			'ease'      => ['frame_0' => false, 'frame_999' => 'inherit'],
			'direction' => ['frame_0' => false, 'frame_999' => 'forward'], //'forward'
			'delay'     => ['frame_0' => false, 'frame_999' => 5], //5
			'x'         => ['frame_0' => 0, 'frame_1' => 0, 'frame_999' => 'inherit'],
			'y'         => ['frame_0' => 0, 'frame_1' => 0, 'frame_999' => 'inherit'],
			'z'         => ['frame_0' => 0, 'frame_1' => 0, 'frame_999' => 'inherit'],
			'scaleX'    => ['frame_0' => 1, 'frame_1' => 1, 'frame_999' => 'inherit'],
			'scaleY'    => ['frame_0' => 1, 'frame_1' => 1, 'frame_999' => 'inherit'],
			'rotationX' => ['frame_0' => 1, 'frame_1' => 1, 'frame_999' => 'inherit'],
			'rotationY' => ['frame_0' => 1, 'frame_1' => 1, 'frame_999' => 'inherit'],
			'rotationZ' => ['frame_0' => 1, 'frame_1' => 1, 'frame_999' => 'inherit'],
			'skewX'     => ['frame_0' => 0, 'frame_1' => 0, 'frame_999' => 'inherit'],
			'skewY'     => ['frame_0' => 0, 'frame_1' => 0, 'frame_999' => 'inherit'],
			'originX'   => ['frame_0' => '50%', 'frame_1' => '50%', 'frame_999' => 'inherit'], //'50%'
			'originY'   => ['frame_0' => '50%', 'frame_1' => '50%', 'frame_999' => 'inherit'], //'50%'
			'originZ'   => ['frame_0' => 0, 'frame_1' => 0, 'frame_999' => 'inherit'],
		];
		...
		$o['chars'] = $this->get_val($o, 'chars', []);
		$o['words'] = $this->get_val($o, 'words', []);
		$o['lines'] = $this->get_val($o, 'lines', []);
		$o['mask'] = $this->get_val($o, 'mask', []);
		$o['color'] = $this->get_val($o, 'color', []);
		$o['bgcolor'] = $this->get_val($o, 'bgcolor', []);

		$gs = $this->get_val($o, 'grayscale', $this->get_val($_base, ['grayscale', $f], 0));
		$bs = $this->get_val($o, 'brightness', $this->get_val($_base, ['brightness', $f], 100));
		$blr = $this->get_val($o, 'blur', $this->get_val($_base, ['blur', $f], 0));
		$fuse = ((int) ($gs) !== 0 || (int) ($bs) !== 100 || (int) ($blr) !== 0) ? true : false;

		$frame = [
			'alias'            => $this->get_val($o, 'alias', $this->l('On Stage')),
			'filter'           => [
				'use'        => $fuse,
				'grayscale'  => $gs,
				'brightness' => $bs,
			...
			'transform'        => [
				'x'                    => $this->c_to_resp(['default' => $this->get_val($o, 'x', $this->get_val($_base, ['x', $f], 0))]),
				'y'                    => $this->c_to_resp(['default' => $this->get_val($o, 'y', $this->get_val($_base, ['y', $f], 0))]),
				'z'                    => $this->get_val($o, 'z', $this->get_val($_base, ['z', $f], 0)),
				'scaleX'               => $this->get_val($o, 'scaleX', $this->get_val($_base, ['scaleX', $f], 1)),
				'scaleY'               => $this->get_val($o, 'scaleY', $this->get_val($_base, ['scaleY', $f], 1)),
				'opacity'              => $this->get_val($o, 'opacity', $this->get_val($_base, ['opacity', $f], 1)),
				'rotationX'            => $this->get_val($o, 'rotationX', $this->get_val($_base, ['rotationX', $f], 0)),
				'rotationY'            => $this->get_val($o, 'rotationY', $this->get_val($_base, ['rotationY', $f], 0)),
				'rotationZ'            => $this->get_val($o, 'rotationZ', $this->get_val($_base, ['rotationZ', $f], 0)),
				'skewX'                => $this->get_val($o, 'skewX', $this->get_val($_base, ['skewX', $f], 0)),
				'skewY'                => $this->get_val($o, 'skewY', $this->get_val($_base, ['skewY', $f], 0)),
				'originX'              => $this->get_val($o, 'originX', $this->get_val($_base, ['originX', $f], '50%')),
				'originY'              => $this->get_val($o, 'originY', $this->get_val($_base, ['originY', $f], '50%')),
				'originZ'              => $this->get_val($o, 'originY', $this->get_val($_base, ['originZ', $f], '0')),
				'transformPerspective' => $this->get_val($o, 'transformPerspective', $this->get_val($_base, ['transformPerspective', $f], '600px')),
			],
			'reverseDirection' => [
			...
			'mask'             => [
				...
				'x'   => $this->c_to_resp(['default' => $this->get_val($o['mask'], 'x', $this->get_val($_mask, ['x', $f], 0))]),
				'y'   => $this->c_to_resp(['default' => $this->get_val($o['mask'], 'y', $this->get_val($_mask, ['y', $f], 0))]),
			],
			'color'            => [
			...
			'timeline'         => [
				...
				'actionTriggered' => $this->get_val($o, 'actionTriggered', $this->get_val($_base, ['actionTriggered', $f], false)),
				'ease'            => $this->get_val($o, 'ease', $this->get_val($_base, ['ease', $f], '')),
				'speed'           => $this->get_val($o, 'speed', $this->get_val($_base, ['speed', $f], 300)),
				'start'           => $this->get_val($o, 'start', $this->get_val($_base, ['start', $f], 0)),
				'startRelative'   => $this->get_val($o, 'startRelative', 0),
				'endWithSlide'    => $this->get_val($o, 'endWithSlide', false),
			],
			'chars'            => [
				'ease'       => $this->get_val($o['chars'], 'ease', $this->get_val($_split, ['ease', $f], '')),
				'use'        => $this->get_val($o['chars'], 'use', false),
				'direction'  => $this->get_val($o['chars'], 'direction', $this->get_val($_split, ['direction', $f], '')),
				'delay'      => $this->get_val($o['chars'], 'delay', $this->get_val($_split, ['delay', $f], '')),
				'x'          => $this->c_to_resp(['default' => $this->get_val($o['chars'], 'x', $this->get_val($_split, ['x', $f], 'inherit'))]),
				'y'          => $this->c_to_resp(['default' => $this->get_val($o['chars'], 'y', $this->get_val($_split, ['y', $f], 'inherit'))]),
				'z'          => $this->get_val($o['chars'], 'z', $this->get_val($_split, ['z', $f], 'inherit')),
				'scaleX'     => $this->get_val($o['chars'], 'scaleX', $this->get_val($_split, ['scaleX', $f], 'inherit')),
				'scaleY'     => $this->get_val($o['chars'], 'scaleY', $this->get_val($_split, ['scaleY', $f], 'inherit')),
				'opacity'    => $this->get_val($o['chars'], 'opacity', 'inherit'),
				'rotationX'  => $this->get_val($o['chars'], 'rotationX', $this->get_val($_split, ['rotationX', $f], 'inherit')),
				'rotationY'  => $this->get_val($o['chars'], 'rotationY', $this->get_val($_split, ['rotationY', $f], 'inherit')),
				'rotationZ'  => $this->get_val($o['chars'], 'rotationZ', $this->get_val($_split, ['rotationZ', $f], 'inherit')),
				'skewX'      => $this->get_val($o['chars'], 'skewX', $this->get_val($_split, ['skewX', $f], 'inherit')),
				'skewY'      => $this->get_val($o['chars'], 'skewY', $this->get_val($_split, ['skewY', $f], 'inherit')),
				'originX'    => $this->get_val($o['chars'], 'originX', $this->get_val($_split, ['originX', $f], '50%')),
				'originY'    => $this->get_val($o['chars'], 'originY', $this->get_val($_split, ['originY', $f], '50%')),
				'originZ'    => $this->get_val($o['chars'], 'originY', $this->get_val($_split, ['originZ', $f], '0')),
				'fuse'       => $fuse,
				'grayscale'  => $gs,
				'brightness' => $bs,
				'blur'       => $blr,
			...
			'words'            => [
				'ease'       => $this->get_val($o['words'], 'ease', $this->get_val($_split, ['ease', $f], '')),
				'use'        => $this->get_val($o['words'], 'use', false),
				'direction'  => $this->get_val($o['words'], 'direction', $this->get_val($_split, ['direction', $f], '')),
				'delay'      => $this->get_val($o['words'], 'delay', $this->get_val($_split, ['delay', $f], '')),
				'x'          => $this->c_to_resp(['default' => $this->get_val($o['words'], 'x', $this->get_val($_split, ['x', $f], 'inherit'))]),
				'y'          => $this->c_to_resp(['default' => $this->get_val($o['words'], 'y', $this->get_val($_split, ['y', $f], 'inherit'))]),
				'z'          => $this->get_val($o['words'], 'z', $this->get_val($_split, ['z', $f], 'inherit')),
				'scaleX'     => $this->get_val($o['words'], 'scaleX', $this->get_val($_split, ['scaleX', $f], 'inherit')),
				'scaleY'     => $this->get_val($o['words'], 'scaleY', $this->get_val($_split, ['scaleY', $f], 'inherit')),
				'opacity'    => $this->get_val($o['words'], 'opacity', 'inherit'),
				'rotationX'  => $this->get_val($o['words'], 'rotationX', $this->get_val($_split, ['rotationX', $f], 'inherit')),
				'rotationY'  => $this->get_val($o['words'], 'rotationY', $this->get_val($_split, ['rotationY', $f], 'inherit')),
				'rotationZ'  => $this->get_val($o['words'], 'rotationZ', $this->get_val($_split, ['rotationZ', $f], 'inherit')),
				'skewX'      => $this->get_val($o['words'], 'skewX', $this->get_val($_split, ['skewX', $f], 'inherit')),
				'skewY'      => $this->get_val($o['words'], 'skewY', $this->get_val($_split, ['skewY', $f], 'inherit')),
				'originX'    => $this->get_val($o['words'], 'originX', $this->get_val($_split, ['originX', $f], '50%')),
				'originY'    => $this->get_val($o['words'], 'originY', $this->get_val($_split, ['originY', $f], '50%')),
				'originZ'    => $this->get_val($o['words'], 'originY', $this->get_val($_split, ['originZ', $f], '0')),
				'fuse'       => $fuse,
				'grayscale'  => $gs,
				'brightness' => $bs,
				'blur'       => $blr,
			...
			'lines'            => [
				'ease'       => $this->get_val($o['lines'], 'ease', $this->get_val($_split, ['ease', $f], '')),
				'use'        => $this->get_val($o['lines'], 'use', false),
				'direction'  => $this->get_val($o['lines'], 'direction', $this->get_val($_split, ['direction', $f], '')),
				'delay'      => $this->get_val($o['lines'], 'delay', $this->get_val($_split, ['delay', $f], '')),
				'x'          => $this->c_to_resp(['default' => $this->get_val($o['lines'], 'x', $this->get_val($_split, ['x', $f], 'inherit'))]),
				'y'          => $this->c_to_resp(['default' => $this->get_val($o['lines'], 'y', $this->get_val($_split, ['y', $f], 'inherit'))]),
				'z'          => $this->get_val($o['lines'], 'z', $this->get_val($_split, ['z', $f], 'inherit')),
				'scaleX'     => $this->get_val($o['lines'], 'scaleX', $this->get_val($_split, ['scaleX', $f], 'inherit')),
				'scaleY'     => $this->get_val($o['lines'], 'scaleY', $this->get_val($_split, ['scaleY', $f], 'inherit')),
				'opacity'    => $this->get_val($o['lines'], 'opacity', 'inherit'),
				'rotationX'  => $this->get_val($o['lines'], 'rotationX', $this->get_val($_split, ['rotationX', $f], 'inherit')),
				'rotationY'  => $this->get_val($o['lines'], 'rotationY', $this->get_val($_split, ['rotationY', $f], 'inherit')),
				'rotationZ'  => $this->get_val($o['lines'], 'rotationZ', $this->get_val($_split, ['rotationZ', $f], 'inherit')),
				'skewX'      => $this->get_val($o['lines'], 'skewX', $this->get_val($_split, ['skewX', $f], 'inherit')),
				'skewY'      => $this->get_val($o['lines'], 'skewY', $this->get_val($_split, ['skewY', $f], 'inherit')),
				'originX'    => $this->get_val($o['lines'], 'originX', $this->get_val($_split, ['originX', $f], '50%')),
				'originY'    => $this->get_val($o['lines'], 'originY', $this->get_val($_split, ['originY', $f], '50%')),
				'originZ'    => $this->get_val($o['lines'], 'originY', $this->get_val($_split, ['originZ', $f], '0')),
				'fuse'       => $fuse,
				'grayscale'  => $gs,
				'brightness' => $bs,
				'blur'       => $blr,
		...
		if ($frame['sfx']['effect'] === 'blockfromtop') {
			$frame['sfx']['effect'] = 'blocktobottom';
		} else
		if ($frame['sfx']['effect'] === 'blockfromleft') {
			$frame['sfx']['effect'] = 'blocktoright';
		} else
		if ($frame['sfx']['effect'] === 'blockfromright') {
		...
		$check = ['originX', 'originY', 'originZ'];
		$path = ['transform', 'chars', 'words', 'lines'];

		foreach ($path as $_path) {

			foreach ($check as $_check) {

				if (strpos($frame[$_path][$_check], '%') !== false) {
					continue;
				}

				if (strpos(strtolower($frame[$_path][$_check]), 'px') !== false) {
					continue;
				}
		...
		if (!empty($overwrite_with)) {
			$_ign = ['ease', 'speed', 'delay', 'direction', 'color']; //ignore these values and do not take them from the animation template
		...
		if (is_object($newObj['v']) || is_array($newObj['v'])) {
			...
			foreach ($newObj['v'] as $vi => $nov) {
				...
				if (!is_numeric($nov)) {
					...
					if ($i == 0 && strpos($nov, '%') !== false) {
						$newObj['u'] = '%';
					}

					//$newObj['u']		= ($i == 0 && strpos($nov, 'px') !== false) ? 'px' : ($i == 0 && strpos($nov, '%') !== false) ? '%' : $newObj['u'];
					$newObj['v'][$vi] = (int) (str_replace(['%', 'px'], '', $nov)) . $newObj['u'];
			...
			if (!is_numeric($newObj['v'])) {
				...
				if (strpos($newObj['v'], '%') !== false) {
					$newObj['u'] = '%';
				}

				//$newObj['u'] = (strpos($newObj['v'], 'px') !== false) ? 'px' : (strpos($newObj['v'], '%') !== false) ? '%' : $newObj['u'];
				$newObj['v'] = (int) (str_replace(['%', 'px'], '', $newObj['v'])) . $newObj['u'];
			}
	...
	/*
		CREATE A DEFAULT FRAME OBJECT
	*/
	public function default_loop_frame($o = []) {

		$o['frame_0'] = $this->get_val($o, 'frame_0', []);
		$o['frame_999'] = $this->get_val($o, 'frame_999', []);
		...
		$loop = [
			...
			'frame_0'     => [
				'zr'         => $this->get_val($o, ['frame_0', 'zr'], 0),
				'z'          => $this->get_val($o, ['frame_0', 'z'], 0),
				'opacity'    => $this->get_val($o, ['frame_0', 'opacity'], 1),
				'rotationX'  => $this->get_val($o, ['frame_0', 'rotationX'], 0),
				'rotationY'  => $this->get_val($o, ['frame_0', 'rotationY'], 0),
				'skewX'      => $this->get_val($o, ['frame_0', 'skewX'], 0),
				'skewY'      => $this->get_val($o, ['frame_0', 'skewY'], 0),
				'blur'       => 0,
				'brightness' => 100,
				'grayscale'  => 0,
			],
			'frame_999'   => [
				'zr'         => $this->get_val($o, ['frame_999', 'zr'], 0),
				'z'          => $this->get_val($o, ['frame_999', 'z'], 0),
				'opacity'    => $this->get_val($o, ['frame_999', 'opacity'], 1),
				'rotationX'  => $this->get_val($o, ['frame_999', 'rotationX'], 0),
				'rotationY'  => $this->get_val($o, ['frame_999', 'rotationY'], 0),
				'skewX'      => $this->get_val($o, ['frame_999', 'skewX'], 0),
				'skewY'      => $this->get_val($o, ['frame_999', 'skewY'], 0),
				'blur'       => 0,
				'brightness' => 100,
				'grayscale'  => 0,
			],
		...
		case 'rs-rotate':
			...
			$loop['frame_0']['rotationZ'] = $this->get_val($o, ['frame_0', 'rotationZ'], 0);
			$loop['frame_999']['rotationZ'] = $this->get_val($o, ['frame_999', 'rotationZ'], 0);
			$loop['originX'] = $this->get_val($o, 'originX', '50%');
			$loop['originY'] = $this->get_val($o, 'originY', '50%');
			break;
		case 'rs-slideloop':
			$loop['ease'] = $this->get_val($o, 'ease', 'none');
			$loop['frame_0']['x'] = $this->get_val($o, ['frame_0', 'x'], 0);
			$loop['frame_999']['x'] = $this->get_val($o, ['frame_999', 'x'], 0);
			$loop['frame_0']['y'] = $this->get_val($o, ['frame_0', 'y'], 0);
			$loop['frame_999']['y'] = $this->get_val($o, ['frame_999', 'y'], 0);
			break;
		case 'rs-pulse':
			$loop['ease'] = $this->get_val($o, 'ease', 'none');
			$loop['frame_0']['scaleX'] = $this->get_val($o, ['frame_0', 'scaleX'], 1);
			$loop['frame_0']['scaleY'] = $this->get_val($o, ['frame_0', 'scaleX'], 1);
			$loop['frame_999']['scaleX'] = $this->get_val($o, ['frame_999', 'scaleX'], 1);
			$loop['frame_999']['scaleY'] = $this->get_val($o, ['frame_999', 'scaleY'], 1);
			break;
		case 'rs-wave':
			$loop['frame_0']['xr'] = $this->get_val($o, ['frame_0', 'xr'], 0);
			$loop['frame_0']['yr'] = $this->get_val($o, ['frame_0', 'yr'], 0);
			$loop['frame_999']['xr'] = $this->get_val($o, ['frame_999', 'xr'], 0);
			$loop['frame_999']['yr'] = $this->get_val($o, ['frame_999', 'yr'], 0);
			$loop['originX'] = $this->get_val($o, 'originX', '50%');
			$loop['originY'] = $this->get_val($o, 'originY', '50%');
			break;
		}
		...
		if (!empty($o)) {
			...
			if (is_array($o) || is_object($o)) {
				...
				foreach ($o as $key => $v) {
					...
					if ($key === 'frameOrder' || $key === 'alias' || $key === 'intelligentInherit') {
						//with in_array we receive unexpected results
						continue;
					}
					...
					if (!is_array($o[$key]) && !is_object($o[$key])) {
						...
						if (!is_array($check) && !is_object($check) && (string) $check == (string) $o[$key]) {
							//int(0) is same as '##', so check by casting them to string
					...
					if ($this->get_val($emp, $key, '######') !== '######') {

						if (in_array($key, ['v', 'borderWidth'], true) && is_array($o[$key])) {
							...
							if ($key === 'idle') {
								//we check for idle, as we want to leave backgroundColor within idle
							...
							if ($key === 'idle') {
								//we check for idle, as we want to leave backgroundColor within idle and we can set it only back to false if we are out of the idle tree
	...
	public function _simplify_layers($_, $slide, $slider) {
		...
		if (in_array($this->get_val($_, 'uid', false), ['top', 'bottom', 'middle', 'zone'])) {
			return $_;
		}

		if (in_array($this->get_val($_, 'type', false), ['top', 'bottom', 'middle', 'zone'])) {
			return $_;
		}
	 ...
	 * @since: 6.0.0
	 **/
	public function isEmptyObject($vars) {
