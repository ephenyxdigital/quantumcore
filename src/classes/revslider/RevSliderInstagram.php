...
class RevSliderInstagram extends RevSliderFunction {
	...
	public function get_public_photos($token, $count, $orig_image = '') {
		...
		if (!empty($this->stream)) {
			...
			RevLoader::_e('Instagram reports: Please check the settings');
			return false;
		}
		...
		foreach ($photos as $photo) {
			...
			if ($count > 0) {
				...
				if ($orig_image) {
					...
					$orig_image = [$images->src, $images->config_width, $images->config_height];
				} else {
					$orig_image = ['', 0, 0];
				}
				...
				if (isset($photo->tags)) {
					$stream['tags'] = implode(',', $photo->tags);
				}
