...
class RevSliderCssParser extends RevSliderFunction {
	...
	public function get_classes($start_text = '', $end_text = '', $explodeonspace = false) {
		...
		foreach ($lines as $key => $line) {
			...
			$class = str_replace(['.caption.', '.tp-caption.'], '.', $class);
			...
			if (is_array($styles) || is_array($styles_adv)) {
				...
				if (is_array($styles)) {
					...
					foreach ($styles as $name => $style) {
						...
						if (in_array($name, $deformations) && $name !== 'cursor') {
							continue;
						}
	...
	public function db_array_to_array($css_array, $handle = false) {
		...
		if (!is_array($css_array) || empty($css_array)) {
			return false;
		}
	...
	public function get_captions_sorted() {
		...
		foreach ($styles as $style) {
			...
			if (array_key_exists('version', $setting) && isset($setting['version'])) {
				$arr[ucfirst($setting['version'])][] = ['label' => trim(str_replace('.tp-caption.', '', $style['handle'])), 'type' => $setting['type']];
			}
		...
		foreach ($arr as $version => $class) {
			...
			foreach ($class as $name) {
				$sorted[] = ['label' => $this->get_val($name, 'label'), 'version' => $version, 'type' => $this->get_val($name, 'type')];
			}
			...
			foreach ($css as $class => $styles) {
				...
				if (strpos($class, ':hover') !== false) {
					...
					$add = [
						...
						'settings' => json_encode(['hover' => 'true']),
					];
				} else {
					$add = [
				...
				$result = Db::getInstance()->getRow(Db::getInstance()->prepare("SELECT * FROM " . _DB_PREFIX_ . RevSliderFront::TABLE_CSS . " WHERE handle = %s", $class), true);
				...
				if (!empty($result)) {
					...
					//insert
					$add['handle'] = $class;
					Db::getInstance()->insert(RevSliderFront::TABLE_CSS, $add);
				}
	...
	public function get_base_css_captions() {
		...
		include DIGITAL_CORE_DIR . '/vendor/basic-css.php';
