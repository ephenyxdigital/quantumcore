...
class RevSliderNavigation extends RevSliderFunction {
	...
	public function get_all_navigations($defaults = true, $raw = false, $old = false) {
		...
		if ($raw == false) {
			...
			foreach ($navigations as $key => $nav) {
				...
				if (isset($navigations[$key]['settings'])) {
					$navigations[$key]['settings'] = RevSliderFunction::stripslashes_deep(json_decode($navigations[$key]['settings'], true));
		...
		if ($defaults) {
			...
			$default_presets = RevLoader::get_option('revslider-nav-preset-default', []);
							...
							foreach ($default_presets as $id => $v) {
								...
								if (!empty($v)) {
									...
									foreach ($v as $pr_v) {
										...
										if ($this->get_val($pr_v, 'type') !== $def[$key]['type']) {
											continue;
										}

										$def[$key]['settings']['presets'][$this->get_val($pr_v, 'name')] = [
	...
	public function create_update_full_navigation($data) {
		if (!empty($data) && is_array($data)) {
			...
			foreach ($data as $vals) {
				...
				if (!isset($vals['markup']) || !isset($vals['css'])) {
					continue;
				}

				if ($this->get_val($vals, 'factory', false) == 'true') {
					continue;
				}
				...
				if (isset($vals['id'])) {
					//new will be added temporary to navs to tell here that they are new
	...
	public static function get_default_navigations() {
		...
		include DIGITAL_CORE_DIR . '/vendor/navigations.php';
	...
	public function export_navigation($nav_handle) {
		...
		if (!is_array($nav_handle)) {
			$nav_handle = [$nav_handle => true];
		}
	...
	public function add_placeholder_modifications($def_navi, $slider, $output) {
		...
		if (!is_array($def_navi)) {
			$def_navi = json_decode($def_navi, true);
		}
		...
		if (!in_array($type, ['arrows', 'bullets', 'thumbs', 'tabs'])) {
			return $css;
		}
		...
		if (!empty($c)) {
			...
			if (!empty($placeholders)) {
				...
				foreach ($placeholders as $k => $d) {
					...
					if ($slide->get_param(['nav', $type, 'presets', $k . '-def'], false) === true) {
						//get from Slide
						...
						foreach ($nav['presets'] as $prkey => $preset) {
							...
							if ($prkey == $data['handle']) {
								...
								if (!isset($preset['editable'])) {
									return RevLoader::__("Can't modify a default preset of default navigations");
								}
						...
						foreach ($default_presets[$nav['id']] as $prkey => $preset) {
							...
							if ($prkey == $data['handle']) {
								...
								if ($data['do_overwrite'] === false || $data['do_overwrite'] === 'false') {
									return $this->l('Preset handle already exists, please choose a different name');
								}

								$default_presets[$nav['id']][$prkey] = [
...
$default_presets = array();
					...
					//return $this->l('Can\'t add a preset to default navigations');
				} else {
		...
		return $this->l('Navigation not found, could not add preset');
	...
	public function delete_preset($data) {
		...
		if (!isset($data['style_handle']) || !isset($data['handle']) || !isset($data['type'])) {
			return false;
		}
		...
		foreach ($navs as $nav) {
			...
			if ($nav['id'] != $data['style_handle']) {
				continue;
			}

			if ($nav['type'] != $data['type']) {
				continue;
			}
			...
			if (isset($nav['factory']) && $nav['factory'] == true) {
				...
				if (!empty($default_presets) && isset($default_presets[$nav['id']])) {
					...
					return $this->l('Can\'t delete default preset of default navigations');
				}

				return $this->l('Preset not found in default navigations');
			} else {
		...
		return $this->l('Navigation not found, could not delete preset');
	}
