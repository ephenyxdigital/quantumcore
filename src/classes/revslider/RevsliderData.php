...
class RevsliderData {
	...
	public function get_font_familys() {
		...
		//Web Safe Fonts
		// GOOGLE Loaded Fonts
		$fonts[] = ['type' => 'websafe', 'version' => $this->l('Loaded Google Fonts'), 'label' => 'Dont Show Me'];

		//Serif Fonts
		$fonts[] = ['type' => 'websafe', 'version' => $this->l('Serif Fonts'), 'label' => 'Georgia, serif'];
		$fonts[] = ['type' => 'websafe', 'version' => $this->l('Serif Fonts'), 'label' => '"Palatino Linotype", "Book Antiqua", Palatino, serif'];
		$fonts[] = ['type' => 'websafe', 'version' => $this->l('Serif Fonts'), 'label' => '"Times New Roman", Times, serif'];

		//Sans-Serif Fonts
		$fonts[] = ['type' => 'websafe', 'version' => $this->l('Sans-Serif Fonts'), 'label' => 'Arial, Helvetica, sans-serif'];
		$fonts[] = ['type' => 'websafe', 'version' => $this->l('Sans-Serif Fonts'), 'label' => '"Arial Black", Gadget, sans-serif'];
		$fonts[] = ['type' => 'websafe', 'version' => $this->l('Sans-Serif Fonts'), 'label' => '"Comic Sans MS", cursive, sans-serif'];
		$fonts[] = ['type' => 'websafe', 'version' => $this->l('Sans-Serif Fonts'), 'label' => 'Impact, Charcoal, sans-serif'];
		$fonts[] = ['type' => 'websafe', 'version' => $this->l('Sans-Serif Fonts'), 'label' => '"Lucida Sans Unicode", "Lucida Grande", sans-serif'];
		$fonts[] = ['type' => 'websafe', 'version' => $this->l('Sans-Serif Fonts'), 'label' => 'Tahoma, Geneva, sans-serif'];
		$fonts[] = ['type' => 'websafe', 'version' => $this->l('Sans-Serif Fonts'), 'label' => '"Trebuchet MS", Helvetica, sans-serif'];
		$fonts[] = ['type' => 'websafe', 'version' => $this->l('Sans-Serif Fonts'), 'label' => 'Verdana, Geneva, sans-serif'];

		//Monospace Fonts
		$fonts[] = ['type' => 'websafe', 'version' => $this->l('Monospace Fonts'), 'label' => '"Courier New", Courier, monospace'];
		$fonts[] = ['type' => 'websafe', 'version' => $this->l('Monospace Fonts'), 'label' => '"Lucida Console", Monaco, monospace'];

		//push all variants to the websafe fonts

		foreach ($fonts as $f => $font) {

			if (!empty($cfl) && is_array($cfl) && $font['type'] === 'custom') {
				continue;
			}

			//already manually added before on these
	...
	public function get_animations_v5() {
		...
		foreach ($this->animations as $value) {
			...
			if (!in_array($type, ['customout', 'customin'])) {
				continue;
			}
	...
	public function fill_animations() {
		...
		if (empty($this->animations)) {
			...
			$result = Db::getInstance()->executeS(
				(new DbQuery())
					->select('*')
					->from('revslider_layer_animations')
			);

			$this->animations = (!empty($result)) ? $result : [];
			...
			if (!empty($this->animations)) {
				array_walk_recursive($this->animations, ['RevSliderData', 'force_to_boolean']);
			}
	...
	public function fill_css() {
		...
		if (empty($this->css)) {
			...
			$css_data = Db::getInstance()->executeS(
				(new DbQuery())
					->select('*')
					->from('revslider_css')
			);

			$this->css = (!empty($css_data)) ? $css_data : [];
		}
	...
	public function get_all_image_sizes($type = 'gallery') {
		...
		case 'flickr':
			$custom_sizes = [
				'original'     => $this->l('Original'),
				'large'        => $this->l('Large'),
				'large-square' => $this->l('Large Square'),
				'medium'       => $this->l('Medium'),
				'medium-800'   => $this->l('Medium 800'),
				'medium-640'   => $this->l('Medium 640'),
				'small'        => $this->l('Small'),
				'small-320'    => $this->l('Small 320'),
		...
		case 'instagram':
			$custom_sizes = [
				'standard_resolution' => $this->l('Standard Resolution'),
				'thumbnail'           => $this->l('Thumbnail'),
				'low_resolution'      => $this->l('Low Resolution'),
				'original_size'       => $this->l('Original Size'),
				'large'               => $this->l('Large Size'),
			];
			break;
		case 'twitter':
			$custom_sizes = [
				'large' => $this->l('Standard Resolution'),
			];
			break;
		case 'facebook':
			$custom_sizes = [
				'full'      => $this->l('Original Size'),
				'thumbnail' => $this->l('Thumbnail'),
			];
			break;
		case 'youtube':
			$custom_sizes = [
				...
				'maxres'   => $this->l('Max. Res.'),
			];
			break;
		case 'vimeo':
			$custom_sizes = [
				'thumbnail_large'  => $this->l('Large'),
				'thumbnail_medium' => $this->l('Medium'),
				'thumbnail_small'  => $this->l('Small'),
			];
			break;
		case 'gallery':
		default:
