...
if (!empty($my_query)) {
	foreach ($my_query as $qpost) {
		...
		$post_link = smartblog::GetSmartBlogLink('smartblog_post', ['id_post' => $post_id, 'slug' => $qpost['link_rewrite']]);
...
$output .= widget_title(['title' => $title, 'extraclass' => 'wpb_posts_slider_heading']);
